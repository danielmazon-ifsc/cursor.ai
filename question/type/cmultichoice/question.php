 ...
 * @author     Ricardo Drummond
 */
abstract class qtype_cmultichoice_base extends question_graded_automatically {
    ...
    public function apply_attempt_state(question_attempt_step $step) {
        ...
        foreach ($this->order as $ansid) {
            ...
            $a = new stdClass();
            $a->id = 0;
            $a->answer = html_writer::span(get_string('deletedchoice', 'qtype_cmultichoice'), 'notifyproblem');
    ...
    public function get_question_summary() {
        ...
        $choices = array();
        foreach ($this->order as $ansid) {
            $choices[] = $this->html_to_text($this->answers[$ansid]->answer, $this->answers[$ansid]->answerformat);
        }
    ...
    /**
     * Retrieve competencies associated with this question
     * 
     * @param int $answerid Id of answer to be considered
     * @return array of object Competencies
     */
    public function get_competencies($answerid = null) {
        global $DB;
        // Try to find competencies associated with a specific answer first
        if ($answerid) {
            $sql = "SELECT c.* "
                    . "FROM {competency} c, {qtype_cmultichoice_compts} q "
                    . "WHERE q.competencyid = c.id "
                    . "AND q.answerid = :answerid";
            $competencies = $DB->get_records_sql($sql, array('answerid' => $answerid));
        } else {
            $competencies = null;
        }
        // If not found, try to find competencies associated with this question
        if (!$competencies) {
            $sql = "SELECT c.* "
                    . "FROM {competency} c, {qtype_cmultichoice_compts} q "
                    . "WHERE q.competencyid = c.id "
                    . "AND q.questionid = :questionid";
            $competencies = $DB->get_records_sql($sql, array('questionid' => $this->id));
        }
        return $competencies;
    }
 ...
 * @author     Ricardo Drummond
 */
class qtype_cmultichoice_single_question extends qtype_cmultichoice_base {

    public function get_renderer(moodle_page $page) {
        return $page->get_renderer('qtype_cmultichoice', 'single');
    }
    ...
    public function classify_response(array $response) {
        ...
        return array($this->id => new question_classified_response($choiceid, $this->html_to_text($ans->answer, $ans->answerformat), $ans->fraction));
    }
    ...
    public function get_validation_error(array $response) {
        ...
        return get_string('pleaseselectananswer', 'qtype_cmultichoice');
    }
 ...
 * @author     Ricardo Drummond
 */
class qtype_cmultichoice_multi_question extends qtype_cmultichoice_base {

    public function get_renderer(moodle_page $page) {
        return $page->get_renderer('qtype_cmultichoice', 'multi');
    }
    ...
    public function summarise_response(array $response) {
        ...
        foreach ($this->order as $key => $ans) {
            $fieldname = $this->field($key);
            if (array_key_exists($fieldname, $response) && $response[$fieldname]) {
                $selectedchoices[] = $this->html_to_text($this->answers[$ans]->answer, $this->answers[$ans]->answerformat);
            }
        }
    ...
    public function classify_response(array $response) {
        ...
        foreach ($this->answers as $ansid => $ans) {
            if (isset($selectedchoices[$ansid])) {
                $choices[$ansid] = new question_classified_response($ansid, $this->html_to_text($ans->answer, $ans->answerformat), $ans->fraction);
            }
        }
        return $choices;
    }
    ...
    public function get_validation_error(array $response) {
        ...
        return get_string('pleaseselectatleastoneanswer', 'qtype_cmultichoice');
    }
