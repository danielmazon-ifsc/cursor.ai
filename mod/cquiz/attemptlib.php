...
/**
 * Back-end code for handling data about cquizzes and the current user's attempt.
 *
 * There are classes for loading all the information about a cquiz and attempts,
...
/**
 * Class for cquiz exceptions. Just saves a couple of arguments on the
 ...
 * @since     Moodle 2.0
 */
class moodle_cquiz_exception extends moodle_exception {

    public function __construct($cquizobj, $errorcode, $a = null, $link = '', $debuginfo = null) {
        if (!$link) {
            $link = $cquizobj->view_url();
        }
        parent::__construct($errorcode, 'cquiz', $link, $a, $debuginfo);
    }

}

/**
 * A class encapsulating a cquiz and the questions it contains, and making the
...
class cquiz {
    ...
    /** @var stdClass the cquiz settings from the database. */
    protected $cquiz;
    ...
    /** @var array of cquiz_section rows. */
    protected $sections = null;

    /** @var cquiz_access_manager the access manager for this cquiz. */
    protected $accessmanager = null;

    /** @var bool whether the current user has capability mod/cquiz:preview. */
     ...
     *
     * @param object $cquiz the row from the cquiz table.
     * @param object $cm the course_module object for this cquiz.
    ...
    /**
     * Static function to create a new cquiz object for a specific user.
     *
     * @param int $cquizid the the cquiz id.
     * @param int $userid the the userid.
     * @return cquiz the new cquiz object
     */
    public static function create($cquizid, $userid = null) {
        global $DB;

        $cquiz = cquiz_access_manager::load_cquiz_and_settings($cquizid);
        $course = $DB->get_record('course', array('id' => $cquiz->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('cquiz', $cquiz->id, $course->id, false, MUST_EXIST);

        // Update cquiz with override information.
        if ($userid) {
            $cquiz = cquiz_update_effective_access($cquiz, $userid);
        }

        return new cquiz($cquiz, $cm, $course);
    }

    /**
     * Create a {@link cquiz_attempt} for an attempt at this cquiz.
     * @param object $attemptdata row from the cquiz_attempts table.
     * @return cquiz_attempt the new cquiz_attempt object.
     */
    public function create_attempt_object($attemptdata) {
        return new cquiz_attempt($attemptdata, $this->cquiz, $this->cm, $this->course);
    }

    // Functions for loading more data =========================================

    /**
     * Load just basic information about all the questions in this cquiz.
     */
    public function preload_questions() {
        $this->questions = question_preload_questions(null, 'slot.maxmark, slot.id AS slotid, slot.slot, slot.page', '{cquiz_slots} slot ON slot.cquizid = :cquizid AND q.id = slot.questionid', array('cquizid' => $this->cquiz->id), 'slot.slot');
    }

    /**
     * Fully load some or all of the questions for this cquiz. You must call
    ...
    /**
     * Get an instance of the {@link \mod_cquiz\structure} class for this cquiz.
     * @return \mod_cquiz\structure describes the questions in the cquiz.
     */
    public function get_structure() {
        return \mod_cquiz\structure::create_for_cquiz($this);
    }
    ...
    /** @return int the cquiz id. */
    public function get_cquizid() {
        return $this->cquiz->id;
    }

    /** @return object the row of the cquiz table. */
    public function get_cquiz() {
        return $this->cquiz;
    }

    /** @return string the name of this cquiz. */
    public function get_cquiz_name() {
        return $this->cquiz->name;
    }

    /** @return int the cquiz navigation method. */
    public function get_navigation_method() {
        return $this->cquiz->navmethod;
    }

    /** @return int the number of attempts allowed at this cquiz (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->cquiz->attempts;
    }
    ...
    /** @return object the module context for this cquiz. */
    public function get_context() {
        return $this->context;
    }

    /**
     * @return bool wether the current user is someone who previews the cquiz,
     * rather than attempting it.
     */
    public function is_preview_user() {
        if (is_null($this->ispreviewuser)) {
            $this->ispreviewuser = has_capability('mod/cquiz:preview', $this->context);
        }
        return $this->ispreviewuser;
    }

    /**
     * @return whether any questions have been added to this cquiz.
     */
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new moodle_exception('cannotstartmissingquestion', 'cquiz', $this->view_url());
            }
    ...
    /**
     * Get all the sections in this cquiz.
     * @return array 0, 1, 2, ... => cquiz_sections row from the database.
     */
    public function get_sections() {
        global $DB;
        if ($this->sections === null) {
            $this->sections = array_values($DB->get_records('cquiz_sections', array('cquizid' => $this->get_cquizid()), 'firstslot'));
        }
        return $this->sections;
    }

    /**
     * Return cquiz_access_manager and instance of the cquiz_access_manager class
     * for this cquiz at this time.
    ...
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new cquiz_access_manager($this, $timenow, has_capability('mod/cquiz:ignoretimelimits', $this->context, null, false));
        }
        return $this->accessmanager;
    }

    /**
     * Wrapper round the has_capability funciton that automatically passes in the cquiz context.
     */
    ...
    /**
     * Wrapper round the require_capability funciton that automatically passes in the cquiz context.
     */
    ...
    /**
     * @return string the URL of this cquiz's view page.
     */
    public function view_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/cquiz/view.php?id=' . $this->cm->id;
    }

    /**
     * @return string the URL of this cquiz's edit page.
     */
    public function edit_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/cquiz/edit.php?cmid=' . $this->cm->id;
    }

    /**
     * @param int $attemptid the id of an attempt.
     * @param int $page optional page number to go to in the attempt.
     * @param bool $preview optional preview attemp
     * @return string the URL of that attempt.
     */
    public function attempt_url($attemptid, $page = 0, $preview = 0) {
        global $CFG;
        $url = $CFG->wwwroot . '/mod/cquiz/attempt.php?attempt=' . $attemptid;
        if ($page) {
            $url .= '&page=' . $page;
        }
        $url .= '&preview=' . $preview;
    ...
    /**
     * @return string the URL of this cquiz's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($page = 0, $preview = 0) {
        $params = array('cmid' => $this->cm->id, 'sesskey' => sesskey(), 'preview' => $preview);
        if ($page) {
            $params['page'] = $page;
        }
        return new moodle_url('/mod/cquiz/startattempt.php', $params);
    }
    ...
    public function review_url($attemptid) {
        return new moodle_url('/mod/cquiz/review.php', array('attempt' => $attemptid));
    }
    ...
    public function summary_url($attemptid) {
        return new moodle_url('/mod/cquiz/summary.php', array('attempt' => $attemptid));
    }
     ...
     * @deprecated since 3.1. This sort of functionality is now entirely handled by cquiz access rules.
     */
    public function confirm_start_attempt_message($notused) {
        debugging('confirm_start_attempt_message is deprecated. ' .
                'This sort of functionality is now entirely handled by cquiz access rules.');
        return '';
    }
     ...
     *
     * @param int $when One of the mod_cquiz_display_options::DURING,
    ...
    public function cannot_review_message($when, $short = false) {
        ...
        if ($when == mod_cquiz_display_options::DURING ||
                $when == mod_cquiz_display_options::IMMEDIATELY_AFTER) {
            return '';
        } else if ($when == mod_cquiz_display_options::LATER_WHILE_OPEN && $this->cquiz->timeclose &&
                $this->cquiz->reviewattempt & mod_cquiz_display_options::AFTER_CLOSE) {
            return get_string('noreviewuntil' . $langstrsuffix, 'cquiz', userdate($this->cquiz->timeclose, $dateformat));
        } else {
            return get_string('noreview' . $langstrsuffix, 'cquiz');
        }
    }

    /**
     * @param string $title the name of this particular cquiz page.
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new moodle_cquiz_exception($this, 'questionnotloaded', $id);
        }
    }

    /**
     * Return all the question types used in this cquiz.
     *
     * @param  boolean $includepotential if the cquiz include random questions, setting this flag to true will make the function to
    ...
    public function get_all_question_types_used($includepotential = false) {
        ...
        // We must be careful with random questions, if we find a random question we must assume that the cquiz may content
...
/**
 * This class extends the cquiz class to hold data about the state of a particular attempt,
 * in addition to the data about the cquiz.
 *
 * @copyright  2017 Viddia (http://viddia.com.br)
 * @author     Ricardo Drummond
 * @since      Moodle 2.0
...
class cquiz_attempt {
    ...
    /** @var int maximum number of slots in the cquiz for the review page to default to show all. */
    const MAX_SLOTS_FOR_DEFAULT_REVIEW_SHOW_ALL = 50;

    /** @var cquiz object containing the cquiz settings. */
    protected $cquizobj;

    /** @var stdClass the cquiz_attempts row. */
    protected $attempt;

    /** @var question_usage_by_activity the question usage for this cquiz attempt. */
    protected $quba;
    ...
    /** @var array of cquiz_sections rows, with a ->lastslot field added. */
    protected $sections;
    ...
    /** @var mod_cquiz_display_options cache for the appropriate review options. */
     ...
     * Constructor assuming we already have the necessary data loaded.
     *
     * @param object $attempt the row of the cquiz_attempts table.
     * @param object $cquiz the cquiz object for this attempt and user.
     * @param object $cm the course_module object for this cquiz.
     * @param object $course the row from the course table for the course we belong to.
     ...
     */
    public function __construct($attempt, $cquiz, $cm, $course, $loadquestions = true) {
        global $DB;

        $this->attempt = $attempt;
        $this->cquizobj = new cquiz($cquiz, $cm, $course);
        ...
        $this->slots = $DB->get_records('cquiz_slots', array('cquizid' => $this->get_cquizid()), 'slot', 'slot, requireprevious, questionid');
        $this->sections = array_values($DB->get_records('cquiz_sections', array('cquizid' => $this->get_cquizid()), 'firstslot'));
     ...
     * @param array $conditions passed to $DB->get_record('cquiz_attempts', $conditions).
     */
    protected static function create_helper($conditions) {
        global $DB;

        $attempt = $DB->get_record('cquiz_attempts', $conditions, '*', MUST_EXIST);
        $cquiz = cquiz_access_manager::load_cquiz_and_settings($attempt->cquiz);
        $course = $DB->get_record('course', array('id' => $cquiz->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('cquiz', $cquiz->id, $course->id, false, MUST_EXIST);

        // Update cquiz with override information.
        $cquiz = cquiz_update_effective_access($cquiz, $attempt->userid);

        return new cquiz_attempt($attempt, $cquiz, $cm, $course);
    }

    /**
     * Static function to create a new cquiz_attempt object given an attemptid.
     *
    ...
    /**
     * Static function to create a new cquiz_attempt object given a usage id.
     *
    ...
    public static function state_name($state) {
        return cquiz_attempt_state_name($state);
    }
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                if ($length = $this->is_real_question($slot)) {
                    ...
                    $this->questionnumbers[$slot] = get_string('infoshort', 'cquiz');
                }
    ...
    // Simple getters ==========================================================
    public function get_cquiz() {
        return $this->cquizobj->get_cquiz();
    }

    public function get_cquizobj() {
        return $this->cquizobj;
    }

    /** @return int the course id. */
    public function get_courseid() {
        return $this->cquizobj->get_courseid();
    }

    /** @return int the course id. */
    public function get_course() {
        return $this->cquizobj->get_course();
    }

    /** @return int the cquiz id. */
    public function get_cquizid() {
        return $this->cquizobj->get_cquizid();
    }

    /** @return string the name of this cquiz. */
    public function get_cquiz_name() {
        return $this->cquizobj->get_cquiz_name();
    }

    /** @return int the cquiz navigation method. */
    public function get_navigation_method() {
        return $this->cquizobj->get_navigation_method();
    }

    /** @return object the course_module object. */
    public function get_cm() {
        return $this->cquizobj->get_cm();
    }

    /** @return object the course_module object. */
    public function get_cmid() {
        return $this->cquizobj->get_cmid();
    }
    ...
    /** @return int the number of attempts allowed at this cquiz (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->cquizobj->get_num_attempts_allowed();
    }

    /** @return int number fo pages in this cquiz. */
     ...
     */
    public function get_access_manager($timenow) {
        return $this->cquizobj->get_access_manager($timenow);
    }
    ...
    /** @return object the row from the cquiz_attempts table. */
    ...
    /** @return string one of the cquiz_attempt::IN_PROGRESS, FINISHED, OVERDUE or ABANDONED constants. */
    ...
    public function is_review_allowed() {
        if (!$this->has_capability('mod/cquiz:viewreports')) {
            return false;
        }
    ...
    /**
     * Has the student, in this attempt, engaged with the cquiz in a non-trivial way?
     ...
     * To see how this works, try setting a cquiz to one of the CBM behaviours,
     * and then look at the extra information displayed at the top of the cquiz
     ...
     *
     * @param question_display_options $options the display options for this cquiz attempt at this time.
    ...
    public function get_overall_feedback($grade) {
        return cquiz_feedback_for_grade($grade, $this->get_cquiz(), $this->cquizobj->get_context());
    }
     ...
     */
    public function has_capability($capability, $userid = null, $doanything = true) {
        return $this->cquizobj->has_capability($capability, $userid, $doanything);
    }
     ...
     */
    public function require_capability($capability, $userid = null, $doanything = true) {
        return $this->cquizobj->require_capability($capability, $userid, $doanything);
    }
    ...
    public function check_review_capability() {
        if ($this->get_attempt_state() == mod_cquiz_display_options::IMMEDIATELY_AFTER) {
            $capability = 'mod/cquiz:attempt';
        } else {
            $capability = 'mod/cquiz:reviewmyattempts';
        }

        // These next tests are in a slighly funny order. The point is that the
        // common and most performance-critical case is students attempting a cquiz
        // so we want to check that permisison first.

        if ($this->has_capability($capability)) {
            // User has the permission that lets you do the cquiz as a student. Fine.
            return;
        }

        if ($this->has_capability('mod/cquiz:viewreports') ||
                $this->has_capability('mod/cquiz:preview')) {
    ...
    public function can_navigate_to($slot) {
        switch ($this->get_navigation_method()) {
            case CQUIZ_NAVMETHOD_FREE:
                return true;
                break;
            case CQUIZ_NAVMETHOD_SEQ:
                return false;
                break;
        }
        return true;
    ...
    public function get_attempt_state() {
        return cquiz_attempt_state($this->get_cquiz(), $this->attempt);
    }

    /**
     * Wrapper that the correct mod_cquiz_display_options for this cquiz at the
     * moment.
     *
    ...
    public function get_display_options($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = cquiz_get_review_options($this->get_cquiz(), $this->attempt, $this->cquizobj->get_context());
            ...
            $options = mod_cquiz_display_options::make_from_cquiz($this->get_cquiz(), mod_cquiz_display_options::DURING);
            $options->flags = cquiz_get_flag_option($this->attempt, $this->cquizobj->get_context());
            return $options;
        }
    }
     ...
     * @return bool true if this is the last page of the cquiz.
     */
    ...
    /**
     * Return the list of slot numbers for either a given page of the cquiz, or for the
     * whole cquiz.
     *
    ...
    public function is_blocked_by_previous_question($slot) {
        return $slot > 1 && isset($this->slots[$slot]) && $this->slots[$slot]->requireprevious &&
                ...
                $this->get_navigation_method() != CQUIZ_NAVMETHOD_SEQ &&
    ...
    public function can_question_be_redone_now($slot) {
        return $this->get_cquiz()->canredoquestions && !$this->is_finished() &&
    ...
    /**
     * Return the page of the cquiz where this question appears.
     * @param int $slot the number used to identify this question within this attempt.
     * @return int the page of the cquiz this question appears on.
     */
     ...
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return string the formatted grade, to the number of decimal places specified by the cquiz.
     */
    public function get_question_mark($slot) {
        return cquiz_format_question_grade($this->get_cquiz(), $this->quba->get_question_mark($slot));
    }
    ...
    public function get_due_date() {
        $deadlines = array();
        if ($this->cquizobj->get_cquiz()->timelimit) {
            $deadlines[] = $this->attempt->timestart + $this->cquizobj->get_cquiz()->timelimit;
        }
        if ($this->cquizobj->get_cquiz()->timeclose) {
            $deadlines[] = $this->cquizobj->get_cquiz()->timeclose;
        }
        if ($deadlines) {
        ...
        switch ($this->attempt->state) {
            ...
            case self::OVERDUE:
                return $duedate + $this->cquizobj->get_cquiz()->graceperiod;
    ...
    // URLs related to this attempt ============================================
    /**
     * @return string cquiz view url.
     */
    public function view_url() {
        return $this->cquizobj->view_url();
    }

    /**
     * @return string the URL of this cquiz's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($slot = null, $page = -1, $preview = 0) {
        if ($page == -1 && !is_null($slot)) {
            $page = $this->get_question_page($slot);
        } else {
            $page = 0;
        }
        return $this->cquizobj->start_attempt_url($page, $preview);
    }
    ...
    /**
     * @return string the URL of this cquiz's summary page.
     */
    public function summary_url() {
        return new moodle_url('/mod/cquiz/summary.php', array('attempt' => $this->attempt->id));
    }
    ...
    public function processattempt_url() {
        return new moodle_url('/mod/cquiz/processattempt.php');
    }
    ...
    public function cannot_review_message($short = false) {
        return $this->cquizobj->cannot_review_message(
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new moodle_url(
                            $this->start_attempt_url(null, 0, true), array('forcenew' => true)), get_string('startnewpreview', 'cquiz'));
        } else {
            return '';
        }
    }
     ...
     * @param mod_cquiz_renderer $renderer the cquiz renderer.
     ...
     */
    public function render_question($slot, $reviewing, mod_cquiz_renderer $renderer, $thispageurl = null) {
        if ($this->is_blocked_by_previous_question($slot)) {
            ...
            return html_writer::div($placeholderqa->render($displayoptions, $this->get_question_number($this->get_original_slot($slot))), 'mod_cquiz-blocked_question_warning');
        }
     ...
     */
    protected function render_question_helper($slot, $reviewing, $thispageurl, mod_cquiz_renderer $renderer, $seq) {
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $displayoptions->extrahistorycontent = html_writer::tag('p', get_string('redoesofthisquestion', 'cquiz', $renderer->render($links)));
            }
        }
    ...
    protected function make_blocked_question_placeholder($slot) {
        ...
        $question->name = '';
        $question->questiontext = get_string('questiondependsonprevious', 'cquiz');
        ...
        $placeholderqa->start($this->get_cquiz()->preferredbehaviour, 1);
     ...
     *
     * @param int $id the id of a question in this cquiz attempt.
     ...
     * @return string HTML for the question in its current state.
     */
    public function render_question_at_step($slot, $seq, $reviewing, mod_cquiz_renderer $renderer, $thispageurl = '') {
     ...
     *
     * @param $panelclass The type of panel, cquiz_attempt_nav_panel or cquiz_review_nav_panel
     * @param $page the current page number.
     * @param $showall whether we are showing the whole cquiz on one page. (Used by review.php)
     * @return cquiz_nav_panel_base the requested object.
     */
    public function get_navigation_panel(mod_cquiz_renderer $output, $panelclass, $page, $showall = false) {
        $panel = new $panelclass($this, $this->get_display_options(true), $page, $showall);

        $bc = new block_contents();
        $bc->attributes['id'] = 'mod_cquiz_navblock';
        $bc->attributes['role'] = 'navigation';
        $bc->attributes['aria-labelledby'] = 'mod_cquiz_navblock_title';
        $bc->title = html_writer::span(get_string('cquiznavigation', 'cquiz'), '', array('id' => 'mod_cquiz_navblock_title'));
        $bc->content = $output->navigation_panel($panel);
        return $bc;
    }

    /**
     * Return an array of variant URLs to other attempts at this cquiz.
     *
     * The $url passed in must contain an attempt parameter.
     *
     * The {@link mod_cquiz_links_to_other_attempts} object returned contains an
     ...
     * @return mod_cquiz_links_to_other_attempts containing array int => null|moodle_url.
     */
    public function links_to_other_attempts(moodle_url $url) {
        $attempts = cquiz_get_user_attempts($this->get_cquiz()->id, $this->attempt->userid, 'all');
        if (count($attempts) <= 1) {
            return false;
        }

        $links = new mod_cquiz_links_to_other_attempts();
     ...
     * @return mod_cquiz_links_to_other_attempts|null containing array int => null|moodle_url,
    ...
    public function links_to_other_redos($slot, moodle_url $baseurl) {
        ...
        foreach ($qas as $qa) {
            if ($qa->get_slot() == $slot) {
                ...
                $links->links[$index] = new action_link($url, $index, new popup_action('click', $url, 'reviewquestion', array('width' => 450, 'height' => 650)), array('title' => get_string('reviewresponse', 'question')));
            }
            $index++;
        }
        return $links;
    ...
    public function handle_if_time_expired($timestamp, $studentisonline) {
        ...
        if ($this->attempt->state == self::OVERDUE) {
            ...
            $graceperiod = $this->cquizobj->get_cquiz()->graceperiod;
        ...
        // Otherwise, we were in cquiz_attempt::IN_PROGRESS, and time has now expired.
        // Transition to the appropriate state.
        switch ($this->cquizobj->get_cquiz()->overduehandling) {
            case 'autosubmit':
    ...
    public function process_submitted_actions($timestamp, $becomingoverdue = false, $simulatedresponses = null) {
        ...
        if (!$this->is_preview() && $this->attempt->state == self::FINISHED) {
            cquiz_save_best_grade($this->get_cquiz(), $this->get_userid());
        }
    ...
    public function process_redo_question($slot, $timestamp) {
        ...
        $qubaids = new \mod_cquiz\question\qubaids_for_users_attempts(
                $this->get_cquizid(), $this->get_userid());
    ...
    public function process_finish($timestamp, $processsubmitted) {
        ...
        if (!$this->is_preview()) {
            ...
            // Trigger event.
            $this->fire_state_transition_event('\mod_cquiz\event\attempt_submitted', $timestamp);
    ...
    public function update_timecheckstate($time) {
        ...
        if ($this->attempt->timecheckstate !== $time) {
            ...
            $DB->set_field('cquiz_attempts', 'timecheckstate', $time, array('id' => $this->attempt->id));
        }
    }
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_cquiz\event\attempt_becameoverdue', $timestamp);

        $transaction->allow_commit();

        cquiz_send_overdue_message($this);
    }
    ...
    public function process_abandon($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_cquiz\event\attempt_abandoned', $timestamp);
    ...
    protected function fire_state_transition_event($eventclass, $timestamp) {
        global $USER;
        $cquizrecord = $this->get_cquiz();
        $params = array(
            'context' => $this->get_cquizobj()->get_context(),
            'courseid' => $this->get_courseid(),
            'objectid' => $this->attempt->id,
            'relateduserid' => $this->attempt->userid,
            'other' => array(
                'submitterid' => CLI_SCRIPT ? null : $USER->id,
                'cquizid' => $cquizrecord->id
            )
        );

        $event = $eventclass::create($params);
        $event->add_record_snapshot('cquiz', $this->get_cquiz());
        $event->add_record_snapshot('cquiz_attempts', $this->get_attempt());
        $event->trigger();
    }
    ...
    /**
     * Get a URL for a particular question on a particular page of the cquiz.
     * Used by {@link attempt_url()} and {@link review_url()}.
     *
     * @param string $script. Used in the URL like /mod/cquiz/$script.php
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/cquiz/' . $script . '.php' . $fragment, array('attempt' => $this->attempt->id));
    ...
    /**
     * Process responses during an attempt at a cquiz.
     *
    ...
    public function process_attempt($timenow, $finishattempt, $timeup, $thispage) {
        ...
        // to show the student another page of the cquiz. Just finish now.
        $graceperiodmin = null;
        ...
        $toolate = false;
        if ($timeclose !== false && $timenow > $timeclose - CQUIZ_MIN_TIME_TO_CONTINUE) {
            $timeup = true;
        ...
        if ($timeup) {
            if ($this->get_cquiz()->overduehandling == 'graceperiod') {
                if (is_null($graceperiodmin)) {
                    $graceperiodmin = get_config('cquiz', 'graceperiodmin');
                }
                if ($timenow > $timeclose + $this->get_cquiz()->graceperiod + $graceperiodmin) {
        ...
        if (!$finishattempt) {
            ...
            if (!$toolate) {
                try {
                    ...
                    throw new moodle_exception('errorprocessingresponses', 'question', $this->attempt_url(null, $thispage), $e->getMessage(), $debuginfo);
                }
        ...
        // Update the cquiz attempt record.
        try {
    ...
    public function check_page_access($page) {
        ...
        if ($this->get_currentpage() != $page) {
            if ($this->get_navigation_method() == CQUIZ_NAVMETHOD_SEQ && $this->get_currentpage() > $page) {
                return false;
            }
        }
        return true;
    ...
    public function set_currentpage($page) {
        ...
        if ($this->check_page_access($page)) {
            $DB->set_field('cquiz_attempts', 'currentpage', $page, array('id' => $this->get_attemptid()));
            return true;
        }
        return false;
    }
 ...
 * @since      Moodle 2.9
 */
class cquiz_nav_section_heading implements renderable {
 ...
 * @since      Moodle 2.1
 */
class cquiz_nav_question_button implements renderable {
 ...
 * @since      Moodle 2.0
 */
abstract class cquiz_nav_panel_base {
    ...
    public function __construct(cquiz_attempt $attemptobj, question_display_options $options, $page, $showall) {
    ...
    /**
     * Get the buttons and section headings to go in the cquiz navigation block.
    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            if ($heading = $this->attemptobj->get_heading_before_slot($slot)) {
                $buttons[] = new cquiz_nav_section_heading(format_string($heading));
            }

            $qa = $this->attemptobj->get_question_attempt($slot);
            $showcorrectness = $this->options->correctness && $qa->has_marks();

            $button = new cquiz_nav_question_button();
            $button->id = 'cquiznavbutton' . $slot;
            ...
            if ($this->attemptobj->is_blocked_by_previous_question($slot)) {
                ...
                $button->statestring = get_string('questiondependsonprevious', 'cquiz');
            }
            $buttons[] = $button;
        }
    ...
    protected function get_state_string(question_attempt $qa, $showcorrectness) {
        ...
        if ($qa->get_state() == question_state::$todo) {
            return get_string('notyetviewed', 'cquiz');
        } else {
            return get_string('viewed', 'cquiz');
        }
    }

    public function render_before_button_bits(mod_cquiz_renderer $output) {
        return '';
    }

    abstract public function render_end_bits(mod_cquiz_renderer $output);
    ...
    protected function render_restart_preview_link($output) {
        ...
        return $output->restart_preview_button(new moodle_url(
                        $this->attemptobj->start_attempt_url(null, 0, true), array('forcenew' => true)));
    }
    ...
    public function user_picture() {
        global $DB;
        if ($this->attemptobj->get_cquiz()->showuserpicture == CQUIZ_SHOWIMAGE_NONE) {
            return null;
        }
        $user = $DB->get_record('user', array('id' => $this->attemptobj->get_userid()));
        $userpicture = new user_picture($user);
        $userpicture->courseid = $this->attemptobj->get_courseid();
        if ($this->attemptobj->get_cquiz()->showuserpicture == CQUIZ_SHOWIMAGE_LARGE) {
    ...
    public function get_button_container_class() {
        // Cquiz navigation is set on 'Show all questions on one page'.
        if ($this->showall) {
            return 'allquestionsononepage';
        }
        // Cquiz navigation is set on 'Show one page at a time'.
        return 'multipages';
    }
...
/**
 * Specialisation of {@link cquiz_nav_panel_base} for the attempt cquiz page.
 *
 * @copyright  2017 Viddia (http://viddia.com.br)
 * @author     Ricardo Drummond
 * @since      Moodle 2.0
 */
class cquiz_attempt_nav_panel extends cquiz_nav_panel_base {
    ...
    public function render_before_button_bits(mod_cquiz_renderer $output) {
        return html_writer::tag('div', get_string('navnojswarning', 'cquiz'), array('id' => 'cquiznojswarning'));
    }

    public function render_end_bits(mod_cquiz_renderer $output) {
        return html_writer::link($this->attemptobj->summary_url(), get_string('endtest', 'cquiz'), array('class' => 'endtestlink')) .
...
/**
 * Specialisation of {@link cquiz_nav_panel_base} for the review cquiz page.
 *
 * @copyright  2017 Viddia (http://viddia.com.br)
 * @author     Ricardo Drummond
 * @since      Moodle 2.0
 */
class cquiz_review_nav_panel extends cquiz_nav_panel_base {
    ...
    public function render_end_bits(mod_cquiz_renderer $output) {
        ...
        if ($this->attemptobj->get_num_pages() > 1) {
            if ($this->showall) {
                $html .= html_writer::link($this->attemptobj->review_url(null, 0, false), get_string('showeachpage', 'cquiz'));
            } else {
                $html .= html_writer::link($this->attemptobj->review_url(null, 0, true), get_string('showall', 'cquiz'));
            }
        }
