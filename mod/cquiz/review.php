...
/**
 * This page prints a review of a particular cquiz attempt
 *
...
$url = new moodle_url('/mod/cquiz/review.php', array('attempt' => $attemptid));
if ($page !== 0) {
    $url->param('page', $page);
} else if ($showall) {
...
if ($attemptobj->is_own_attempt()) {
    if (!$attemptobj->is_finished()) {
        ...
        $accessmanager->back_to_view_page($PAGE->get_renderer('mod_cquiz'), $attemptobj->cannot_review_message());
    }
...
if ($attemptobj->is_own_preview()) {
    $strreviewtitle = get_string('reviewofpreview', 'cquiz');
    navigation_node::override_active_url($attemptobj->start_attempt_url());
} else {
    $strreviewtitle = get_string('reviewofattempt', 'cquiz', $attemptobj->get_attempt_number());
    if (empty($attemptobj->get_cquiz()->showblocks) && !$attemptobj->is_preview_user()) {
...
if ($attempt->state == cquiz_attempt::FINISHED) {
    if ($timetaken = ($attempt->timefinish - $attempt->timestart)) {
        if ($cquiz->timelimit && $timetaken > ($cquiz->timelimit + 60)) {
            $overtime = $timetaken - $cquiz->timelimit;
    ...
    $timetaken = get_string('unfinished', 'cquiz');
}

// Prepare summary informat about the whole attempt.
$summarydata = array();
if (!$attemptobj->get_cquiz()->showuserpicture && $attemptobj->get_userid() != $USER->id) {
...
$summarydata['startedon'] = array(
    'title' => get_string('startedon', 'cquiz'),
    'content' => date(get_string('datetimeformat', 'mod_cquiz'), $attempt->timestart),
);

$summarydata['state'] = array(
    'title' => get_string('attemptstate', 'cquiz'),
    'content' => cquiz_attempt::state_name($attempt->state),
);

if ($attempt->state == cquiz_attempt::FINISHED) {
    $summarydata['completedon'] = array(
        'title' => get_string('completedon', 'cquiz'),
        'content' => date(get_string('datetimeformat', 'mod_cquiz'), $attempt->timefinish),
    );
    $summarydata['timetaken'] = array(
        'title' => get_string('timetaken', 'cquiz'),
        'content' => $timetaken,
    );
}
...
if (!empty($overtime)) {
    $summarydata['overdue'] = array(
        'title' => get_string('overdue', 'cquiz'),
        'content' => $overtime,
    );
}

// Show marks (if the user is allowed to see marks at the moment).
$grade = cquiz_rescale_grade($attempt->sumgrades, $cquiz, false);
if ($options->marks >= question_display_options::MARK_AND_MAX && cquiz_has_grades($cquiz)) {

    if ($attempt->state != cquiz_attempt::FINISHED) {
        // Cannot display grade.
    } else if (is_null($grade)) {
        $summarydata['grade'] = array(
            'title' => get_string('grade', 'cquiz'),
            'content' => cquiz_format_grade($cquiz, $grade),
        );
    } else {
        // Show raw marks only if they are different from the grade (like on the view page).
        if ($cquiz->grade != $cquiz->sumgrades) {
            $a = new stdClass();
            $a->grade = cquiz_format_grade($cquiz, $attempt->sumgrades);
            $a->maxgrade = cquiz_format_grade($cquiz, $cquiz->sumgrades);
            $summarydata['marks'] = array(
                'title' => get_string('marks', 'cquiz'),
                'content' => get_string('outofshort', 'cquiz', $a),
            );
        }

        // Now the scaled grade.
        $a = new stdClass();
        $a->grade = html_writer::tag('b', cquiz_format_grade($cquiz, $grade));
        $a->maxgrade = cquiz_format_grade($cquiz, $cquiz->grade);
        if ($cquiz->grade != 100) {
            $a->percent = html_writer::tag('b', format_float(
                                    $attempt->sumgrades * 100 / $cquiz->sumgrades, 0));
            $formattedgrade = get_string('outofpercent', 'cquiz', $a);
        } else {
            $formattedgrade = get_string('outof', 'cquiz', $a);
        }
        $summarydata['grade'] = array(
            'title' => get_string('grade', 'cquiz'),
