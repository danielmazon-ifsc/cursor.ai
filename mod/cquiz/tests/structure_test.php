 ...
 * @author     Ricardo Drummond
 */
class mod_cquiz_structure_testcase extends advanced_testcase {

    /**
     * Create a course with an empty cquiz.
     * @return array with three elements cquiz, cm and course.
     */
    protected function prepare_cquiz_data() {
     ...
     * @return cquiz the created cquiz.
     */
    protected function create_test_cquiz($layout) {
        list($cquiz, $cm, $course) = $this->prepare_cquiz_data();
        ...
        foreach ($layout as $item) {
            if (is_string($item)) {
                ...
                $q = $questiongenerator->create_question($qtype, null, array('name' => $name, 'category' => $cat->id));

                cquiz_add_cquiz_question($q->id, $cquiz, $page);
                $lastpage = $page;
            }
        }

        $cquizobj = new cquiz($cquiz, $cm, $course);
        $structure = \mod_cquiz\structure::create_for_cquiz($cquizobj);
        if (isset($headings[1])) {
     ...
     * @param array $expectedlayout as for $layout in {@link create_test_cquiz()}.
     * @param \mod_cquiz\structure $structure the structure to test.
     */
    protected function assert_cquiz_layout($expectedlayout, \mod_cquiz\structure $structure) {
        $sections = $structure->get_sections();
        ...
        foreach ($expectedlayout as $item) {
            if (is_string($item)) {
                ...
                if ($slot > 1 && $section->heading == '' && $section->firstslot == 1) {
                    // The array $expectedlayout did not contain default first cquiz section, so skip over it.
                    $section = array_shift($sections);
                }
        ...
        if ($slot - 1 != count($structure->get_slots())) {
            $this->fail('The cquiz contains more slots than expected.');
        }

        if (!empty($sections)) {
            $section = array_shift($sections);
            if ($section->heading != '' || $section->firstslot != 1) {
                $this->fail('Unexpected section (' . $section->heading . ') found in the cquiz.');
            }
        }
    }
     ...
     * used by create_test_cquiz as assert_cquiz_layout.
    ...
    public function test_get_cquiz_slots() {
        $cquizobj = $this->create_test_cquiz(array(
            array('TF1', 1, 'truefalse'),
    ...
    public function test_cquiz_has_one_section_by_default() {
        $cquizobj = $this->create_test_cquiz(array(
            array('TF1', 1, 'truefalse'),
        ));
        $structure = \mod_cquiz\structure::create_for_cquiz($cquizobj);
    ...
    public function test_remove_section_heading() {
        ...
        $structure = \mod_cquiz\structure::create_for_cquiz($cquizobj);
        $this->assert_cquiz_layout(array(
            'Heading 1',
            array('TF1', 1, 'truefalse'),
            array('TF2', 2, 'truefalse'),
                ), $structure);
    ...
    public function test_cquiz_remove_slot() {
        $cquizobj = $this->create_test_cquiz(array(
            array('TF1', 1, 'truefalse'),
            array('TF2', 1, 'truefalse'),
            'Heading 2',
    ...
    public function test_cquiz_removing_a_random_question_deletes_the_question() {
        global $DB;
        ...
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category();
        cquiz_add_random_questions($cquizobj->get_cquiz(), 1, $cat->id, 1, false);
        $structure = \mod_cquiz\structure::create_for_cquiz($cquizobj);
    ...
    public function test_can_remove_last_question_in_a_cquiz() {
        $cquizobj = $this->create_test_cquiz(array(
            'Heading 1',
            array('TF1', 1, 'truefalse'),
        ));
        ...
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category();
        $q = $questiongenerator->create_question('truefalse', null, array('name' => 'TF2', 'category' => $cat->id));

        cquiz_add_cquiz_question($q->id, $cquizobj->get_cquiz(), 0);
        $structure = \mod_cquiz\structure::create_for_cquiz($cquizobj);
    ...
    public function test_add_question_updates_headings() {
        ...
        cquiz_add_cquiz_question($q->id, $cquizobj->get_cquiz(), 1);
    ...
    public function test_remove_page_break() {
        ...
        $slots = $structure->update_page_break($slotid, \mod_cquiz\repaginate::LINK);
    ...
    public function test_add_page_break() {
        ...
        $slotid = $structure->get_question_in_slot(2)->slotid;
        $slots = $structure->update_page_break($slotid, \mod_cquiz\repaginate::UNLINK);
