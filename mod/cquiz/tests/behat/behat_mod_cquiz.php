...
/**
 * Steps definitions related to mod_cquiz.
 *
 * @package   mod_cquiz
 * @category  test
 ...
 * @author     Ricardo Drummond
 */
class behat_mod_cquiz extends behat_question_base {

    /**
     * Put the specified questions on the specified pages of a given cquiz.
     *
     ...
     *
     * @param string $cquizname the name of the cquiz to add questions to.
     * @param TableNode $data information about the questions to add.
     *
     * @Given /^cquiz "([^"]*)" contains the following questions:$/
     */
    public function cquiz_contains_the_following_questions($cquizname, TableNode $data) {
        global $DB;

        $cquiz = $DB->get_record('cquiz', array('name' => $cquizname), '*', MUST_EXIST);
        ...
        if (!in_array('question', $firstrow) && !in_array('page', $firstrow)) {
            if (count($firstrow) == 2) {
                ...
                throw new ExpectationException('When adding questions to a cquiz, you should give 2 or three 3 things: ' .
        ...
        foreach ($data->getHash() as $questiondata) {
            if (!array_key_exists('question', $questiondata)) {
                throw new ExpectationException('When adding questions to a cquiz, ' .
            ...
            if ($page <= 0 || (string) $page !== $questiondata['page']) {
                ...
                $questiondata['question'] . '" must be a positive integer.', $this->getSession());
            }
            if ($page < $lastpage || $page > $lastpage + 1) {
                throw new ExpectationException('When adding questions to a cquiz, ' .
                'the page number for each question must either be the same, ' .
                'or one more, then the page number for the previous question.', $this->getSession());
            }
            $lastpage = $page;
            ...
            if (!array_key_exists('maxmark', $questiondata) || $questiondata['maxmark'] === '') {
                ...
                if (!is_numeric($questiondata['maxmark']) || $maxmark < 0) {
                    ...
                    $questiondata['question'] . '" must be a positive number.', $this->getSession());
                }
            }

            // Add the question.
            cquiz_add_cquiz_question($questionid, $cquiz, $page, $maxmark);

            // Require previous.
            if (array_key_exists('requireprevious', $questiondata)) {
                if ($questiondata['requireprevious'] === '1') {
                    $slot = $DB->get_field('cquiz_slots', 'MAX(slot)', array('cquizid' => $cquiz->id));
                    $DB->set_field('cquiz_slots', 'requireprevious', 1, array('cquizid' => $cquiz->id, 'slot' => $slot));
                    ...
                    $questiondata['question'] . '" should be 0, 1 or blank.', $this->getSession());
                }
            }
        }
    ...
    /**
     * Put the specified section headings to start at specified pages of a given cquiz.
     *
     * The first row should be column names:
     ...
     *
     * @param string $cquizname the name of the cquiz to add sections to.
     * @param TableNode $data information about the sections to add.
     *
     * @Given /^cquiz "([^"]*)" contains the following sections:$/
     */
    public function cquiz_contains_the_following_sections($cquizname, TableNode $data) {
        global $DB;
        ...
        foreach ($data->getHash() as $rownumber => $sectiondata) {
            if (!array_key_exists('heading', $sectiondata)) {
                throw new ExpectationException('When adding sections to a cquiz, ' .
            ...
            if ($rownumber == 0) {
                $section = $DB->get_record('cquiz_sections', array('cquizid' => $cquiz->id), '*', MUST_EXIST);
            } else {
                $section = new stdClass();
                $section->cquizid = $cquiz->id;
            }
            ...
            if ($section->firstslot <= $previousfirstslot ||
                ...
                $sectiondata['heading'] . '" must an integer greater than the previous section firstslot.', $this->getSession());
            }
            if ($rownumber == 0 && $section->firstslot != 1) {
                throw new ExpectationException('The first section must have firstslot set to 1.', $this->getSession());
            }
            ...
            if ((string) $section->shufflequestions !== $sectiondata['shuffle']) {
                ...
                $sectiondata['heading'] . '" must be 0 or 1.', $this->getSession());
            }

            if ($rownumber == 0) {
                $DB->update_record('cquiz_sections', $section);
            } else {
                $DB->insert_record('cquiz_sections', $section);
            }
        }

        if ($section->firstslot > $DB->count_records('cquiz_slots', array('cquizid' => $cquiz->id))) {
            throw new ExpectationException('The section firstslot must be less than the total number of slots in the cquiz.', $this->getSession());
        }
    }

    /**
     * Adds a question to the existing cquiz with filling the form.
     *
     * The form for creating a question should be on one page.
     *
     * @When /^I add a "(?P<question_type_string>(?:[^"]|\\")*)" question to the "(?P<cquiz_name_string>(?:[^"]|\\")*)" cquiz with:$/
     * @param string $questiontype
     * @param string $cquizname
     * @param TableNode $questiondata with data for filling the add question form
     */
    public function i_add_question_to_the_cquiz_with($questiontype, $cquizname, TableNode $questiondata) {
        $cquizname = $this->escape($cquizname);
        $editcquiz = $this->escape(get_string('editcquiz', 'cquiz'));
        $cquizadmin = $this->escape(get_string('pluginadministration', 'cquiz'));
        $addaquestion = $this->escape(get_string('addaquestion', 'cquiz'));
        ...
        $this->execute('behat_general::click_link', $cquizname);

        $this->execute("behat_navigation::i_navigate_to_node_in", array($editcquiz, $cquizadmin));
    ...
    /**
     * Set the max mark for a question on the Edit competency quiz page.
     *
     * @When /^I set the max mark in cquiz for question "(?P<question_name_string>(?:[^"]|\\")*)" to "(?P<new_mark_string>(?:[^"]|\\")*)"$/
     * @param string $questionname the name of the question to set the max mark for.
     * @param string $newmark the mark to set
     */
    public function i_set_the_max_mark_for_cquiz_question($questionname, $newmark) {
        $this->execute('behat_general::click_link', $this->escape(get_string('editmaxmark', 'cquiz')));
    ...
    /**
     * Open the add menu on a given page, or at the end of the Edit competency quiz page.
     * @Given /^I open the "(?P<page_n_or_last_string>(?:[^"]|\\")*)" add to cquiz menu$/
     * @param string $pageorlast either "Page n" or "last".
     */
    public function i_open_the_add_to_cquiz_menu_for($pageorlast) {
        ...
        if ($pageorlast == 'last') {
            ...
            throw new ExpectationException("The I open the add to cquiz menu step must specify either 'Page N' or 'last'.");
        }
     ...
     * @Given /^I follow "(?P<link_string>(?:[^"]|\\")*)" in the cquiz open menu$/
    ...
    public function i_follow_in_the_open_menu($linkstring) {
        ...
        $this->execute('behat_general::i_click_on_in_the', array($linkstring, "link", $openmenuxpath, "xpath_element")
        );
    }

    /**
     * Check whether a particular question is on a particular page of the cquiz on the Edit competency quiz page.
     * @Given /^I should see "(?P<question_name>(?:[^"]|\\")*)" on cquiz page "(?P<page_number>\d+)"$/
     ...
     */
    public function i_should_see_on_cquiz_page($questionname, $pagenumber) {
    ...
    /**
     * Check whether a particular question is not on a particular page of the cquiz on the Edit competency quiz page.
     * @Given /^I should not see "(?P<question_name>(?:[^"]|\\")*)" on cquiz page "(?P<page_number>\d+)"$/
     * @param string $questionname the name of the question we are looking for.
     * @param number $pagenumber the page it should be found on.
     */
    public function i_should_not_see_on_cquiz_page($questionname, $pagenumber) {
        $xpath = "//li[contains(., '" . $this->escape($questionname) .
                "')][./preceding-sibling::li[contains(@class, 'pagenumber')][1][contains(., 'Page " .
                $pagenumber . "')]]";
    ...
    /**
     * Check whether one question comes before another on the Edit competency quiz page.
     * The two questions must be on the same page.
     * @Given /^I should see "(?P<first_q_name>(?:[^"]|\\")*)" before "(?P<second_q_name>(?:[^"]|\\")*)" on the edit cquiz page$/
     ...
     */
    public function i_should_see_before_on_the_edit_cquiz_page($firstquestionname, $secondquestionname) {
    ...
    /**
     * Check the number displayed alongside a question on the Edit competency quiz page.
     * @Given /^"(?P<question_name>(?:[^"]|\\")*)" should have number "(?P<number>(?:[^"]|\\")*)" on the edit cquiz page$/
     * @param string $questionname the name of the question we are looking for.
     * @param number $number the number (or 'i') that should be displayed beside that question.
     */
    public function should_have_number_on_the_edit_cquiz_page($questionname, $number) {
     ...
     * @When /^I click on the "(Add|Remove)" page break icon after cquiz question "(?P<question_name>(?:[^"]|\\")*)"$/
     * @param string $addorremoves 'Add' or 'Remove'.
     ...
     * @When /^the "(Add|Remove)" page break icon after cquiz question "(?P<question_name>(?:[^"]|\\")*)" should exist$/
     * @param string $addorremoves 'Add' or 'Remove'.
     * @param string $questionname the name of the question before the icon to click.
     ...
     * @When /^the "(Add|Remove)" page break icon after cquiz question "(?P<question_name>(?:[^"]|\\")*)" should not exist$/
     * @param string $addorremoves 'Add' or 'Remove'.
     * @param string $questionname the name of the question before the icon to click.
     * @return array of steps.
     */
     ...
     * @When /^the "(Add|Remove)" page break link after cquiz question "(?P<question_name>(?:[^"]|\\")*) should contain:"$/
     * @param string $addorremoves 'Add' or 'Remove'.
     * @param string $questionname the name of the question before the icon to click.
     ...
     *
     * @Given /^I click on shuffle for section "([^"]*)" on the cquiz edit page$/
     */
     ...
     *
     * @Given /^shuffle for section "([^"]*)" should be "(On|Off)" on the cquiz edit page$/
     */
    public function shuffle_for_section_should_be($heading, $value) {
        ...
        if ($value == 'On' && !$checkbox->isChecked()) {
            $msg = "Shuffle for section '$heading' is not checked, but you are expecting it to be checked ($value). " .
                    "Check the line with: \nshuffle for section \"$heading\" should be \"$value\" on the cquiz edit page" .
    ...
    /**
     * Move a question on the Edit competency quiz page by first clicking on the Move icon,
     * then clicking one of the "After ..." links.
     * @When /^I move "(?P<question_name>(?:[^"]|\\")*)" to "(?P<target>(?:[^"]|\\")*)" in the cquiz by clicking the move icon$/
     * @param string $questionname the name of the question we are looking for.
    ...
    /**
     * Move a question on the Edit competency quiz page by dragging a given question on top of another item.
     * @When /^I move "(?P<question_name>(?:[^"]|\\")*)" to "(?P<target>(?:[^"]|\\")*)" in the cquiz by dragging$/
     * @param string $questionname the name of the question we are looking for.
    ...
    public function i_move_question_after_item_by_dragging($questionname, $target) {
        ...
        $this->execute('behat_general::i_drag_and_i_drop_it_in', array($iconxpath, 'xpath_element', $destinationxpath, 'xpath_element')
        );
    }

    /**
     * Delete a question on the Edit competency quiz page by first clicking on the Delete icon,
     * then clicking one of the "After ..." links.
     * @When /^I delete "(?P<question_name>(?:[^"]|\\")*)" in the cquiz by clicking the delete icon$/
     * @param string $questionname the name of the question we are looking for.
     * @return array of steps.
     */
    public function i_delete_question_by_clicking_the_delete_icon($questionname) {
        ...
        $this->execute('behat_general::i_click_on_in_the', array('Yes', "button", "Confirm", "dialogue")
        );
    }

    /**
     * Set the section heading for a given section on the Edit competency quiz page
     *
     * @When /^I change cquiz section heading "(?P<section_name_string>(?:[^"]|\\")*)" to "(?P<new_section_heading_string>(?:[^"]|\\")*)"$/
     ...
     * cquiz navigation block.
     *
     * @Then /^I should see question "(?P<questionnumber>\d+)" in section "(?P<section_heading_string>(?:[^"]|\\")*)" in the cquiz navigation$/
     ...
     */
    public function i_should_see_question_in_section_in_the_cquiz_navigation($questionnumber, $sectionheading) {
        ...
        $exception = new ExpectationException('Question "' . $questionnumber . '" is not in section "' .
                $sectionheading . '" in the cquiz navigation.', $this->getSession());
        $xpath = "//div[@id = 'mod_cquiz_navblock']//*[contains(concat(' ', normalize-space(@class), ' '), ' qnbutton ') and " .
