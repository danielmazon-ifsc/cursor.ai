 ...
 * @author     Ricardo Drummond
 */
class mod_cquiz_attempt_walkthrough_from_csv_testcase extends advanced_testcase {

    protected $files = array('questions', 'steps', 'results');

    /**
     * @var stdClass the cquiz record we create.
     */
    protected $cquiz;
     ...
     * directory.
     *
     * @param array $cquizsettings of settings read from csv file cquizzes.csv
    ...
    public function test_walkthrough_from_csv($cquizsettings, $csvdata) {
        ...
        // https://github.com/jamiepratt/moodle-cquiz-tools/tree/master/responsegenerator

        $this->create_cquiz_simulate_attempts_and_check_results($cquizsettings, $csvdata);
    }

    public function create_cquiz($cquizsettings, $qs) {
        global $SITE, $DB;
        $this->setAdminUser();
        ...
        $aggregratedsettings = $cquizsettings + array('course' => $SITE->id,
            'questionsperpage' => 0,
            'grade' => 100.0,
            'sumgrades' => $sumofgrades);

        $this->cquiz = $cquizgenerator->create_instance($aggregratedsettings);

        $this->randqids = array();
        foreach ($slots as $slotno => $slotquestion) {
            if ($slotquestion['type'] !== 'random') {
                cquiz_add_cquiz_question($slotquestion['id'], $this->cquiz, 0, $slotquestion['mark']);
            } else {
                cquiz_add_random_questions($this->cquiz, 0, $slotquestion['catid'], 1, 0);
    ...
    /**
     * Create cquiz, simulate attempts and check results (if resultsXX.csv exists).
     *
     * @param array $cquizsettings Cquiz overrides for this cquiz.
     * @param PHPUnit_Extensions_Database_DataSet_ITable[] $csvdata Data loaded from csv files for this test.
     */
    protected function create_cquiz_simulate_attempts_and_check_results($cquizsettings, $csvdata) {
    ...
    public function get_data_for_walkthrough() {
        $cquizzes = $this->load_csv_data_file('cquizzes');
        $datasets = array();
        for ($rowno = 0; $rowno < $cquizzes->getRowCount(); $rowno++) {
            $cquizsettings = $cquizzes->getRow($rowno);
            $dataset = array();
            foreach ($this->files as $file) {
                if (file_exists($this->get_full_path_of_csv_file($file, $cquizsettings['testnumber']))) {
                    $dataset[$file] = $this->load_csv_data_file($file, $cquizsettings['testnumber']);
                }
            }
            $datasets[] = array($cquizsettings, $dataset);
        }
        return $datasets;
    }
     ...
     * @return array attempt no as in csv file => the id of the cquiz_attempt as stored in the db.
     */
    protected function walkthrough_attempts($steps) {
        ...
        for ($rowno = 0; $rowno < $steps->getRowCount(); $rowno++) {
            ...
            // Find existing user or make a new user to do the cquiz.
            ...
            if (!isset($attemptids[$step['cquizattempt']])) {
                // Start the attempt.
                $cquizobj = cquiz::create($this->cquiz->id, $user->id);
                ...
                $prevattempts = cquiz_get_user_attempts($this->cquiz->id, $user->id, 'all', true);
                $attemptnumber = count($prevattempts) + 1;
                $timenow = time();
                $attempt = cquiz_create_attempt($cquizobj, $attemptnumber, false, $timenow, false, $user->id);
                ...
                cquiz_start_new_attempt($cquizobj, $quba, $attempt, $attemptnumber, $timenow, $step['randqs'], $step['variants']);
                cquiz_attempt_save_started($cquizobj, $quba, $attempt);
                $attemptid = $attemptids[$step['cquizattempt']] = $attempt->id;
            } else {
                $attemptid = $attemptids[$step['cquizattempt']];
            }
     ...
     * @param $attemptids array attempt no as in csv file => the id of the cquiz_attempt as stored in the db.
     */
    protected function check_attempts_results($results, $attemptids) {
        for ($rowno = 0; $rowno < $results->getRowCount(); $rowno++) {
            ...
            $attemptobj = cquiz_attempt::create($attemptids[$result['cquizattempt']]);
     ...
     * @param cquiz_attempt $attemptobj         the attempt object loaded from db.
        ...
        foreach ($result as $fieldname => $value) {
            ...
            switch ($fieldname) {
                    ...
                    foreach ($value as $slotno => $slottests) {
                        foreach ($slottests as $slotfieldname => $slotvalue) {
                            switch ($slotfieldname) {
                                case 'mark' :
                                    $this->assertEquals(round($slotvalue, 2), $attemptobj->get_question_mark($slotno), "Mark for slot $slotno of attempt {$result['cquizattempt']}.");
                                    break;
                                default :
                ...
                case 'finished' :
                    ...
                    break;
                case 'summarks' :
                    $this->assertEquals($value, $attemptobj->get_sum_marks(), "Sum of marks of attempt {$result['cquizattempt']}.");
                    break;
                case 'cquizgrade' :
                    // Check cquiz grades.
                    $grades = cquiz_get_user_grades($attemptobj->get_cquiz(), $attemptobj->get_userid());
                    $grade = array_shift($grades);
                    $this->assertEquals($value, $grade->rawgrade, "Cquiz grade for attempt {$result['cquizattempt']}.");
                    break;
                case 'gradebookgrade' :
                    // Check grade book.
                    $gradebookgrades = grade_get_grades($attemptobj->get_courseid(), 'mod', 'cquiz', $attemptobj->get_cquizid(), $attemptobj->get_userid());
                    $gradebookitem = array_shift($gradebookgrades->items);
                    $gradebookgrade = array_shift($gradebookitem->grades);
                    $this->assertEquals($value, $gradebookgrade->grade, "Gradebook grade for attempt {$result['cquizattempt']}.");
                    break;
                default :
