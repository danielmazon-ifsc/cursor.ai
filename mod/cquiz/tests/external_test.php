...
/**
 * Cquiz module external functions tests.
 *
 * @package    mod_cquiz
 * @category   external
...
/**
 * Silly class to access mod_cquiz_external internal methods.
 *
 * @package mod_cquiz
 * @copyright 2017 Viddia (http://viddia.com.br)
 * @author     Ricardo Drummond
 * @since  Moodle 3.1
 */
class testable_mod_cquiz_external extends mod_cquiz_external {
 ...
 * @since      Moodle 3.1
 */
class mod_cquiz_external_testcase extends externallib_advanced_testcase {
    ...
    public function setUp() {
        ...
        $this->cquiz = $this->getDataGenerator()->create_module('cquiz', array('course' => $this->course->id));
        $this->context = context_module::instance($this->cquiz->cmid);
        $this->cm = get_coursemodule_from_instance('cquiz', $this->cquiz->id);
    ...
    /**
     * Create a cquiz with questions including a started or finished attempt optionally
     *
     * @param  boolean $startattempt whether to start a new attempt
     * @param  boolean $finishattempt whether to finish the new attempt
     * @return array array containing the cquiz, context and the attempt
     */
    private function create_cquiz_with_questions($startattempt = false, $finishattempt = false) {

        // Create a new cquiz with attempts.
        ...
        $cquizobj = cquiz::create($cquiz->id, $this->student->id);
        ...
        if ($startattempt or $finishattempt) {
            ...
            $timenow = time();
            $attempt = cquiz_create_attempt($cquizobj, 1, false, $timenow, false, $this->student->id);
            ...
            return array($cquiz, $context, $cquizobj, $attempt, $attemptobj, $quba);
        } else {
            return array($cquiz, $context, $cquizobj);
        }
    }

    /*
     * Test get cquizzes by courses
     */

    public function test_mod_cquiz_get_cquizzes_by_courses() {
        global $DB;
        ...
        $cquiz2 = self::getDataGenerator()->create_module('cquiz', $record);
        ...
        $returndescription = mod_cquiz_external::get_cquizzes_by_courses_returns();
        ...
        $cquiz1 = $this->cquiz;
        $cquiz1->coursemodule = $cquiz1->cmid;
        $cquiz1->introformat = 1;
        $cquiz1->section = 0;
        $cquiz1->visible = true;
        $cquiz1->groupmode = 0;
        $cquiz1->groupingid = 0;
        $cquiz1->hasquestions = 0;
        $cquiz1->hasfeedback = 0;
        $cquiz1->autosaveperiod = get_config('cquiz', 'autosaveperiod');

        $cquiz2->coursemodule = $cquiz2->cmid;
        $cquiz2->introformat = 1;
        $cquiz2->section = 0;
        $cquiz2->visible = true;
        $cquiz2->groupmode = 0;
        $cquiz2->groupingid = 0;
        $cquiz2->hasquestions = 0;
        $cquiz2->hasfeedback = 0;
        $cquiz2->autosaveperiod = get_config('cquiz', 'autosaveperiod');

        foreach (array_merge($allusersfields, $userswithaccessfields) as $field) {
            $expected1[$field] = $cquiz1->{$field};
            $expected2[$field] = $cquiz2->{$field};
        }

        $expectedcquizzes = array($expected2, $expected1);

        // Call the external function passing course ids.
        $result = mod_cquiz_external::get_cquizzes_by_courses(array($course2->id, $this->course->id));
        $result = external_api::clean_returnvalue($returndescription, $result);

        $this->assertEquals($expectedcquizzes, $result['cquizzes']);
        $this->assertCount(0, $result['warnings']);

        // Call the external function without passing course id.
        $result = mod_cquiz_external::get_cquizzes_by_courses();
        $result = external_api::clean_returnvalue($returndescription, $result);
        $this->assertEquals($expectedcquizzes, $result['cquizzes']);
        $this->assertCount(0, $result['warnings']);

        // Unenrol user from second course and alter expected cquizzes.
        $enrol->unenrol_user($instance2, $this->student->id);
        array_shift($expectedcquizzes);
        ...
        $result = mod_cquiz_external::get_cquizzes_by_courses(array($course2->id));
        ...
        foreach ($managerfields as $field) {
            $expectedcquizzes[0][$field] = $cquiz1->{$field};
        }
        ...
        $result = mod_cquiz_external::get_cquizzes_by_courses(array($this->course->id));
        $result = external_api::clean_returnvalue($returndescription, $result);
        $this->assertEquals($expectedcquizzes, $result['cquizzes']);
        ...
        $cquiz2->timeclose = time() - DAYSECS;
        $DB->update_record('cquiz', $cquiz2);

        $result = mod_cquiz_external::get_cquizzes_by_courses();
        $result = external_api::clean_returnvalue($returndescription, $result);
        $this->assertCount(2, $result['cquizzes']);
        // We only see a limited set of fields.
        $this->assertCount(4, $result['cquizzes'][0]);
        $this->assertEquals($cquiz2->id, $result['cquizzes'][0]['id']);
        $this->assertEquals($cquiz2->coursemodule, $result['cquizzes'][0]['coursemodule']);
        $this->assertEquals($cquiz2->course, $result['cquizzes'][0]['course']);
        $this->assertEquals($cquiz2->name, $result['cquizzes'][0]['name']);
        $this->assertEquals($cquiz2->course, $result['cquizzes'][0]['course']);

        $this->assertFalse(isset($result['cquizzes'][0]['timelimit']));
    }

    /**
     * Test test_view_cquiz
     */
    public function test_view_cquiz() {
        global $DB;

        // Test invalid instance id.
        try {
            mod_cquiz_external::view_cquiz(0);
            $this->fail('Exception expected due to invalid mod_cquiz instance id.');
        ...
        try {
            mod_cquiz_external::view_cquiz($this->cquiz->id);
        ...
        $result = mod_cquiz_external::view_cquiz($this->cquiz->id);
        $result = external_api::clean_returnvalue(mod_cquiz_external::view_cquiz_returns(), $result);
        ...
        $moodlecquiz = new \moodle_url('/mod/cquiz/view.php', array('id' => $this->cm->id));
        $this->assertEquals($moodlecquiz, $event->get_url());
        $this->assertEventContextNotUsed($event);
        $this->assertNotEmpty($event->get_name());

        // Test user with no capabilities.
        // We need a explicit prohibit since this capability is only defined in authenticated user and guest roles.
        assign_capability('mod/cquiz:view', CAP_PROHIBIT, $this->studentrole->id, $this->context->id);
    ...
    public function test_get_user_attempts() {
        ...
        // Create a cquiz with one attempt finished.
        list($cquiz, $context, $cquizobj, $attempt, $attemptobj) = $this->create_cquiz_with_questions(true, true);

        $this->setUser($this->student);
        $result = mod_cquiz_external::get_user_attempts($cquiz->id);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_user_attempts_returns(), $result);

        $this->assertCount(1, $result['attempts']);
        $this->assertEquals($attempt->id, $result['attempts'][0]['id']);
        $this->assertEquals($cquiz->id, $result['attempts'][0]['cquiz']);
        ...
        $result = mod_cquiz_external::get_user_attempts($cquiz->id, 0, 'finished', false);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_user_attempts_returns(), $result);
        ...
        $result = mod_cquiz_external::get_user_attempts($cquiz->id, 0, 'all', false);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_user_attempts_returns(), $result);
        ...
        $result = mod_cquiz_external::get_user_attempts($cquiz->id, 0, 'unfinished', false);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_user_attempts_returns(), $result);
        ...
        $timenow = time();
        $attempt = cquiz_create_attempt($cquizobj, 2, false, $timenow, false, $this->student->id);
        $quba = question_engine::make_questions_usage_by_activity('mod_cquiz', $cquizobj->get_context());
        $quba->set_preferred_behaviour($cquizobj->get_cquiz()->preferredbehaviour);
        ...
        $result = mod_cquiz_external::get_user_attempts($cquiz->id, $this->student->id);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_user_attempts_returns(), $result);

        $this->assertCount(1, $result['attempts']);
        $this->assertEquals($this->student->id, $result['attempts'][0]['userid']);

        $result = mod_cquiz_external::get_user_attempts($cquiz->id, $this->student->id, 'all');
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_user_attempts_returns(), $result);
        ...
        // Invalid parameters.
        try {
            mod_cquiz_external::get_user_attempts($cquiz->id, $this->student->id, 'INVALID_PARAMETER');
            $this->fail('Exception expected due to missing capability.');
    ...
    public function test_get_user_best_grade() {
        ...
        $result = mod_cquiz_external::get_user_best_grade($this->cquiz->id);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_user_best_grade_returns(), $result);
        ...
        $grade = new stdClass();
        $grade->cquiz = $this->cquiz->id;
        $grade->userid = $this->student->id;
        $grade->grade = 8.9;
        $grade->timemodified = time();
        $grade->id = $DB->insert_record('cquiz_grades', $grade);
        ...
        try {
            mod_cquiz_external::get_user_best_grade($this->cquiz->id, $anotherstudent->id);
            $this->fail('Exception expected due to missing capability.');
        ...
        $result = mod_cquiz_external::get_user_best_grade($this->cquiz->id, $this->student->id);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_user_best_grade_returns(), $result);
        ...
        // Invalid user.
        try {
            mod_cquiz_external::get_user_best_grade($this->cquiz->id, -1);
            $this->fail('Exception expected due to missing capability.');
        ...
        $DB->delete_records('cquiz_grades', array('id' => $grade->id));
    }

    /**
     * Test get_combined_review_options.
     * This is a basic test, this is already tested in mod_cquiz_display_options_testcase.
     */
    public function test_get_combined_review_options() {
        ...
        $result = mod_cquiz_external::get_combined_review_options($cquiz->id);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_combined_review_options_returns(), $result);
        ...
        $result = mod_cquiz_external::get_combined_review_options($cquiz->id, $this->student->id);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_combined_review_options_returns(), $result);
        ...
        // Invalid user.
        try {
            mod_cquiz_external::get_combined_review_options($cquiz->id, -1);
            $this->fail('Exception expected due to missing capability.');
        } catch (dml_missing_record_exception $e) {
            $this->assertEquals('invaliduser', $e->errorcode);
        }
    ...
    public function test_start_attempt() {
        ...
        // Create a new cquiz with questions.
        list($cquiz, $context, $cquizobj) = $this->create_cquiz_with_questions();

        $this->setUser($this->student);

        // Try to open attempt in closed cquiz.
        $cquiz->timeopen = time() - WEEKSECS;
        $cquiz->timeclose = time() - DAYSECS;
        $DB->update_record('cquiz', $cquiz);
        $result = mod_cquiz_external::start_attempt($cquiz->id);
        $result = external_api::clean_returnvalue(mod_cquiz_external::start_attempt_returns(), $result);
        ...
        try {
            mod_cquiz_external::start_attempt($cquiz->id, array(array("name" => "cquizpassword", "value" => 'bad')));
            $this->fail('Exception expected due to invalid passwod.');
        } catch (moodle_exception $e) {
            $this->assertEquals(get_string('passworderror', 'cquizaccess_password'), $e->errorcode);
        }

        // Now, try everything correct.
        $result = mod_cquiz_external::start_attempt($cquiz->id, array(array("name" => "cquizpassword", "value" => 'abc')));
        $result = external_api::clean_returnvalue(mod_cquiz_external::start_attempt_returns(), $result);

        $this->assertEquals(1, $result['attempt']['attempt']);
        $this->assertEquals($this->student->id, $result['attempt']['userid']);
        $this->assertEquals($cquiz->id, $result['attempt']['cquiz']);
        $this->assertCount(0, $result['warnings']);
        ...
        try {
            mod_cquiz_external::start_attempt($cquiz->id, array(array("name" => "cquizpassword", "value" => 'abc')));
            $this->fail('Exception expected due to attempt not finished.');
        } catch (moodle_cquiz_exception $e) {
        ...
        // Test user with no capabilities.
        // We need a explicit prohibit since this capability is only defined in authenticated user and guest roles.
        assign_capability('mod/cquiz:attempt', CAP_PROHIBIT, $this->studentrole->id, $context->id);
        // Empty all the caches that may be affected  by this change.
        accesslib_clear_all_caches_for_unit_testing();
        course_modinfo::clear_instance_cache();

        try {
            mod_cquiz_external::start_attempt($cquiz->id);
            $this->fail('Exception expected due to missing capability.');
        } catch (required_capability_exception $e) {
            $this->assertEquals('nopermissions', $e->errorcode);
        }
    ...
    public function test_validate_attempt() {
        ...
        // Create a new cquiz with one attempt started.
        list($cquiz, $context, $cquizobj, $attempt, $attemptobj) = $this->create_cquiz_with_questions(true);
        ...
        try {
            ...
            testable_mod_cquiz_external::validate_attempt($params);
        ...
        $result = testable_mod_cquiz_external::validate_attempt($params);
        ...
        try {
            $params = array('attemptid' => $attempt->id, 'page' => 0,
                'preflightdata' => array(array("name" => "cquizpassword", "value" => 'bad')));
            testable_mod_cquiz_external::validate_attempt($params);
            $this->fail('Exception expected due to invalid passwod.');
        } catch (moodle_exception $e) {
            $this->assertEquals(get_string('passworderror', 'cquizaccess_password'), $e->errorcode);
        ...
        testable_mod_cquiz_external::validate_attempt($params, false);
    ...
    public function test_get_attempt_data() {
        ...
        $cquizobj = $attemptobj->get_cquizobj();
        $cquizobj->preload_questions();
        $cquizobj->load_questions();
        $questions = $cquizobj->get_questions();

        $this->setUser($this->student);

        // We receive one question per page.
        $result = mod_cquiz_external::get_attempt_data($attempt->id, 0);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_attempt_data_returns(), $result);
        ...
        $result = mod_cquiz_external::get_attempt_data($attempt->id, 1);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_attempt_data_returns(), $result);
        ...
        $cquiz->questionsperpage = 4;
        $DB->update_record('cquiz', $cquiz);
        cquiz_repaginate_questions($cquiz->id, $cquiz->questionsperpage);
    ...
    public function test_get_attempt_summary() {
        ...
        $this->setUser($this->student);
        $result = mod_cquiz_external::get_attempt_summary($attempt->id);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_attempt_summary_returns(), $result);
    ...
    public function test_save_attempt() {
        ...
        // Create a new cquiz with one attempt started.
        list($cquiz, $context, $cquizobj, $attempt, $attemptobj, $quba) = $this->create_cquiz_with_questions(true);
        ...
        $result = mod_cquiz_external::save_attempt($attempt->id, $data);
        $result = external_api::clean_returnvalue(mod_cquiz_external::save_attempt_returns(), $result);
        $this->assertTrue($result['status']);
    ...
    public function test_process_attempt() {
        ...
        // Create a new cquiz with two questions and one attempt started.
        list($cquiz, $context, $cquizobj, $attempt, $attemptobj, $quba) = $this->create_cquiz_with_questions(true);
        ...
        $result = mod_cquiz_external::process_attempt($attempt->id, $data);
        $result = external_api::clean_returnvalue(mod_cquiz_external::process_attempt_returns(), $result);
        $this->assertEquals(cquiz_attempt::IN_PROGRESS, $result['state']);
        ...
        // Finish the attempt.
        $result = mod_cquiz_external::process_attempt($attempt->id, array(), true);
        $result = external_api::clean_returnvalue(mod_cquiz_external::process_attempt_returns(), $result);
        $this->assertEquals(cquiz_attempt::FINISHED, $result['state']);
        ...
        $cquiz->timeclose = $timenow - 10;
        $cquiz->graceperiod = 60;
        $cquiz->overduehandling = 'graceperiod';
        $DB->update_record('cquiz', $cquiz);

        $result = mod_cquiz_external::process_attempt($attempt->id, array());
        $result = external_api::clean_returnvalue(mod_cquiz_external::process_attempt_returns(), $result);
        $this->assertEquals(cquiz_attempt::OVERDUE, $result['state']);

        // New attempt.
        $timenow = time();
        $quba = question_engine::make_questions_usage_by_activity('mod_cquiz', $cquizobj->get_context());
        $quba->set_preferred_behaviour($cquizobj->get_cquiz()->preferredbehaviour);
        $attempt = cquiz_create_attempt($cquizobj, 3, 2, $timenow, false, $this->student->id);
        cquiz_start_new_attempt($cquizobj, $quba, $attempt, 3, $timenow);
        cquiz_attempt_save_started($cquizobj, $quba, $attempt);

        // Force abandon.
        $cquiz->timeclose = $timenow - HOURSECS;
        $DB->update_record('cquiz', $cquiz);

        $result = mod_cquiz_external::process_attempt($attempt->id, array());
        $result = external_api::clean_returnvalue(mod_cquiz_external::process_attempt_returns(), $result);
        $this->assertEquals(cquiz_attempt::ABANDONED, $result['state']);
    }
    ...
    public function test_validate_attempt_review() {
        ...
        try {
            ...
            testable_mod_cquiz_external::validate_attempt_review($params);
    ...
    public function test_get_attempt_review() {
        ...
        // Create a new cquiz with two questions and one attempt finished.
        list($cquiz, $context, $cquizobj, $attempt, $attemptobj, $quba) = $this->create_cquiz_with_questions(true, true);

        // Add feedback to the cquiz.
        ...
        $result = mod_cquiz_external::get_attempt_review($attempt->id);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_attempt_review_returns(), $result);
        ...
        // Only first page.
        $result = mod_cquiz_external::get_attempt_review($attempt->id, 0);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_attempt_review_returns(), $result);
    ...
    public function test_view_attempt() {
        ...
        // Create a new cquiz with two questions and one attempt started.
        list($cquiz, $context, $cquizobj, $attempt, $attemptobj, $quba) = $this->create_cquiz_with_questions(true, false);
        ...
        $result = mod_cquiz_external::view_attempt($attempt->id, 0);
        $result = external_api::clean_returnvalue(mod_cquiz_external::view_attempt_returns(), $result);
        $this->assertTrue($result['status']);
        ...
        // Now, force the cquiz with CQUIZ_NAVMETHOD_SEQ (sequencial) navigation method.
        $DB->set_field('cquiz', 'navmethod', CQUIZ_NAVMETHOD_SEQ, array('id' => $cquiz->id));
        // Cquiz requiring preflightdata.
        $DB->set_field('cquiz', 'password', 'abcdef', array('id' => $cquiz->id));
        $preflightdata = array(array("name" => "cquizpassword", "value" => 'abcdef'));

        // See next page.
        $result = mod_cquiz_external::view_attempt($attempt->id, 1, $preflightdata);
        $result = external_api::clean_returnvalue(mod_cquiz_external::view_attempt_returns(), $result);
        $this->assertTrue($result['status']);
        ...
        try {
            mod_cquiz_external::view_attempt($attempt->id, 0);
    ...
    public function test_view_attempt_summary() {
        ...
        $result = mod_cquiz_external::view_attempt_summary($attempt->id);
        $result = external_api::clean_returnvalue(mod_cquiz_external::view_attempt_summary_returns(), $result);
        $this->assertTrue($result['status']);
        ...
        $this->assertEquals($context, $event->get_context());
        $moodlecquiz = new \moodle_url('/mod/cquiz/summary.php', array('attempt' => $attempt->id));
        $this->assertEquals($moodlecquiz, $event->get_url());
        $this->assertEventContextNotUsed($event);
        $this->assertNotEmpty($event->get_name());
        ...
        $result = mod_cquiz_external::view_attempt_summary($attempt->id, $preflightdata);
        $result = external_api::clean_returnvalue(mod_cquiz_external::view_attempt_summary_returns(), $result);
        $this->assertTrue($result['status']);
    }
    ...
    public function test_view_attempt_review() {
        ...
        $result = mod_cquiz_external::view_attempt_review($attempt->id, 0);
        $result = external_api::clean_returnvalue(mod_cquiz_external::view_attempt_review_returns(), $result);
        $this->assertTrue($result['status']);
        ...
        $this->assertEquals($context, $event->get_context());
        $moodlecquiz = new \moodle_url('/mod/cquiz/review.php', array('attempt' => $attempt->id));
        $this->assertEquals($moodlecquiz, $event->get_url());
        $this->assertEventContextNotUsed($event);
        $this->assertNotEmpty($event->get_name());
    }
    ...
    /**
     * Test get_cquiz_feedback_for_grade
     */
    public function test_get_cquiz_feedback_for_grade() {
        global $DB;

        // Add feedback to the cquiz.
        $feedback = new stdClass();
        $feedback->cquizid = $this->cquiz->id;
        $feedback->feedbacktext = 'Feedback text 1';
        $feedback->feedbacktextformat = 1;
        $feedback->mingrade = 49;
        $feedback->maxgrade = 100;
        ...
        $result = mod_cquiz_external::get_cquiz_feedback_for_grade($this->cquiz->id, 50);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_cquiz_feedback_for_grade_returns(), $result);
        $this->assertEquals('Feedback text 1', $result['feedbacktext']);
        $this->assertEquals(FORMAT_HTML, $result['feedbacktextformat']);

        $result = mod_cquiz_external::get_cquiz_feedback_for_grade($this->cquiz->id, 30);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_cquiz_feedback_for_grade_returns(), $result);
        $this->assertEquals('Feedback text 2', $result['feedbacktext']);
        $this->assertEquals(FORMAT_HTML, $result['feedbacktextformat']);

        $result = mod_cquiz_external::get_cquiz_feedback_for_grade($this->cquiz->id, 10);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_cquiz_feedback_for_grade_returns(), $result);
    ...
    /**
     * Test get_cquiz_access_information
     */
    public function test_get_cquiz_access_information() {
        global $DB;
        ...
        $result = mod_cquiz_external::get_cquiz_access_information($cquiz->id);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_cquiz_access_information_returns(), $result);
        ...
        $expected = array(
            ...
            'activerulenames' => ['cquizaccess_openclosedate'],
        ...
        $cquiz->timeopen = time() + DAYSECS;
        $cquiz->timeclose = time() + WEEKSECS;
        $cquiz->password = '123456';
        $DB->update_record('cquiz', $cquiz);
    ...
    public function test_get_attempt_access_information() {
        ...
        // Default restrictions (none).
        $result = mod_cquiz_external::get_attempt_access_information($cquiz->id);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_attempt_access_information_returns(), $result);
        ...
        $result = mod_cquiz_external::get_attempt_access_information($cquiz->id, $attempt->id);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_attempt_access_information_returns(), $result);
        ...
        $this->assertEquals(get_string('nomoreattempts', 'cquiz'), $result['preventnewattemptreasons'][0]);
    }

    /**
     * Test get_cquiz_required_qtypes
     */
    public function test_get_cquiz_required_qtypes() {
        global $DB;
        ...
        $result = mod_cquiz_external::get_cquiz_required_qtypes($cquiz->id);
        $result = external_api::clean_returnvalue(mod_cquiz_external::get_cquiz_required_qtypes_returns(), $result);
