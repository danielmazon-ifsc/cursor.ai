...
/**
 * Classes to enforce the various access rules that can apply to a cquiz.
 *
 * @package   mod_cquiz
 ...
 * cquiz, with convinient methods for seeing whether access is allowed.
 *
 * @copyright 2017 Viddia (http://viddia.com.br)
 * @author     Ricardo Drummond
 * @since     Moodle 2.2
 */
class cquiz_access_manager {

    /** @var cquiz the cquiz settings object. */
    protected $cquizobj;

    /** @var int the time to be considered as 'now'. */
    protected $timenow;

    /** @var array of cquiz_access_rule_base. */
    protected $rules = array();

    /**
     * Create an instance for a particular cquiz.
     * @param object $cquizobj An instance of the class cquiz from attemptlib.php.
     *      The cquiz we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/cquiz:ignoretimelimits', ...)).
     */
    public function __construct($cquizobj, $timenow, $canignoretimelimits) {
        $this->cquizobj = $cquizobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($cquizobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular cquiz.
     * @param cquiz $cquizobj information about the cquiz in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/cquiz:ignoretimelimits capability.
     * @return array of {@link cquiz_access_rule_base}s.
     */
    protected function make_rules($cquizobj, $timenow, $canignoretimelimits) {

        $rules = array();
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($cquizobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['cquizaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('cquizaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the cquiz table.
     *
     * @param mod_cquiz_mod_form $cquizform the cquiz settings form that is being built.
     * @param MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    public static function add_settings_form_fields(
    mod_cquiz_mod_form $cquizform, MoodleQuickForm $mform) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($cquizform, $mform);
        }
    }
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'cquiz'));
        foreach (self::get_rule_classes() as $rule) {
     ...
     * @param mod_cquiz_mod_form $cquizform the cquiz form object.
     * @return array $errors the updated $errors array.
     */
    public static function validate_settings_form_fields(array $errors, array $data, $files, mod_cquiz_mod_form $cquizform) {

        foreach (self::get_rule_classes() as $rule) {
            $errors = $rule::validate_settings_form_fields($errors, $data, $files, $cquizform);
        }
    ...
    /**
     * Save any submitted settings when the cquiz settings form is submitted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the cquiz table.
     *
     * @param object $cquiz the data from the cquiz form, including $cquiz->id
     *      which is the id of the cquiz being saved.
     */
    public static function save_settings($cquiz) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($cquiz);
        }
    }

    /**
     * Delete any rule-specific settings when the cquiz is deleted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the cquiz table.
     *
     * @param object $cquiz the data from the database, including $cquiz->id
     *      which is the id of the cquiz being deleted.
     * @since Moodle 2.7.1, 2.6.4, 2.5.7
     */
    public static function delete_settings($cquiz) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::delete_settings($cquiz);
        }
    }
     ...
     */
    protected static function get_load_sql($cquizid, $rules, $basefields) {
        $allfields = $basefields;
        $alljoins = '{cquiz} cquiz';
        $allparams = array('cquizid' => $cquizid);

        foreach ($rules as $rule) {
            list($fields, $joins, $params) = $rule::get_settings_sql($cquizid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return array("SELECT $allfields FROM $alljoins WHERE cquiz.id = :cquizid", $allparams);
    }
     ...
     */
    public static function load_settings($cquizid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($cquizid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($cquizid);
        }
    ...
    /**
     * Load the cquiz settings and any settings required by the access rules.
     ...
     *
     * @param int $cquizid the cquiz id.
     * @return object mdl_cquiz row with extra fields.
     */
    public static function load_cquiz_and_settings($cquizid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($cquizid, $rules, 'cquiz.*');
        $cquiz = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($cquizid) as $name => $value) {
                $cquiz->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this cquiz, such
     * as is shown at the top of the cquiz view page. Note that not all
    ...
    /**
     * Whether or not a user should be allowed to start a new attempt at this cquiz now.
    ...
    public function prevent_new_attempt($numprevattempts, $lastattempt) {
        $reasons = array();
        foreach ($this->rules as $rule) {
            $reasons = $this->accumulate_messages($reasons, $rule->prevent_new_attempt($numprevattempts, $lastattempt));
        }
        return $reasons;
    }
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_cquiz_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        ...
        $method = 'post';
        if (!empty($_GET['_qf__mod_cquiz_preflight_check_form'])) {
            $method = 'get';
        }
        return new mod_cquiz_preflight_check_form($url->out_omit_querystring(), array('rules' => $this->rules, 'cquizobj' => $this->cquizobj,
     ...
     * cquiz. Used, for example, to change the label by the grade displayed on the view page from
     ...
     *
     * @param object $attempt the data from the relevant cquiz_attempts row.
    ...
    /**
     * @return bolean if this cquiz should only be shown to students in a popup window.
     */
    ...
    /**
     * Send the user back to the cquiz view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param mod_cquiz_renderer $output the cquiz renderer.
    ...
    public function back_to_view_page($output, $message = '') {
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup($this->cquizobj->view_url(), $message);
            die();
        } else {
            redirect($this->cquizobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the cquiz, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(cquiz_attempt::IN_PROGRESS, cquiz_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = cquiz_attempt_state($this->cquizobj->get_cquiz(), $attempt);
        $reviewoptions = mod_cquiz_display_options::make_from_cquiz(
                        $this->cquizobj->get_cquiz(), $when);

        if (!$reviewoptions->attempt) {
            return $output->no_review_message($this->cquizobj->cannot_review_message($when, true));
        } else {
            return $output->review_link($this->cquizobj->review_url($attempt->id), $this->attempt_must_be_in_popup(), $this->get_popup_options());
        }
    }
