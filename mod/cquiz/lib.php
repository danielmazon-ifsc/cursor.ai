...
function cquiz_add_instance($cquiz) {
    ...
    // Create the first section for this cquiz.
    $DB->insert_record('cquiz_sections', array('cquizid' => $cquiz->id,
...
function cquiz_update_instance($cquiz, $mform) {
    ...
    $cquizdateschanged = $oldcquiz->timelimit != $cquiz->timelimit || $oldcquiz->timeclose != $cquiz->timeclose || $oldcquiz->graceperiod != $cquiz->graceperiod;
...
function cquiz_delete_instance($id) {
    ...
    // Look for random questions that may no longer be used when this cquiz is gone.
    $sql = "SELECT q.id
              FROM {cquiz_slots} slot
              JOIN {question} q ON q.id = slot.questionid
             WHERE slot.cquizid = ? AND q.qtype = ?";
    $questionids = $DB->get_fieldset_sql($sql, array($cquiz->id, 'random'));

    // We need to do this before we try and delete randoms, otherwise they would still be 'in use'.
    $DB->delete_records('cquiz_slots', array('cquizid' => $cquiz->id));
    $DB->delete_records('cquiz_sections', array('cquizid' => $cquiz->id));
    ...
    cquiz_access_manager::delete_settings($cquiz);
...
function cquiz_delete_override($cquiz, $overrideid) {
    ...
    $params = array(
        ...
        'other' => array(
            'cquizid' => $override->cquiz
        )
    );
    // Determine which override deleted event to fire.
    if (!empty($override->userid)) {
        $params['relateduserid'] = $override->userid;
        $event = \mod_cquiz\event\user_override_deleted::create($params);
    } else {
        $params['other']['groupid'] = $override->groupid;
        $event = \mod_cquiz\event\group_override_deleted::create($params);
    }

    // Trigger the override deleted event.
    $event->add_record_snapshot('cquiz_overrides', $override);
    $event->trigger();
...
function cquiz_get_best_grade($cquiz, $userid) {
    global $DB;
    $grade = $DB->get_field('cquiz_grades', 'grade', array('cquiz' => $cquiz->id, 'userid' => $userid));
...
/**
 * Does this cquiz allow multiple tries?
 *
 * @return bool
 */
function cquiz_allows_multiple_tries($cquiz) {
    $bt = question_engine::get_behaviour_type($cquiz->preferredbehaviour);
...
function cquiz_user_complete($course, $user, $mod, $cquiz) {
    ...
    if ($attempts = $DB->get_records('cquiz_attempts', array('userid' => $user->id, 'cquiz' => $cquiz->id), 'attempt')) {
...
function cquiz_get_user_attempts($cquizid, $userid, $status = 'finished', $includepreviews = false) {
    ...
    return $DB->get_records_select('cquiz_attempts', 'cquiz = :cquizid AND userid = :userid' . $previewclause . $statuscondition, $params, 'attempt ASC');
}
 ...
 * @return integer
 */
function cquiz_get_grade_format($cquiz) {
    if (empty($cquiz->questiondecimalpoints)) {
        $cquiz->questiondecimalpoints = -1;
    }

    if ($cquiz->questiondecimalpoints == -1) {
        return $cquiz->decimalpoints;
    }

    return $cquiz->questiondecimalpoints;
}
...
function cquiz_format_question_grade($cquiz, $grade) {
    return format_float($grade, cquiz_get_grade_format($cquiz));
}
...
function cquiz_grade_item_update($cquiz, $grades = null) {
    ...
    $openreviewoptions = mod_cquiz_display_options::make_from_cquiz($cquiz, mod_cquiz_display_options::LATER_WHILE_OPEN);
    $closedreviewoptions = mod_cquiz_display_options::make_from_cquiz($cquiz, mod_cquiz_display_options::AFTER_CLOSE);
    ...
    if (!$params['hidden']) {
        // If the grade item is not hidden by the cquiz logic, then we need to
        // hide it if the cquiz is hidden from students.
        if (property_exists($cquiz, 'visible')) {
            // Saving the cquiz form, and cm not yet updated in the database.
            $params['hidden'] = !$cquiz->visible;
        } else {
    ...
    // Send student a message informing a grade has been earned
    $cquiz->cquizname = $cquiz->name;
    $course = $DB->get_record('course', array('id' => $cquiz->course), '*', MUST_EXIST);
    $cquiz->coursename = $course->fullname;
    if ($grades) {
        foreach ($grades as $grade) {
            if (isset($grade->userid)) {
                $item = grade_item::fetch(array('courseid' => $cquiz->course, 'itemtype' => 'mod', 'itemmodule' => 'cquiz', 'iteminstance' => $cquiz->id, 'outcomeid' => null));
                $old_grade_grade = new grade_grade(array('userid' => $grade->userid, 'itemid' => $item->id), true); //might not exist yet
                if (isset($old_grade_grade->rawgrade) && $old_grade_grade->rawgrade != $grade->rawgrade) {
                    $cquiz->previousgrade = round($old_grade_grade->rawgrade, $cquiz->decimalpoints);
                }
                // Send message only if cm is mandatory and no previous grade or previous grade
                // was different (actually smaller)
                if ((!isset($old_grade_grade->rawgrade) || isset($cquiz->previousgrade)) && (cquiz_is_cm_mandatory($cquiz->id))) {
                    $user = $DB->get_record('user', array('id' => $grade->userid), '*', MUST_EXIST);
                    $cquiz->grade = round($grade->rawgrade, $cquiz->decimalpoints);
                    cquiz_send_grade_message($user, $cquiz);
                }
            }
        }
    }
...
/**
 * Check if a cquiz is mandatory
 * 
 * @param int $cquizid Id of cquiz to be checked
 * @return bool True if cquiz is mandatory
 */
function cquiz_is_cm_mandatory($cquizid) {
    global $DB;

    $sql = 'SELECT ccc.* '
            . 'FROM {course_modules} cm, {cquiz} cq, {course_completion_criteria} ccc '
            . 'WHERE cq.id = cm.instance '
            . 'AND ccc.moduleinstance = cm.id '
            . "AND ccc.module = 'cquiz' "
            . 'AND cq.id = :cquizid;';
    $record = $DB->get_record_sql($sql, array('cquizid' => $cquizid));
    return ($record != null);
}
...
function cquiz_grade_item_delete($cquiz) {
    ...
    return grade_update('mod/cquiz', $cquiz->course, 'mod', 'cquiz', $cquiz->id, 0, null, array('deleted' => 1));
}
 ...
 */
function cquiz_get_recent_mod_activity(&$activities, &$index, $timestart, $courseid, $cmid, $userid = 0, $groupid = 0) {
    global $CFG, $USER, $DB;
    require_once($CFG->dirroot . '/mod/cquiz/locallib.php');
...
function cquiz_after_add_or_update($cquiz) {
    ...
    for ($i = 0; $i <= $cquiz->feedbackboundarycount; $i++) {
        ...
        $feedbacktext = file_save_draft_area_files((int) $cquiz->feedbacktext[$i]['itemid'], $context->id, 'mod_cquiz', 'feedback', $feedback->id, array('subdirs' => false, 'maxfiles' => -1, 'maxbytes' => 0), $cquiz->feedbacktext[$i]['text']);
        $DB->set_field('cquiz_feedback', 'feedbacktext', $feedbacktext, array('id' => $feedback->id));
    }
...
function cquiz_questions_in_use($questionids) {
    ...
    return $DB->record_exists_select('cquiz_slots', 'questionid ' . $test, $params) || question_engine::questions_in_use(
                    $questionids, new qubaid_join('{cquiz_attempts} cquiza', 'cquiza.uniqueid', 'cquiza.preview = 0'));
}
...
function cquiz_reset_course_form_definition($mform) {
    ...
    $mform->addElement('advcheckbox', 'reset_cquiz_attempts', get_string('removeallcquizattempts', 'cquiz'));
    $mform->addElement('advcheckbox', 'reset_cquiz_user_overrides', get_string('removealluseroverrides', 'cquiz'));
    $mform->addElement('advcheckbox', 'reset_cquiz_group_overrides', get_string('removeallgroupoverrides', 'cquiz'));
}
...
function cquiz_reset_course_form_defaults($course) {
    return array('reset_cquiz_attempts' => 1,
        'reset_cquiz_group_overrides' => 1,
        'reset_cquiz_user_overrides' => 1);
}
...
function cquiz_reset_userdata($data) {
    ...
    if (!empty($data->reset_cquiz_attempts)) {
        question_engine::delete_questions_usage_by_activities(new qubaid_join(
                '{cquiz_attempts} cquiza JOIN {cquiz} cquiz ON cquiza.cquiz = cquiz.id', 'cquiza.uniqueid', 'cquiz.course = :cquizcourseid', array('cquizcourseid' => $data->courseid)));

        $DB->delete_records_select('cquiz_attempts', 'cquiz IN (SELECT id FROM {cquiz} WHERE course = ?)', array($data->courseid));
        $status[] = array(
        ...
        $DB->delete_records_select('cquiz_grades', 'cquiz IN (SELECT id FROM {cquiz} WHERE course = ?)', array($data->courseid));
    ...
    if (!empty($data->reset_cquiz_user_overrides)) {
        $DB->delete_records_select('cquiz_overrides', 'cquiz IN (SELECT id FROM {cquiz} WHERE course = ?) AND userid IS NOT NULL', array($data->courseid));
        $status[] = array(
            'component' => $componentstr,
            'item' => get_string('useroverridesdeleted', 'cquiz'),
            'error' => false);
    }
    // Remove group overrides.
    if (!empty($data->reset_cquiz_group_overrides)) {
        $DB->delete_records_select('cquiz_overrides', 'cquiz IN (SELECT id FROM {cquiz} WHERE course = ?) AND groupid IS NOT NULL', array($data->courseid));
        $status[] = array(
            'component' => $componentstr,
            'item' => get_string('groupoverridesdeleted', 'cquiz'),
            'error' => false);
    }
    ...
    if ($data->timeshift) {
        ...
        shift_course_mod_dates('cquiz', array('timeopen', 'timeclose'), $data->timeshift, $data->courseid);
    ...
    foreach ($cquizzes as $cquiz) {
        if ($cquiz->timeclose >= $now && $cquiz->timeopen < $now) {
            ...
            $str = '<div class="cquiz overview">' .
                    ...
                    $cquiz->coursemodule . '">' .
                    $cquiz->name . '</a></div>';
            $str .= '<div class="info">' . get_string('cquizcloseson', 'cquiz', userdate($cquiz->timeclose)) . '</div>';
            ...
            } else if (has_any_capability(array('mod/cquiz:reviewmyattempts', 'mod/cquiz:attempt'), $context)) { // Student
        ...
        if (groups_get_activity_groupmode($cm)) {
            ...
            if ($currentgroup) {
                $a->group = $DB->count_records_sql('SELECT COUNT(DISTINCT qa.id) FROM ' .
                        ...
                        'WHERE cquiz = ? AND preview = 0 AND groupid = ?', array($cquiz->id, $currentgroup));
 ...
 */
function cquiz_attempt_summary_link_to_reports($cquiz, $cm, $context, $returnzero = false, $currentgroup = 0) {
    global $CFG;
...
function cquiz_extend_settings_navigation($settings, $cquiznode) {
    ...
    if (has_capability('mod/cquiz:manageoverrides', $PAGE->cm->context)) {
        ...
        $node = navigation_node::create(get_string('groupoverrides', 'cquiz'), new moodle_url($url, array('mode' => 'group')), navigation_node::TYPE_SETTING, null, 'mod_cquiz_groupoverrides');
        $cquiznode->add_node($node, $beforekey);

        $node = navigation_node::create(get_string('useroverrides', 'cquiz'), new moodle_url($url, array('mode' => 'user')), navigation_node::TYPE_SETTING, null, 'mod_cquiz_useroverrides');
        $cquiznode->add_node($node, $beforekey);
    }

    if (has_capability('mod/cquiz:manage', $PAGE->cm->context)) {
        $node = navigation_node::create(get_string('editcquiz', 'cquiz'), new moodle_url('/mod/cquiz/edit.php', array('cmid' => $PAGE->cm->id)), navigation_node::TYPE_SETTING, null, 'mod_cquiz_edit', new pix_icon('t/edit', ''));
        $cquiznode->add_node($node, $beforekey);
    }

    if (has_capability('mod/cquiz:preview', $PAGE->cm->context)) {
        $url = new moodle_url('/mod/cquiz/startattempt.php', array('cmid' => $PAGE->cm->id, 'sesskey' => sesskey(), 'preview' => 1));
        $node = navigation_node::create(get_string('preview', 'cquiz'), $url, navigation_node::TYPE_SETTING, null, 'mod_cquiz_preview', new pix_icon('i/preview', ''));
        $cquiznode->add_node($node, $beforekey);
    }
    ...
    if (has_any_capability(array('mod/cquiz:viewreports', 'mod/cquiz:grade'), $PAGE->cm->context)) {
        ...
        $url = new moodle_url('/mod/cquiz/report.php', array('id' => $PAGE->cm->id, 'mode' => reset($reportlist)));
        $reportnode = $cquiznode->add_node(navigation_node::create(get_string('results', 'cquiz'), $url, navigation_node::TYPE_SETTING, null, null, new pix_icon('i/report', '')), $beforekey);

        foreach ($reportlist as $report) {
            $url = new moodle_url('/mod/cquiz/report.php', array('id' => $PAGE->cm->id, 'mode' => $report));
            $reportnode->add_node(navigation_node::create(get_string($report, 'cquiz_' . $report), $url, navigation_node::TYPE_SETTING, null, 'cquiz_report_' . $report, new pix_icon('i/item', '')));
        }
    }
 ...
 * @return bool false if file not found, does not return if found - justsend the file
 */
function cquiz_question_pluginfile($course, $context, $component, $filearea, $qubaid, $slot, $args, $forcedownload, array $options = array()) {
    global $CFG;
    require_once($CFG->dirroot . '/mod/cquiz/locallib.php');
    ...
    if (!$attemptobj->check_file_access($slot, $isreviewing, $context->id, $component, $filearea, $args, $forcedownload)) {
        send_file_not_found();
    }
...
function cquiz_page_type_list($pagetype, $parentcontext, $currentcontext) {
    $module_pagetype = array(
        ...
        'mod-cquiz-view' => get_string('page-mod-cquiz-view', 'cquiz'),
        'mod-cquiz-attempt' => get_string('page-mod-cquiz-attempt', 'cquiz'),
        'mod-cquiz-summary' => get_string('page-mod-cquiz-summary', 'cquiz'),
        'mod-cquiz-review' => get_string('page-mod-cquiz-review', 'cquiz'),
        'mod-cquiz-edit' => get_string('page-mod-cquiz-edit', 'cquiz'),
        'mod-cquiz-report' => get_string('page-mod-cquiz-report', 'cquiz'),
    );
...
/**
 * Obtains the automatic completion state for this cquiz on any conditions
 * in cquiz settings, such as if all attempts are used or a certain grade is achieved.
 *
 ...
 */
function cquiz_get_completion_state($course, $cm, $userid, $type) {
    global $DB;
    global $CFG;

    $cquiz = $DB->get_record('cquiz', array('id' => $cm->instance), '*', MUST_EXIST);
    if (!$cquiz->completionattemptsexhausted && !$cquiz->completionpass) {
        return $type;
    }

    // Check if the user has used up all attempts.
    if ($cquiz->completionattemptsexhausted) {
        $attempts = cquiz_get_user_attempts($cquiz->id, $userid, 'finished', true);
        if ($attempts) {
            $lastfinishedattempt = end($attempts);
            $context = context_module::instance($cm->id);
            $cquizobj = cquiz::create($cquiz->id, $userid);
            $accessmanager = new cquiz_access_manager($cquizobj, time(), has_capability('mod/cquiz:ignoretimelimits', $context, $userid, false));
    ...
    if ($cquiz->completionpass) {
        require_once($CFG->libdir . '/gradelib.php');
        $item = grade_item::fetch(array('courseid' => $course->id, 'itemtype' => 'mod',
                    'itemmodule' => 'cquiz', 'iteminstance' => $cm->instance, 'outcomeid' => null));
        if ($item) {
 ...
 */
function cquiz_get_course_module_section($cm) {
    global $DB;
