YUI.add('moodle-mod_cquiz-modform', function (Y, NAME) {

/**
 * The modform class has all the JavaScript specific to mod/cquiz/mod_form.php.
 *
 * @module moodle-mod_cquiz-modform
 */
...
Y.extend(MODFORM, Y.Base, {
    ...
    initializer: function () {
        ...
        if (!this.repaginateCheckbox) {
            // The checkbox only appears when editing an existing cquiz.
            return;
        }
...
// Ensure that M.mod_cquiz exists and that coursebase is initialised correctly
M.mod_cquiz = M.mod_cquiz || {};
M.mod_cquiz.modform = M.mod_cquiz.modform || new MODFORM();
M.mod_cquiz.modform.init = function() {
    return new MODFORM();
};
