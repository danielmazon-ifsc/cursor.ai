YUI.add('moodle-mod_cquiz-util-page', function (Y, NAME) {

/**
 * A collection of utility classes for use with pages.
 *
 * @module moodle-mod_cquiz-util
 * @submodule moodle-mod_cquiz-util-page
 */

Y.namespace('Moodle.mod_cquiz.util.page');

/**
 * A collection of utility classes for use with pages.
 *
 * @class Moodle.mod_cquiz.util.page
 * @static
 */
Y.Moodle.mod_cquiz.util.page = {
    CSS: {
        PAGE : 'page'
    },
    CONSTANTS: {
     ...
     */
    getPages: function() {
        return Y.all(Y.Moodle.mod_cquiz.util.slot.SELECTORS.PAGECONTENT + ' ' +
                     Y.Moodle.mod_cquiz.util.slot.SELECTORS.SECTIONUL + ' ' +
                    this.SELECTORS.PAGE);
    },
    ...
    add: function(beforenode) {
        ...
        var pagehtml = M.mod_cquiz.resource_toolbox.get('config').pagehtml;
        ...
        YUI().use('dd-drop', function(Y) {
            var drop = new Y.DD.Drop({
                node: page,
                groups: M.mod_cquiz.dragres.groups
            });
            page.drop = drop;
        });
    ...
    remove: function(page, keeppagebreak) {
        ...
        var previousslot = page.previous(Y.Moodle.mod_cquiz.util.slot.SELECTORS.SLOT);
        if (!keeppagebreak && previousslot) {
            Y.Moodle.mod_cquiz.util.slot.removePageBreak(previousslot);
        }
        page.remove();
    },
