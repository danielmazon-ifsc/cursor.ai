...
/**
 * This page handles listing of cquiz overrides
 *
 * @package    mod_cquiz
...
$url = new moodle_url('/mod/cquiz/overrides.php', array('cmid' => $cm->id, 'mode' => $mode));
...
$PAGE->set_title(get_string('overrides', 'cquiz'));
...
$sql = 'SELECT o.id
            FROM {cquiz_overrides} o LEFT JOIN {groups} g
            ON o.groupid = g.id
...
if (!empty($orphaned)) {
    $DB->delete_records_list('cquiz_overrides', 'id', array_keys($orphaned));
}
...
if ($groupmode) {
    ...
    $sql = 'SELECT o.*, g.name
                ...
                WHERE o.cquiz = :cquizid
                ORDER BY g.name';
    $params = array('cquizid' => $cquiz->id);
} else {
...
$overridedeleteurl = new moodle_url('/mod/cquiz/overridedelete.php');
$overrideediturl = new moodle_url('/mod/cquiz/overrideedit.php');
...
foreach ($overrides as $override) {
    ...
    if (!$groupmode) {
        if (!has_capability('mod/cquiz:attempt', $context, $override->userid)) {
            // User not allowed to take the cquiz.
            $active = false;
    ...
    if (isset($override->timeopen)) {
        $fields[] = get_string('cquizopens', 'cquiz');
        $values[] = $override->timeopen > 0 ?
                userdate($override->timeopen) : get_string('noopen', 'cquiz');
    }

    // Format timeclose.
    if (isset($override->timeclose)) {
        $fields[] = get_string('cquizcloses', 'cquiz');
        $values[] = $override->timeclose > 0 ?
                userdate($override->timeclose) : get_string('noclose', 'cquiz');
    }

    // Format timelimit.
    if (isset($override->timelimit)) {
        $fields[] = get_string('timelimit', 'cquiz');
        $values[] = $override->timelimit > 0 ?
                format_time($override->timelimit) : get_string('none', 'cquiz');
    }

    // Format number of attempts.
    if (isset($override->attempts)) {
        $fields[] = get_string('attempts', 'cquiz');
    ...
    if (isset($override->password)) {
        $fields[] = get_string('requirepassword', 'cquiz');
        $values[] = $override->password !== '' ?
                get_string('enabled', 'cquiz') : get_string('none', 'cquiz');
    }
...
echo html_writer::start_tag('div', array('id' => 'cquizoverrides'));
if (count($table->data)) {
    echo html_writer::table($table);
}
if ($hasinactive) {
    echo $OUTPUT->notification(get_string('inactiveoverridehelp', 'cquiz'), 'dimmed_text');
}
...
if ($groupmode) {
    if (empty($groups)) {
        // There are no groups.
        echo $OUTPUT->notification(get_string('groupsnone', 'cquiz'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true, array('action' => 'addgroup', 'cmid' => $cm->id)), get_string('addnewgroupoverride', 'cquiz'), 'post', $options);
} else {
    $users = array();
    // See if there are any students in the cquiz.
    $users = get_users_by_capability($context, 'mod/cquiz:attempt', 'u.id');
    $info = new \core_availability\info_module($cm);
    $users = $info->filter_user_list($users);

    if (empty($users)) {
        // There are no students.
        echo $OUTPUT->notification(get_string('usersnone', 'cquiz'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true, array('action' => 'adduser', 'cmid' => $cm->id)), get_string('addnewuseroverride', 'cquiz'), 'get', $options);
}
