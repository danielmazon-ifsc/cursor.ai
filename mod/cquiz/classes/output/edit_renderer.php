...
/**
 * Renderer outputting the cquiz editing UI.
 *
 * @package mod_cquiz
...
namespace mod_cquiz\output;

defined('MOODLE_INTERNAL') || die();

use \mod_cquiz\structure;
use \html_writer;
...
class edit_renderer extends \plugin_renderer_base {
     ...
     *
     * @param \cquiz $cquizobj object containing all the cquiz settings information.
     * @param structure $structure object containing the structure of the cquiz.
     * @param question_edit_contexts $contexts the relevant question bank contexts.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @param array $pagevars the variables from {@link question_edit_setup()}.
     * @return string HTML to output.
     */
    public function edit_page(\cquiz $cquizobj, structure $structure, question_edit_contexts $contexts, \moodle_url $pageurl, array $pagevars) {
        $output = '';

        // Page title.
        $output .= $this->heading_with_help(get_string('editingcquizx', 'cquiz', format_string($cquizobj->get_cquiz_name())), 'editingcquiz', 'cquiz', '', get_string('basicideasofcquiz', 'cquiz'), 2);

        // Information at the top.
        $output .= $this->cquiz_state_warnings($structure);
        $output .= $this->cquiz_information($structure);
        $output .= $this->maximum_grade_input($structure, $pageurl);
        $output .= $this->repaginate_button($structure, $pageurl);
        $output .= $this->total_marks($cquizobj->get_cquiz());
        ...
        if ($structure->can_be_edited()) {
            ...
            $this->page->requires->yui_module('moodle-mod_cquiz-cquizquestionbank', 'M.mod_cquiz.cquizquestionbank.init', array('class' => 'questionbank', 'cmid' => $structure->get_cmid()));

            $popups .= $this->random_question_form($pageurl, $contexts, $pagevars);
            $this->page->requires->yui_module('moodle-mod_cquiz-randomquestion', 'M.mod_cquiz.randomquestion.init');

            $output .= html_writer::div($popups, 'mod_cquiz_edit_forms');

            // Include the question chooser.
            $output .= $this->question_chooser();
            $this->page->requires->yui_module('moodle-mod_cquiz-questionchooser', 'M.mod_cquiz.init_questionchooser');
        }
    ...
    /**
     * Render any warnings that might be required about the state of the cquiz,
     * e.g. if it has been attempted, or if the shuffle questions option is
     * turned on.
     *
     * @param structure $structure the cquiz structure.
     * @return string HTML to output.
     */
    public function cquiz_state_warnings(structure $structure) {
     ...
     * @return string HTML to output.
     */
    public function cquiz_information(structure $structure) {
        list($currentstatus, $explanation) = $structure->get_dates_summary();

        $output = html_writer::span(
                        get_string('numquestionsx', 'cquiz', $structure->get_question_count()), 'numberofquestions') . ' | ' .
                html_writer::span($currentstatus, 'cquizopeningstatus', array('title' => $explanation));

        return html_writer::div($output, 'statusbar');
    }

    /**
     * Render the form for setting a cquiz' overall grade
     *
     * @param structure $structure the cquiz structure.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
     ...
     * @param structure $structure the structure of the cquiz being edited.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
     */
    protected function repaginate_button(structure $structure, \moodle_url $pageurl) {

        $header = html_writer::tag('span', get_string('repaginatecommand', 'cquiz'), array('class' => 'repaginatecommand'));
        ...
        $buttonoptions = array(
            ...
            'value' => get_string('repaginatecommand', 'cquiz'),
        );
        if (!$structure->can_be_repaginated()) {
            $buttonoptions['disabled'] = 'disabled';
        } else {
            $this->page->requires->yui_module('moodle-mod_cquiz-repaginate', 'M.mod_cquiz.repaginate.init');
        }

        return html_writer::tag('div', html_writer::empty_tag('input', $buttonoptions), $containeroptions);
    }
    ...
    protected function repaginate_form(structure $structure, \moodle_url $pageurl) {
        ...
        $select = html_writer::select($perpage, 'questionsperpage', $structure->get_questions_per_page(), false);
        ...
        $formcontent = html_writer::tag('form', html_writer::div(
                                ...
                                get_string('repaginate', 'cquiz', $select) .
    ...
    /**
     * Render the total marks available for the cquiz.
     *
     * @param \stdClass $cquiz the cquiz settings from the database.
     * @return string HTML to output.
     */
    public function total_marks($cquiz) {
        $totalmark = html_writer::span(cquiz_format_grade($cquiz, $cquiz->sumgrades), 'mod_cquiz_summarks');
        return html_writer::tag('span', get_string('totalmarksx', 'cquiz', $totalmark), array('class' => 'totalpoints'));
    }
     ...
     *
     * @param structure $structure the structure of the cquiz being edited.
     * @param \stdClass $section The cquiz_section entry from DB
     * @return string HTML to output.
     */
    protected function start_section($structure, $section) {
        ...
        if (!$structure->can_be_edited()) {
            ...
            $editsectionheadingicon = html_writer::link(new \moodle_url('#'), $this->pix_icon('t/editstring', get_string('sectionheadingedit', 'cquiz', $section->heading), 'moodle', array('class' => 'editicon visibleifjs')), array('class' => 'editing_section', 'data-action' => 'edit_section_title'));
        }
     ...
     *
     * @param structure $structure object containing the structure of the cquiz.
     * @param \stdClass $section data from the cquiz_section table.
     * @return string HTML to output.
     */
    public function section_shuffle_questions(structure $structure, $section) {
        ...
        if ($structure->is_first_section($section)) {
            $help = $this->help_icon('shufflequestions', 'cquiz');
        } else {
            $help = '';
        }
        ...
        $label = html_writer::label(get_string('shufflequestions', 'cquiz') . ' ' . $help, $checkboxattributes['id'], false);
    ...
    /**
     * Render an icon to remove a section from the cquiz.
     *
     * @param object $section the section to be removed.
     * @return string HTML to output.
     */
    public function section_remove_icon($section) {
        $title = get_string('sectionheadingremove', 'cquiz', $section->heading);
        $url = new \moodle_url('/mod/cquiz/edit.php', array('sesskey' => sesskey(), 'removesection' => '1', 'sectionid' => $section->id));
    ...
    /**
     * Renders HTML to display the questions in a section of the cquiz.
     *
     * This function calls {@link core_course_renderer::cquiz_section_question()}
     *
     * @param structure $structure object containing the structure of the cquiz.
     ...
     * @return string HTML to output.
     */
    public function questions_in_section(structure $structure, $section, question_edit_contexts $contexts, $pagevars, $pageurl) {
     ...
     * @return string HTML to output.
     */
    public function question_row(structure $structure, $slot, question_edit_contexts $contexts, $pagevars, $pageurl) {
        $output = '';
        ...
        $joinhtml = '';
        if ($structure->can_be_edited() && !$structure->is_last_slot_in_cquiz($slot) &&
        ...
        $output .= html_writer::tag('li', $questionhtml . $joinhtml, array('class' => $questionclasses, 'id' => 'slot-' . $structure->get_slot_id_for_slot($slot),
     ...
     * @return string HTML to output.
     */
    public function page_row(structure $structure, $slot, question_edit_contexts $contexts, $pagevars, $pageurl) {
        $output = '';
    ...
    public function add_menu_actions(structure $structure, $page, \moodle_url $pageurl, question_edit_contexts $contexts, array $pagevars) {
        ...
        $menu->set_constraint('.mod-cquiz-edit-content');
        $trigger = html_writer::tag('span', get_string('add', 'cquiz'), array('class' => 'add-menu'));
        ...
        // Disable the link if cquiz has attempts.
        if (!$structure->can_be_edited()) {
    ...
    public function edit_menu_actions(structure $structure, $page, \moodle_url $pageurl, array $pagevars) {
        ...
        if (!isset($str)) {
            $str = get_strings(array('addasection', 'addaquestion', 'addarandomquestion',
                'addarandomselectedquestion', 'questionbank'), 'cquiz');
        }
        ...
        $actions['addasection'] = new \action_menu_link_secondary(
                new \moodle_url($pageurl, $params), new \pix_icon('t/add', $str->addasection, 'moodle', array('class' => 'iconsmall', 'title' => '')), $str->addasection, array('class' => 'cm-edit-action addasection', 'data-action' => 'addasection')
        );

        // Add a new question to the cquiz.
        ...
        $actions['addaquestion'] = new \action_menu_link_secondary(
                new \moodle_url('/question/addquestion.php', $params), new \pix_icon('t/add', $str->addaquestion, 'moodle', array('class' => 'iconsmall', 'title' => '')), $str->addaquestion, array('class' => 'cm-edit-action addquestion', 'data-action' => 'addquestion')
        );

        // Call question bank.
        $icon = new \pix_icon('t/add', $str->questionbank, 'moodle', array('class' => 'iconsmall', 'title' => ''));
        if ($page) {
            $title = get_string('addquestionfrombanktopage', 'cquiz', $page);
        } else {
            $title = get_string('addquestionfrombankatend', 'cquiz');
        }
        ...
        $returnurl = new \moodle_url('/mod/cquiz/edit.php', array('cmid' => $structure->get_cmid(), 'data-addonpage' => $page));
        $params = array('returnurl' => $returnurl, 'cmid' => $structure->get_cmid(), 'appendqnumstring' => 'addarandomquestion');
        $url = new \moodle_url('/mod/cquiz/addrandom.php', $params);
        ...
        if ($page) {
            $title = get_string('addrandomquestiontopage', 'cquiz', $page);
        } else {
            $title = get_string('addrandomquestionatend', 'cquiz');
        }
    ...
    /**
     * Render the form that contains the data for adding a new question to the cquiz.
     *
     * @param structure $structure object containing the structure of the cquiz.
     * @param int $page the page number that this menu will add to.
     * @param \moodle_url $pageurl the canonical URL of this page.
    ...
    protected function add_question_form(structure $structure, $page, \moodle_url $pageurl, array $pagevars) {
        ...
        $output = html_writer::tag('input', null, array('type' => 'hidden', 'name' => 'returnurl',
                    'value' => $pageurl->out_as_local_url(false, array('addonpage' => $page))));
        $output .= html_writer::tag('input', null, array('type' => 'hidden', 'name' => 'cmid', 'value' => $structure->get_cmid()));
        $output .= html_writer::tag('input', null, array('type' => 'hidden', 'name' => 'appendqnumstring', 'value' => 'addquestion'));
        $output .= html_writer::tag('input', null, array('type' => 'hidden', 'name' => 'category', 'value' => $questioncategoryid));

        return html_writer::tag('form', html_writer::div($output), array('class' => 'addnewquestion', 'method' => 'post',
    ...
    public function question(structure $structure, $slot, \moodle_url $pageurl) {
        ...
        // Action icons.
        $questionicons = '';
        $questionicons .= $this->question_preview_icon($structure->get_cquiz(), $structure->get_question_in_slot($slot));
        if ($structure->can_be_edited()) {
    ...
    public function question_move_icon(structure $structure, $slot) {
        return html_writer::link(new \moodle_url('#'), $this->pix_icon('i/dragdrop', get_string('move'), 'moodle', array('class' => 'iconsmall', 'title' => '')), array('class' => 'editing_move', 'data-action' => 'move')
        );
    }
     ...
     *
     * @param \stdClass $cquiz the cquiz settings from the database.
     * @param \stdClass $question data from the question and cquiz_slots tables.
     * @param bool $label if true, show the preview question label after the icon
     * @param int $variant which question variant to preview (optional).
     * @return string HTML to output.
     */
    public function question_preview_icon($cquiz, $question, $label = null, $variant = null) {
        $url = cquiz_question_preview_url($cquiz, $question, $variant);
    ...
    /**
     * Render an icon to remove a question from the cquiz.
     *
     * @param structure $structure object containing the structure of the cquiz.
     * @param int $slot the first slot on the page we are outputting.
    ...
    /**
     * Display an icon to split or join two pages of the cquiz.
     *
     * @param structure $structure object containing the structure of the cquiz.
     * @param int $slot the first slot on the page we are outputting.
     * @return string HTML to output.
    ...
    public function page_split_join_button($structure, $slot) {
        ...
        $url = new \moodle_url('repaginate.php', array('cquizid' => $structure->get_cquizid(),
            'slot' => $slot, 'repag' => $insertpagebreak ? 2 : 1, 'sesskey' => sesskey()));

        if ($insertpagebreak) {
            $title = get_string('addpagebreak', 'cquiz');
            $image = $this->pix_icon('e/insert_page_break', $title);
            $action = 'addpagebreak';
        } else {
            $title = get_string('removepagebreak', 'cquiz');
        ...
        return html_writer::span($this->action_link($url, $image, null, array('title' => $title,
                            'class' => 'page_split_join cm-edit-action', 'disabled' => $disabled, 'data-action' => $action)), 'page_split_join_wrapper');
    }
    ...
    public function question_dependency_icon($structure, $slot) {
        ...
        if ($structure->is_question_dependent_on_previous_slot($slot)) {
            $title = get_string('questiondependencyremove', 'cquiz', $a);
            $image = $this->pix_icon('t/locked', get_string('questiondependsonprevious', 'cquiz'), 'moodle', array('title' => ''));
            $action = 'removedependency';
        } else {
            $title = get_string('questiondependencyadd', 'cquiz', $a);
            $image = $this->pix_icon('t/unlocked', get_string('questiondependencyfree', 'cquiz'), 'moodle', array('title' => ''));
        ...
        return html_writer::span($this->action_link('#', $image, null, array('title' => $title,
                            'class' => 'cm-edit-action', 'disabled' => $disabled, 'data-action' => $action)), 'question_dependency_wrapper' . $extraclass);
    }

    /**
     * Renders html to display a name with the link to the question on a cquiz edit page
     *
    ...
    public function question_name(structure $structure, $slot, $pageurl) {
        ...
        $editurl = new \moodle_url('/question/edit.php', array(
            'returnurl' => $pageurl->out_as_local_url(),
            'cmid' => $structure->get_cmid(), 'id' => $question->id));

        $instancename = cquiz_question_tostring($question);
        ...
        $output .= html_writer::link($editurl, $activitylink, array('title' => get_string('editquestion', 'cquiz') . ' ' . $title));
    ...
    public function random_question(structure $structure, $slot, $pageurl) {
        ...
        $temp = clone($question);
        $temp->questiontext = '';
        $instancename = cquiz_question_tostring($temp);

        $configuretitle = get_string('configurerandomquestion', 'cquiz');
        $qtype = \question_bank::get_qtype($question->qtype, false);
        $namestr = $qtype->local_name();
        $icon = $this->pix_icon('icon', $namestr, $qtype->plugin_name(), array('title' => $namestr,
            'class' => 'icon activityicon', 'alt' => ' ', 'role' => 'presentation'));
        ...
        $qbanklink = ' ' . \html_writer::link($qbankurl, get_string('seequestions', 'cquiz'), array('class' => 'mod_cquiz_random_qbank_link'));
    ...
    public function marked_out_of_field(structure $structure, $slot) {
        if (!$structure->is_real_question($slot)) {
            ...
            $output .= html_writer::span(
                            $this->pix_icon('spacer', '', 'moodle', array('class' => 'editicon visibleifjs', 'title' => '')), 'editing_maxmark');
            return html_writer::span($output, 'instancemaxmarkcontainer infoitem');
        }

        $output = html_writer::span($structure->formatted_question_grade($slot), 'instancemaxmark decimalplaces_' . $structure->get_decimal_places_for_question_marks(), array('title' => get_string('maxmark', 'cquiz')));

        $output .= html_writer::span(
                        html_writer::link(
                                new \moodle_url('#'), $this->pix_icon('t/editstring', '', 'moodle', array('class' => 'editicon visibleifjs', 'title' => '')), array(
                            'class' => 'editing_maxmark',
                            'data-action' => 'editmaxmark',
                            'title' => get_string('editmaxmark', 'cquiz'),
                                )
                        )
        );
    ...
    protected function random_question_form(\moodle_url $thispageurl, question_edit_contexts $contexts, array $pagevars) {
        ...
        $randomform = new \cquiz_add_random_form(new \moodle_url('/mod/cquiz/addrandom.php'), array('contexts' => $contexts, 'cat' => $pagevars['cat']));
    ...
    protected function initialise_editing_javascript(structure $structure, question_edit_contexts $contexts, array $pagevars, \moodle_url $pageurl) {
        ...
        $config = new \stdClass();
        $config->resourceurl = '/mod/cquiz/edit_rest.php';
        $config->sectionurl = '/mod/cquiz/edit_rest.php';
        ...
        $this->page->requires->yui_module('moodle-mod_cquiz-toolboxes', 'M.mod_cquiz.init_resource_toolbox', array(array(
                'courseid' => $structure->get_courseid(),
                'cquizid' => $structure->get_cquizid(),
        ...
        $this->page->requires->yui_module('moodle-mod_cquiz-toolboxes', 'M.mod_cquiz.init_section_toolbox', array(array(
                'courseid' => $structure,
                'cquizid' => $structure->get_cquizid(),
        ...
        $this->page->requires->yui_module('moodle-mod_cquiz-dragdrop', 'M.mod_cquiz.init_section_dragdrop', array(array(
                'courseid' => $structure,
                'cquizid' => $structure->get_cquizid(),
                'ajaxurl' => $config->sectionurl,
                'config' => $config,
        ...
        $this->page->requires->yui_module('moodle-mod_cquiz-dragdrop', 'M.mod_cquiz.init_resource_dragdrop', array(array(
                'courseid' => $structure,
                'cquizid' => $structure->get_cquizid(),
                'ajaxurl' => $config->resourceurl,
                'config' => $config,
    ...
    protected function new_page_template(structure $structure, question_edit_contexts $contexts, array $pagevars, \moodle_url $pageurl) {
        ...
        $strcontexts[] = 'addonpage="';
        $strcontexts[] = get_string('addquestionfrombanktopage', 'cquiz', '');
     ...
     *
     * @param \mod_cquiz\question\bank\custom_view $questionbank the question bank view object.
     * @param array $pagevars the variables from {@link \question_edit_setup()}.
     * @return string HTML to output / send back in response to an AJAX request.
     */
    public function question_bank_contents(\mod_cquiz\question\bank\custom_view $questionbank, array $pagevars) {

        $qbank = $questionbank->render('editq', $pagevars['qpage'], $pagevars['qperpage'], $pagevars['cat'], $pagevars['recurse'], $pagevars['showhidden'], $pagevars['qbshowtext']);
