...
/**
 * Admin settings class for the cquiz review options.
 *
 * @package   mod_cquiz
 ...
 * @author     Ricardo Drummond
 */
class mod_cquiz_admin_review_setting extends admin_setting {

    /**
     * @var integer should match the constants defined in
     * {@link mod_cquiz_display_options}. Copied for performance reasons.
     */
    ...
    /**
     * This should match {@link mod_cquiz_mod_form::$reviewfields} but copied
     * here because generating the admin tree needs to be fast.
     * @return array
     */
    public static function fields() {
        return array(
            'attempt' => get_string('theattempt', 'cquiz'),
            ...
            'overallfeedback' => get_string('overallfeedback', 'cquiz'),
        );
    }
    ...
    protected static function times() {
        return array(
            self::DURING => get_string('reviewduring', 'cquiz'),
            self::IMMEDIATELY_AFTER => get_string('reviewimmediately', 'cquiz'),
            self::LATER_WHILE_OPEN => get_string('reviewopen', 'cquiz'),
            self::AFTER_CLOSE => get_string('reviewclosed', 'cquiz'),
        );
    }
    ...
    public function output_html($data, $query = '') {
        ...
        return format_admin_setting($this, $this->visiblename, $return, $this->description, true, '', get_string('everythingon', 'cquiz'), $query);
    }
