 ...
 * @since      Moodle 3.1
 */
class mod_cquiz_external extends external_api {

    /**
     * Describes the parameters for get_cquizzes_by_courses.
     *
     * @return external_external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_cquizzes_by_courses_parameters() {
    ...
    /**
     * Returns a list of cquizzes in a provided list of courses,
     * if no list is provided all cquizzes that the user can view will be returned.
     *
     * @param array $courseids Array of course ids
     * @return array of cquizzes details
     * @since Moodle 3.1
     */
    public static function get_cquizzes_by_courses($courseids = array()) {
        global $USER;

        $warnings = array();
        $returnedcquizzes = array();

        $params = array(
            'courseids' => $courseids,
        );
        $params = self::validate_parameters(self::get_cquizzes_by_courses_parameters(), $params);
        ...
        if (!empty($params['courseids'])) {
            ...
            // Get the cquizzes in this course, this function checks users visibility permissions.
            // We can avoid then additional validate_context calls.
            $cquizzes = get_all_instances_in_courses("cquiz", $courses);
            foreach ($cquizzes as $cquiz) {
                $context = context_module::instance($cquiz->coursemodule);

                // Update cquiz with override information.
                $cquiz = cquiz_update_effective_access($cquiz, $USER->id);

                // Entry to return.
                $cquizdetails = array();
                // First, we return information that any user can see in the web interface.
                $cquizdetails['id'] = $cquiz->id;
                $cquizdetails['coursemodule'] = $cquiz->coursemodule;
                $cquizdetails['course'] = $cquiz->course;
                $cquizdetails['name'] = external_format_string($cquiz->name, $context->id);

                if (has_capability('mod/cquiz:view', $context)) {
                    // Format intro.
                    list($cquizdetails['intro'], $cquizdetails['introformat']) = external_format_text($cquiz->intro, $cquiz->introformat, $context->id, 'mod_cquiz', 'intro', null);
                    ...
                    // Fields the user could see if have access to the cquiz.
                    if (!$accessmanager->prevent_access()) {
                        // Some times this function returns just empty.
                        $hasfeedback = cquiz_has_feedback($cquiz);
                        $cquizdetails['hasfeedback'] = (!empty($hasfeedback)) ? 1 : 0;
                        $cquizdetails['hasquestions'] = (int) $cquizobj->has_questions();
                        $cquizdetails['autosaveperiod'] = get_config('cquiz', 'autosaveperiod');
                    ...
                    foreach ($viewablefields as $field) {
                        $cquizdetails[$field] = $cquiz->{$field};
                    }
                }
                $returnedcquizzes[] = $cquizdetails;
            }
        }
        $result = array();
        $result['cquizzes'] = $returnedcquizzes;
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_cquizzes_by_courses return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_cquizzes_by_courses_returns() {
        return new external_single_structure(
                array(
            'cquizzes' => new external_multiple_structure(
                    new external_single_structure(
                    array(
                'id' => new external_value(PARAM_INT, 'Standard Moodle primary key.'),
                'course' => new external_value(PARAM_INT, 'Foreign key reference to the course this cquiz is part of.'),
                'coursemodule' => new external_value(PARAM_INT, 'Course module id.'),
                'name' => new external_value(PARAM_RAW, 'Cquiz name.'),
                'intro' => new external_value(PARAM_RAW, 'Cquiz introduction text.', VALUE_OPTIONAL),
                'introformat' => new external_format_value('intro', VALUE_OPTIONAL),
                'timeopen' => new external_value(PARAM_INT, 'The time when this cquiz opens. (0 = no restriction.)', VALUE_OPTIONAL),
                'timeclose' => new external_value(PARAM_INT, 'The time when this cquiz closes. (0 = no restriction.)', VALUE_OPTIONAL),
                'timelimit' => new external_value(PARAM_INT, 'The time limit for cquiz attempts, in seconds.', VALUE_OPTIONAL),
                'overduehandling' => new external_value(PARAM_ALPHA, 'The method used to handle overdue attempts.
                                                                    \'autosubmit\', \'graceperiod\' or \'autoabandon\'.', VALUE_OPTIONAL),
                ...
                'preferredbehaviour' => new external_value(PARAM_ALPHANUMEXT, 'The behaviour to ask questions to use.', VALUE_OPTIONAL),
                'canredoquestions' => new external_value(PARAM_INT, 'Allows students to redo any completed question
                                                                        within a cquiz attempt.', VALUE_OPTIONAL),
                'attempts' => new external_value(PARAM_INT, 'The maximum number of attempts a student is allowed.', VALUE_OPTIONAL),
                'attemptonlast' => new external_value(PARAM_INT, 'Whether subsequent attempts start from the answer
                                                                    to the previous attempt (1) or start blank (0).', VALUE_OPTIONAL),
                'grademethod' => new external_value(PARAM_INT, 'One of the values CQUIZ_GRADEHIGHEST, CQUIZ_GRADEAVERAGE,
                                                                    CQUIZ_ATTEMPTFIRST or CQUIZ_ATTEMPTLAST.', VALUE_OPTIONAL),
                ...
                'reviewattempt' => new external_value(PARAM_INT, 'Whether users are allowed to review their cquiz
                                                                    attempts at various times. This is a bit field, decoded by the
                                                                    mod_cquiz_display_options class. It is formed by ORing together
                                                                    the constants defined there.', VALUE_OPTIONAL),
                'reviewcorrectness' => new external_value(PARAM_INT, 'Whether users are allowed to review their cquiz
                                                                        attempts at various times.
                                                                        A bit field, like reviewattempt.', VALUE_OPTIONAL),
                'reviewmarks' => new external_value(PARAM_INT, 'Whether users are allowed to review their cquiz attempts
                                                                at various times. A bit field, like reviewattempt.', VALUE_OPTIONAL),
                'reviewspecificfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their
                                                                            cquiz attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                'reviewgeneralfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their
                                                                            cquiz attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                'reviewrightanswer' => new external_value(PARAM_INT, 'Whether users are allowed to review their cquiz
                                                                        attempts at various times. A bit field, like
                                                                        reviewattempt.', VALUE_OPTIONAL),
                'reviewoverallfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their cquiz
                                                                            attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                'questionsperpage' => new external_value(PARAM_INT, 'How often to insert a page break when editing
                                                                        the cquiz, or when shuffling the question order.', VALUE_OPTIONAL),
                'navmethod' => new external_value(PARAM_ALPHA, 'Any constraints on how the user is allowed to navigate
                                                                around the cquiz. Currently recognised values are
                ...
                'sumgrades' => new external_value(PARAM_FLOAT, 'The total of all the question instance maxmarks.', VALUE_OPTIONAL),
                'grade' => new external_value(PARAM_FLOAT, 'The total that the cquiz overall grade is scaled to be
                                                            out of.', VALUE_OPTIONAL),
                'timecreated' => new external_value(PARAM_INT, 'The time when the cquiz was added to the course.', VALUE_OPTIONAL),
                ...
                'password' => new external_value(PARAM_RAW, 'A password that the student must enter before starting or
                                                                continuing a cquiz attempt.', VALUE_OPTIONAL),
                'subnet' => new external_value(PARAM_RAW, 'Used to restrict the IP addresses from which this cquiz can
                ...
                'completionattemptsexhausted' => new external_value(PARAM_INT, 'Mark cquiz complete when the student has
                                                                                exhausted the maximum number of attempts', VALUE_OPTIONAL),
                ...
                'hasfeedback' => new external_value(PARAM_INT, 'Whether the cquiz has any non-blank feedback text', VALUE_OPTIONAL),
                'hasquestions' => new external_value(PARAM_INT, 'Whether the cquiz has questions', VALUE_OPTIONAL),
    ...
    /**
     * Utility function for validating a cquiz.
     *
     * @param int $cquizid cquiz instance id
     * @return array array containing the cquiz, course, context and course module objects
     * @since  Moodle 3.1
     */
    protected static function validate_cquiz($cquizid) {
        global $DB;
        ...
        return array($cquiz, $course, $cm, $context);
    }

    /**
     * Describes the parameters for view_cquiz.
     *
     * @return external_external_function_parameters
     * @since Moodle 3.1
     */
    public static function view_cquiz_parameters() {
        return new external_function_parameters(
                array(
            'cquizid' => new external_value(PARAM_INT, 'cquiz instance id'),
                )
        );
    }
     ...
     */
    public static function view_cquiz($cquizid) {
        global $DB;

        $params = self::validate_parameters(self::view_cquiz_parameters(), array('cquizid' => $cquizid));
        $warnings = array();

        list($cquiz, $course, $cm, $context) = self::validate_cquiz($params['cquizid']);
    ...
    /**
     * Describes the view_cquiz return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function view_cquiz_returns() {
        return new external_single_structure(
                array(
    ...
    public static function get_user_attempts_parameters() {
        return new external_function_parameters(
            ...
            'status' => new external_value(PARAM_ALPHA, 'cquiz status: all, finished or unfinished', VALUE_DEFAULT, 'finished'),
    ...
    /**
     * Return a list of attempts for the given cquiz and user.
     *
     * @param int $cquizid cquiz instance id
     * @param int $userid user id
     * @param string $status cquiz status: all, finished or unfinished
     * @param bool $includepreviews whether to include previews or not
     * @return array of warnings and the list of attempts
     * @since Moodle 3.1
     * @throws invalid_parameter_exception
     */
    public static function get_user_attempts($cquizid, $userid = 0, $status = 'finished', $includepreviews = false) {
        global $DB, $USER;
        ...
        if ($USER->id != $user->id) {
            require_capability('mod/cquiz:viewreports', $context);
        }

        $attempts = cquiz_get_user_attempts($cquiz->id, $user->id, $params['status'], $params['includepreviews']);
    ...
    private static function attempt_structure() {
        return new external_single_structure(
            ...
            'cquiz' => new external_value(PARAM_INT, 'Foreign key reference to the cquiz that was attempted.', VALUE_OPTIONAL),
            'userid' => new external_value(PARAM_INT, 'Foreign key reference to the user whose attempt this is.', VALUE_OPTIONAL),
            'attempt' => new external_value(PARAM_INT, 'Sequentially numbers this students attempts at this cquiz.', VALUE_OPTIONAL),
            'uniqueid' => new external_value(PARAM_INT, 'Foreign key reference to the question_usage that holds the
                                                    details of the the question_attempts that make up this cquiz
            ...
            'timemodified' => new external_value(PARAM_INT, 'Last modified time.', VALUE_OPTIONAL),
            'timecheckstate' => new external_value(PARAM_INT, 'Next time cquiz cron should check attempt for
    ...
    /**
     * Get the best current grade for the given user on a cquiz.
     *
     * @param int $cquizid cquiz instance id
     * @param int $userid user id
     ...
     * @since Moodle 3.1
     */
    public static function get_user_best_grade($cquizid, $userid = 0) {
        global $DB, $USER;
        ...
        $result = array();
        $grade = cquiz_get_best_grade($cquiz, $user->id);
    ...
    public static function get_user_best_grade_returns() {
        return new external_single_structure(
                array(
            'hasgrade' => new external_value(PARAM_BOOL, 'Whether the user has a grade on the given cquiz.'),
     ...
     * @since Moodle 3.1
     */
    public static function get_combined_review_options($cquizid, $userid = 0) {
        global $DB, $USER;
        ...
        $attempts = cquiz_get_user_attempts($cquiz->id, $user->id, 'all', true);
    ...
    /**
     * Starts a new attempt at a cquiz.
     *
     * @param int $cquizid cquiz instance id
     ...
     */
    public static function start_attempt($cquizid, $preflightdata = array(), $forcenew = false) {
        global $DB, $USER;
        ...
        if (!$cquizobj->has_questions()) {
            throw new moodle_cquiz_exception($cquizobj, 'noquestionsfound');
        }
        ...
        list($currentattemptid, $attemptnumber, $lastattempt, $messages, $page) = cquiz_validate_new_attempt($cquizobj, $accessmanager, $forcenew, -1, false);
        ...
        if (!$cquizobj->is_preview_user() && $messages) {
            ...
            if ($accessmanager->is_preflight_check_required($currentattemptid)) {
                ...
                if (!empty($errors)) {
                    throw new moodle_cquiz_exception($cquizobj, array_shift($errors));
                }
            ...
            if ($currentattemptid) {
                if ($lastattempt->state == cquiz_attempt::OVERDUE) {
                    throw new moodle_cquiz_exception($cquizobj, 'stateoverdue');
                } else {
                    throw new moodle_cquiz_exception($cquizobj, 'attemptstillinprogress');
                }
            }
            $attempt = cquiz_prepare_and_start_new_attempt($cquizobj, $attemptnumber, $lastattempt);
        }
     ...
     * @param  bool $checkaccessrules whether to check the cquiz access rules or not
    ...
    protected static function validate_attempt($params, $checkaccessrules = true, $failifoverdue = true) {
        ...
        $attemptobj = cquiz_attempt::create($params['attemptid']);
        ...
        if ($checkaccessrules) {
            ...
            if (!$ispreviewuser && $messages) {
                throw new moodle_cquiz_exception($attemptobj->get_cquizobj(), 'attempterror');
            }
        }

        // Attempt closed?.
        if ($attemptobj->is_finished()) {
            throw new moodle_cquiz_exception($attemptobj->get_cquizobj(), 'attemptalreadyclosed');
        } else if ($failifoverdue && $attemptobj->get_state() == cquiz_attempt::OVERDUE) {
            throw new moodle_cquiz_exception($attemptobj->get_cquizobj(), 'stateoverdue');
        }

        // User submitted data (like the cquiz password).
        if ($accessmanager->is_preflight_check_required($attemptobj->get_attemptid())) {
            ...
            if (!empty($errors)) {
                throw new moodle_cquiz_exception($attemptobj->get_cquizobj(), array_shift($errors));
            }
            // Pre-flight check passed.
        ...
        if (isset($params['page'])) {
            ...
            if ($params['page'] != $attemptobj->force_page_number_into_range($params['page'])) {
                throw new moodle_cquiz_exception($attemptobj->get_cquizobj(), 'Invalid page number');
            }

            // Prevent out of sequence access.
            if (!$attemptobj->check_page_access($params['page'])) {
                throw new moodle_cquiz_exception($attemptobj->get_cquizobj(), 'Out of sequence access');
            }
    ...
    private static function question_structure() {
        return new external_single_structure(
            ...
            'page' => new external_value(PARAM_INT, 'page of the cquiz this question appears on'),
            ...
            'number' => new external_value(PARAM_INT, 'question ordering number in the cquiz', VALUE_OPTIONAL),
     ...
     *
     * @param  cquiz_attempt  $attemptobj  the cquiz attempt object
     * @param  bool  $review  whether if we are in review mode or not
     * @param  mixed  $page  string 'all' or integer page number
     * @return array array of questions including data
     */
    private static function get_attempt_questions_data(cquiz_attempt $attemptobj, $review, $page = 'all') {
        global $PAGE;

        $questions = array();
        $contextid = $attemptobj->get_cquizobj()->get_context()->id;
        $displayoptions = $attemptobj->get_display_options($review);
        $renderer = $PAGE->get_renderer('mod_cquiz');
    ...
    /**
     * Returns information for the given attempt page for a cquiz attempt in progress.
     *
     ...
     * @since Moodle 3.1
     * @throws moodle_cquiz_exceptions
     */
    ...
    public static function get_attempt_data_returns() {
        return new external_single_structure(
            ...
            'messages' => new external_multiple_structure(
                    new external_value(PARAM_TEXT, 'access message'), 'access messages, will only be returned for users with mod/cquiz:preview capability,
    ...
    /**
     * Returns a summary of a cquiz attempt before it is submitted.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Processes save requests during the cquiz. This function is intended for the cquiz auto-save feature.
     *
     * @param int $attemptid attempt id
    ...
    public static function process_attempt_parameters() {
        return new external_function_parameters(
            ...
            'timeup' => new external_value(PARAM_BOOL, 'whether the WS was called by a timer when the time is up', VALUE_DEFAULT, false),
            'preflightdata' => new external_multiple_structure(
                    new external_single_structure(
                    array(
                'name' => new external_value(PARAM_ALPHANUMEXT, 'data name'),
    ...
    /**
     * Process responses during an attempt at a cquiz and also deals with attempts finishing.
     *
     * @param int $attemptid attempt id
     * @param array $data the data to be saved
    ...
    protected static function validate_attempt_review($params) {
        ...
        if ($attemptobj->is_own_attempt()) {
            if (!$attemptobj->is_finished()) {
                throw new moodle_cquiz_exception($attemptobj->get_cquizobj(), 'attemptclosed');
    ...
    public static function get_attempt_review($attemptid, $page = -1) {
        ...
        foreach ($summarydata as $key => $data) {
            ...
            $result['additionaldata'][] = array(
                'id' => $key,
                'title' => $data['title'], $attemptobj->get_cquizobj()->get_context()->id,
                'content' => $data['content'],
            );
        }

        // Feedback if there is any, and the user is allowed to see it now.
        $grade = cquiz_rescale_grade($attemptobj->get_attempt()->sumgrades, $attemptobj->get_cquiz(), false);

        $feedback = $attemptobj->get_overall_feedback($grade);
        if ($displayoptions->overallfeedback && $feedback) {
            $result['additionaldata'][] = array(
                'id' => 'feedback',
                'title' => get_string('feedback', 'cquiz'),
                'content' => $feedback,
            );
        }
    ...
    public static function get_attempt_review_returns() {
        return new external_single_structure(
                array(
            'grade' => new external_value(PARAM_RAW, 'grade for the cquiz (or empty or "notyetgraded")'),
            'attempt' => self::attempt_structure(),
     ...
     * @since Moodle 3.1
     */
    public static function get_cquiz_feedback_for_grade_parameters() {
        return new external_function_parameters(
                array(
            'cquizid' => new external_value(PARAM_INT, 'cquiz instance id'),
    ...
    /**
     * Get the feedback text that should be show to a student who got the given grade in the given cquiz.
     *
     * @param int $cquizid cquiz instance id
     ...
     * @throws moodle_exception
     */
    public static function get_cquiz_feedback_for_grade($cquizid, $grade) {
        global $DB;
        ...
        $params = self::validate_parameters(self::get_cquiz_feedback_for_grade_parameters(), $params);
        $warnings = array();
        ...
        $feedback = cquiz_feedback_record_for_grade($params['grade'], $cquiz);
        if (!empty($feedback->feedbacktext)) {
            list($text, $format) = external_format_text($feedback->feedbacktext, $feedback->feedbacktextformat, $context->id, 'mod_cquiz', 'feedback', $feedback->id);
    ...
    /**
     * Describes the get_cquiz_feedback_for_grade return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_cquiz_feedback_for_grade_returns() {
        return new external_single_structure(
                array(
    ...
    /**
     * Describes the parameters for get_cquiz_access_information.
     *
     * @return external_external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_cquiz_access_information_parameters() {
        return new external_function_parameters(
                array(
            'cquizid' => new external_value(PARAM_INT, 'cquiz instance id')
                )
    ...
    /**
     * Return access information for a given cquiz.
     *
     * @param int $cquizid cquiz instance id
     ...
     * @throws  moodle_cquiz_exception
     */
    public static function get_cquiz_access_information($cquizid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_cquiz_access_information_parameters(), $params);

        list($cquiz, $course, $cm, $context) = self::validate_cquiz($params['cquizid']);

        $result = array();
        // Capabilities first.
        $result['canattempt'] = has_capability('mod/cquiz:attempt', $context);
        ;
        $result['canmanage'] = has_capability('mod/cquiz:manage', $context);
        ;
        $result['canpreview'] = has_capability('mod/cquiz:preview', $context);
        ;
        $result['canreviewmyattempts'] = has_capability('mod/cquiz:reviewmyattempts', $context);
        ;
        $result['canviewreports'] = has_capability('mod/cquiz:viewreports', $context);
        ;

        // Access manager now.
        $cquizobj = cquiz::create($cm->instance, $USER->id);
        $ignoretimelimits = has_capability('mod/cquiz:ignoretimelimits', $context, null, false);
        $timenow = time();
        $accessmanager = new cquiz_access_manager($cquizobj, $timenow, $ignoretimelimits);
    ...
    /**
     * Describes the get_cquiz_access_information return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_cquiz_access_information_returns() {
        return new external_single_structure(
                array(
            'canattempt' => new external_value(PARAM_BOOL, 'Whether the user can do the cquiz or not.'),
            'canmanage' => new external_value(PARAM_BOOL, 'Whether the user can edit the cquiz settings or not.'),
            'canpreview' => new external_value(PARAM_BOOL, 'Whether the user can preview the cquiz or not.'),
            'canreviewmyattempts' => new external_value(PARAM_BOOL, 'Whether the users can review their previous attempts
                                                                or not.'),
            'canviewreports' => new external_value(PARAM_BOOL, 'Whether the user can view the cquiz reports or not.'),
    ...
    /**
     * Return access information for a given attempt in a cquiz.
     *
     * @param int $cquizid cquiz instance id
     ...
     * @throws  moodle_cquiz_exception
     */
    public static function get_attempt_access_information($cquizid, $attemptid = 0) {
        global $DB, $USER;
        ...
        if ($unfinishedattempt = cquiz_get_user_attempt_unfinished($cquiz->id, $USER->id)) {
            ...
            if ($unfinishedattempt->state != cquiz_attempt::IN_PROGRESS and $unfinishedattempt->state != cquiz_attempt::OVERDUE) {
    ...
    public static function get_attempt_access_information_returns() {
        return new external_single_structure(
                array(
            'endtime' => new external_value(PARAM_INT, 'When the attempt must be submitted (determined by rules).', VALUE_OPTIONAL),
    ...
    /**
     * Describes the parameters for get_cquiz_required_qtypes.
     *
     * @return external_external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_cquiz_required_qtypes_parameters() {
        return new external_function_parameters(
                array(
            'cquizid' => new external_value(PARAM_INT, 'cquiz instance id')
                )
    ...
    /**
     * Return the potential question types that would be required for a given cquiz.
     ...
     * @throws  moodle_cquiz_exception
     */
    public static function get_cquiz_required_qtypes($cquizid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_cquiz_required_qtypes_parameters(), $params);
        ...
        // Question types used.
        $result = array();
        $result['questiontypes'] = $cquizobj->get_all_question_types_used(true);
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_cquiz_required_qtypes return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_cquiz_required_qtypes_returns() {
        return new external_single_structure(
                array(
            'questiontypes' => new external_multiple_structure(
                    new external_value(PARAM_PLUGIN, 'question type'), 'list of question types used in the cquiz'),
            'warnings' => new external_warnings(),
                )
        );
    }
