...
/**
 * Defines the cquiz repaginate class.
 *
 * @package   mod_cquiz
 ...
 *
 * The cquiz setting allows users to write cquizzes with one question per page,
...
class repaginate {
    ...
    /** @var int the id of the cquiz being manipulated. */
    private $cquizid;

    /** @var array the cquiz_slots for that cquiz. */
    private $slots;

    /**
     * Constructor.
     * @param int $cquizid the id of the cquiz being manipulated.
     * @param stdClass[] $slots the cquiz_slots for that cquiz.
     */
    public function __construct($cquizid = 0, $slots = null) {
        global $DB;
        $this->cquizid = $cquizid;
        if (!$this->cquizid) {
            $this->slots = array();
        }
        if (!$slots) {
            $this->slots = $DB->get_records('cquiz_slots', array('cquizid' => $this->cquizid), 'slot');
        } else {
            $this->slots = $slots;
        }
    }
    ...
    public function repaginate_slots($nextslotnumber, $type) {
        ...
        foreach ($this->slots as $slot) {
            if ($slot->slot < $nextslotnumber) {
                ...
                // Update DB.
                $DB->update_record('cquiz_slots', $nextslot, true);
    ...
    /**
     * Repaginate the rest.
     * @param stdClass[] $cquizslots
     * @param int $slotfrom
     * @param int $type
     * @param bool $dbupdate
     * @return stdClass[]
     */
    public function repaginate_the_rest($cquizslots, $slotfrom, $type, $dbupdate = true) {
        global $DB;
        if (!$cquizslots) {
            return null;
        }
        $newslots = array();
        foreach ($cquizslots as $slot) {
            if ($type == self::LINK) {
