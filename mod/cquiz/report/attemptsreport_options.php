...
/**
 * Base class for the options that control what is visible in an {@link cquiz_attempts_report}.
 *
 * @package   mod_cquiz
 ...
 * @author     Ricardo Drummond
 */
class mod_cquiz_attempts_report_options {

    /** @var string the report mode. */
    public $mode;

    /** @var object the settings for the cquiz being reported on. */
    public $cquiz;

    /** @var object the course module objects for the cquiz being reported on. */
    public $cm;

    /** @var object the course settings for the course the cquiz is in. */
    public $course;

    /**
     * @var array form field name => corresponding cquiz_attempt:: state constant.
     */
    protected static $statefields = array(
        'stateinprogress' => cquiz_attempt::IN_PROGRESS,
        'stateoverdue' => cquiz_attempt::OVERDUE,
        'statefinished' => cquiz_attempt::FINISHED,
        'stateabandoned' => cquiz_attempt::ABANDONED,
    );

    /**
     * @var string cquiz_attempts_report::ALL_WITH or cquiz_attempts_report::ENROLLED_WITH
     *      cquiz_attempts_report::ENROLLED_WITHOUT or cquiz_attempts_report::ENROLLED_ALL
     */
    public $attempts = cquiz_attempts_report::ENROLLED_WITH;

    /** @var int the currently selected group. 0 if no group is selected. */
    public $group = 0;

    /**
     * @var array|null of cquiz_attempt::IN_PROGRESS, etc. constants. null means
     *      no restriction.
     */
    public $states = array(cquiz_attempt::IN_PROGRESS, cquiz_attempt::OVERDUE,
        cquiz_attempt::FINISHED, cquiz_attempt::ABANDONED);
    ...
    public $pagesize = cquiz_attempts_report::DEFAULT_PAGE_SIZE;
     ...
     * @param object $cquiz the settings for the cquiz being reported on.
     * @param object $cm the course module objects for the cquiz being reported on.
     * @param object $coures the course settings for the coures this cquiz is in.
     */
    public function __construct($mode, $cquiz, $cm, $course) {
        $this->mode = $mode;
        $this->cquiz = $cquiz;
        $this->cm = $cm;
        $this->course = $course;

        $this->usercanseegrades = cquiz_report_should_show_grades($cquiz, context_module::instance($cm->id));
    }
    ...
    public function get_url() {
        return new moodle_url('/mod/cquiz/report.php', $this->get_url_params());
    }
    ...
    public function setup_from_user_preferences() {
        $this->pagesize = get_user_preferences('cquiz_report_pagesize', $this->pagesize);
    }
    ...
    public function update_user_preferences() {
        set_user_preference('cquiz_report_pagesize', $this->pagesize);
    }
    ...
    public function resolve_dependencies() {
        if ($this->group) {
            ...
            if ($this->attempts === null || $this->attempts == cquiz_attempts_report::ALL_WITH) {
                $this->attempts = cquiz_attempts_report::ENROLLED_WITH;
            }
        } else if (!$this->group && $this->course->id == SITEID) {
            // Force report on front page to show all, unless a group is selected.
            $this->attempts = cquiz_attempts_report::ALL_WITH;
        } else if (!in_array($this->attempts, array(cquiz_attempts_report::ALL_WITH, cquiz_attempts_report::ENROLLED_WITH,
                    cquiz_attempts_report::ENROLLED_WITHOUT, cquiz_attempts_report::ENROLLED_ALL))) {
            $this->attempts = cquiz_attempts_report::ENROLLED_WITH;
        }
        ...
        if (!cquiz_report_can_filter_only_graded($this->cquiz)) {
        ...
        if ($this->attempts == cquiz_attempts_report::ENROLLED_WITHOUT) {
            $this->states = null;
            $this->onlygraded = false;
        }
        ...
        if ($this->pagesize < 1) {
            $this->pagesize = cquiz_attempts_report::DEFAULT_PAGE_SIZE;
        }
    }
    ...
    protected function is_showing_finished_attempts() {
        return $this->states === null || in_array(cquiz_attempt::FINISHED, $this->states);
    }
