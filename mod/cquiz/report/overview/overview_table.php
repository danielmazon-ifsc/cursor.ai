...
class cquiz_overview_table extends cquiz_attempts_report_table {
     ...
     */
    public function __construct($cquiz, $context, $qmsubselect, cquiz_overview_options $options, $groupstudents, $students, $questions, $reporturl) {
        parent::__construct('mod-cquiz-report-overview-report', $cquiz, $context, $qmsubselect, $options, $groupstudents, $students, $questions, $reporturl);
    }
    ...
    public function col_sumgrades($attempt) {
        ...
        return html_writer::link(new moodle_url('/mod/cquiz/review.php', array('attempt' => $attempt->attempt)), $grade, array('title' => get_string('reviewattempt', 'cquiz')));
    }
    ...
    public function other_cols($colname, $attempt) {
        ...
        if (isset($this->regradedqs[$attempt->usageid][$slot])) {
            ...
            $newgrade = cquiz_rescale_grade(
                    $this->regradedqs[$attempt->usageid][$slot]->newfraction * $question->maxmark, $this->cquiz, 'question');
            $oldgrade = cquiz_rescale_grade(
                    $this->regradedqs[$attempt->usageid][$slot]->oldfraction * $question->maxmark, $this->cquiz, 'question');
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('cquiz_overview_regrades', 'questionusageid ' . $qubaids->usage_id_in(), $qubaids->usage_id_in_params());
