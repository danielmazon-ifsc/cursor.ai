...
/**
 * Cquiz report subclass for the overview (grades) report.
 *
 * @copyright 2017 Viddia (http://viddia.com.br)
 * @author     Ricardo Drummond
 */
class cquiz_overview_report extends cquiz_attempts_report {
    ...
    public function display($cquiz, $cm, $course) {
        ...
        list($currentgroup, $students, $groupstudents, $allowed) = $this->init('overview', 'cquiz_overview_settings_form', $cquiz, $cm, $course);
        ...
        $table = new cquiz_overview_table($cquiz, $this->context, $this->qmsubselect, $options, $groupstudents, $students, $questions, $options->get_url());
        $filename = cquiz_report_download_filename(get_string('overviewfilename', 'cquiz_overview'), $courseshortname, $cquiz->name);
            ...
            if (!$table->is_downloading()) {
                ...
                if (has_capability('mod/cquiz:regrade', $this->context)) {
                    ...
                    if ($currentgroup) {
                        ...
                        $regradealldrydolabel = get_string('regradealldrydogroup', 'cquiz_overview', $a);
                        $regradealldrylabel = get_string('regradealldrygroup', 'cquiz_overview', $a);
                        $regradealllabel = get_string('regradeallgroup', 'cquiz_overview', $a);
                    } else {
                        $regradealldrydolabel = get_string('regradealldrydo', 'cquiz_overview', $regradesneeded);
                        $regradealldrylabel = get_string('regradealldry', 'cquiz_overview');
                        $regradealllabel = get_string('regradeall', 'cquiz_overview');
                    }
        ...
        if (!$table->is_downloading() && $options->usercanseegrades) {
            ...
            if ($currentgroup && $groupstudents) {
                ...
                $params[] = $cquiz->id;
                if ($DB->record_exists_select('cquiz_grades', "userid $usql AND cquiz = ?", $params)) {
                    $imageurl = new moodle_url('/mod/cquiz/report/overview/overviewgraph.php', array('id' => $cquiz->id, 'groupid' => $currentgroup));
                    $graphname = get_string('overviewreportgraphgroup', 'cquiz_overview', groups_get_group_name($currentgroup));
                    echo $output->graph($imageurl, $graphname);
                }
            }

            if ($DB->record_exists('cquiz_grades', array('cquiz' => $cquiz->id))) {
                $imageurl = new moodle_url('/mod/cquiz/report/overview/overviewgraph.php', array('id' => $cquiz->id));
    ...
    protected function finish_regrade($nexturl) {
        redirect($nexturl, get_string('regradecomplete', 'cquiz_overview'), null, \core\output\notification::NOTIFY_SUCCESS);
    }
     ...
     */
    protected function regrade_attempts($cquiz, $dryrun = false, $groupstudents = array(), $attemptids = array()) {
        global $DB;
        $this->unlock_session();
        ...
        foreach ($attempts as $attempt) {
            ...
            $a['done'] ++;
            $progressbar->update($a['done'], $a['count'], get_string('regradingattemptxofy', 'cquiz_overview', $a));
        }
    ...
    protected function regrade_attempts_needing_it($cquiz, $groupstudents) {
        ...
        $attempts = $DB->get_records_list('cquiz_attempts', 'uniqueid', array_keys($attemptquestions));
    ...
    protected function has_regraded_questions($from, $where, $params) {
        ...
        return $DB->record_exists_sql("
                SELECT 1
                  FROM {$from}
                  JOIN {cquiz_overview_regrades} qor ON qor.questionusageid = cquiza.uniqueid
                 WHERE {$where}", $params);
    }
    ...
    protected function clear_regrade_table($cquiz, $groupstudents) {
        ...
        $params[] = $cquiz->id;
        $DB->delete_records_select('cquiz_overview_regrades', "questionusageid IN (
                    SELECT uniqueid
                    FROM {cquiz_attempts}
                    WHERE $where cquiz = ?
                )", $params);
