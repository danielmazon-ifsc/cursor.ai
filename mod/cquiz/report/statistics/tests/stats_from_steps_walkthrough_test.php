 ...
 *
 * The cquiz stats below and the question stats found in qstats00.csv were calculated independently in a spreadsheet which is
 * available in open document or excel format here :
 * https://github.com/jamiepratt/moodle-cquiz-tools/tree/master/statsspreadsheet
 *
 ...
 * @author     Ricardo Drummond
 */
class cquiz_report_statistics_from_steps_testcase extends mod_cquiz_attempt_walkthrough_from_csv_testcase {

    /**
     * @var cquiz_statistics_report object to do stats calculations.
     */
    protected $report;
    ...
    public function test_walkthrough_from_csv($cquizsettings, $csvdata) {
        ...
        $whichattempts = CQUIZ_GRADEAVERAGE; // All attempts.
        $whichtries = question_attempt::ALL_TRIES;
        $groupstudents = array();
        list($questions, $cquizstats, $questionstats, $qubaids) = $this->check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries, $groupstudents);
        if ($cquizsettings['testnumber'] === '00') {
            $this->check_variants_count_for_cquiz_00($questions, $questionstats, $whichtries, $qubaids);
            $this->check_cquiz_stats_for_cquiz_00($cquizstats);
        }
    }
        ...
        for ($rowno = 0; $rowno < $qstats->getRowCount(); $rowno++) {
            ...
            foreach ($slotqstats as $statname => $slotqstat) {
                if (!in_array($statname, array('slot', 'subqname')) && $slotqstat !== '') {
                    $this->assert_stat_equals($slotqstat, $questionstats, $slotqstats['slot'], $slotqstats['subqname'], $slotqstats['variant'], $statname);
                }
            }
            // Check that sub-question boolean field is correctly set.
            $this->assert_stat_equals(!empty($slotqstats['subqname']), $questionstats, $slotqstats['slot'], $slotqstats['subqname'], $slotqstats['variant'], 'subquestion');
        }
    }
    ...
    protected function assert_response_count_equals($question, $qubaids, $expected, $whichtries) {
        ...
        list($subpartid, $responseclassid) = $this->get_response_subpart_and_class_id($question, $subpart, $expected['modelresponse']);
            ...
            if ($actualresponsecount->response == $expected['actualresponse'] || count($actualresponsecounts) == 1) {
                ...
                while (isset($expected['count' . $i])) {
                    if ($expected['count' . $i] != 0) {
                        $this->assertTrue(isset($actualresponsecount->trycount[$i]), "There is no count at all for try $i on " . $partofanalysis);
                ...
                if (isset($expected['totalcount'])) {
                    $this->assertEquals($expected['totalcount'], $actualresponsecount->totalcount, "Total count on " . $partofanalysis);
                }
                return;
            }
        }
     ...
     * @param $qubaids
     */
    protected function check_variants_count_for_cquiz_00($questions, $questionstats, $whichtries, $qubaids) {
     ...
     * @param $cquizstats
     */
    protected function check_cquiz_stats_for_cquiz_00($cquizstats) {
        $cquizstatsexpected = array(
            'median' => 4.5,
        ...
        foreach ($cquizstatsexpected as $statname => $statvalue) {
            $this->assertEquals($statvalue, $cquizstats->$statname, $cquizstats->$statname, abs($statvalue) * 1.5e-5);
        }
    }
     ...
     * @return array with contents 0 => $questions, 1 => $cquizstats, 2=> $questionstats, 3=> $qubaids Might be needed for further
     *               testing.
     */
    protected function check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries, $groupstudents) {
        $this->report = new cquiz_statistics_report();
        $questions = $this->report->load_and_initialise_questions_for_calculations($this->cquiz);
        list($cquizstats, $questionstats) = $this->report->get_all_stats_and_analysis($this->cquiz, $whichattempts, $whichtries, $groupstudents, $questions);

        $qubaids = cquiz_statistics_qubaids_condition($this->cquiz->id, $groupstudents, $whichattempts);

        // We will create some cquiz and question stat calculator instances and some response analyser instances, just in order
        // to check the last analysed time then returned.
        $cquizcalc = new \cquiz_statistics\calculator();
        // Should not be a delay of more than one second between the calculation of stats above and here.
        $this->assertTimeCurrent($cquizcalc->get_last_calculated_time($qubaids));
        ...
        if (isset($csvdata['qstats'])) {
            ...
            return array($questions, $cquizstats, $questionstats, $qubaids);
        }
        return array($questions, $cquizstats, $questionstats, $qubaids);
    }
