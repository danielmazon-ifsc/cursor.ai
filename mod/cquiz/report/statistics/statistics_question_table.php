...
/**
 * Cquiz statistics report, table for showing response analysis for a particular question (or sub question).
 *
 ...
 * @author     Ricardo Drummond
 */
class cquiz_statistics_question_table extends flexible_table {
    ...
    public function __construct($qid) {
        parent::__construct('mod-cquiz-report-statistics-question-table' . $qid);
    }
    ...
    public function question_setup($reporturl, $questiondata, $s, $responseanalysis) {
        ...
        $this->set_attribute('class', 'generaltable generalbox boxaligncenter cquizresponseanalysis');
        ...
        if ($responseanalysis->has_subparts()) {
            $columns[] = 'part';
            $headers[] = get_string('partofquestion', 'cquiz_statistics');
        }

        if ($responseanalysis->has_multiple_response_classes()) {
            $columns[] = 'responseclass';
            $headers[] = get_string('modelresponse', 'cquiz_statistics');

            if ($responseanalysis->has_actual_responses()) {
                $columns[] = 'response';
                $headers[] = get_string('actualresponse', 'cquiz_statistics');
            }
        } else {
            $columns[] = 'response';
            $headers[] = get_string('response', 'cquiz_statistics');
        }

        $columns[] = 'fraction';
        $headers[] = get_string('optiongrade', 'cquiz_statistics');

        if (!$responseanalysis->has_multiple_tries_data()) {
            $columns[] = 'totalcount';
            $headers[] = get_string('count', 'cquiz_statistics');
        } else {
            $countcolumns = range(1, $responseanalysis->get_maximum_tries());
            foreach ($countcolumns as $countcolumn) {
                $columns[] = 'trycount' . $countcolumn;
                $headers[] = get_string('counttryno', 'cquiz_statistics', $countcolumn);
            }
        }

        $columns[] = 'frequency';
        $headers[] = get_string('frequency', 'cquiz_statistics');
