...
/**
 * Cquiz statistics report, table for showing statistics of each question in the cquiz.
 *
...
/**
 * This table has one row for each question in the cquiz, with sub-rows when
 ...
 * @author     Ricardo Drummond
 */
class cquiz_statistics_table extends flexible_table {

    /** @var object the cquiz settings. */
    protected $cquiz;

    /** @var integer the cquiz course_module id. */
    protected $cmid;
    ...
    public function __construct() {
        parent::__construct('mod-cquiz-report-statistics-report');
    }
     ...
     * @param int $cmid the cquiz course_module id
     * @param moodle_url $reporturl the URL to redisplay this report.
     * @param int $s number of attempts included in the statistics.
     */
    public function statistics_setup($cquiz, $cmid, $reporturl, $s) {
        $this->cquiz = $cquiz;
        $this->cmid = $cmid;
        ...
        $columns[] = 'number';
        $headers[] = get_string('questionnumber', 'cquiz_statistics');
        ...
        if (!$this->is_downloading()) {
            ...
            $columns[] = 'qtype';
            $headers[] = get_string('questiontype', 'cquiz_statistics');
        }

        $columns[] = 'name';
        $headers[] = get_string('questionname', 'cquiz');

        $columns[] = 's';
        $headers[] = get_string('attempts', 'cquiz_statistics');

        if ($s > 1) {
            $columns[] = 'facility';
            $headers[] = get_string('facility', 'cquiz_statistics');

            $columns[] = 'sd';
            $headers[] = get_string('standarddeviationq', 'cquiz_statistics');
        }

        $columns[] = 'random_guess_score';
        $headers[] = get_string('random_guess_score', 'cquiz_statistics');

        $columns[] = 'intended_weight';
        $headers[] = get_string('intended_weight', 'cquiz_statistics');

        $columns[] = 'effective_weight';
        $headers[] = get_string('effective_weight', 'cquiz_statistics');

        $columns[] = 'discrimination_index';
        $headers[] = get_string('discrimination_index', 'cquiz_statistics');

        $columns[] = 'discriminative_efficiency';
        $headers[] = get_string('discriminative_efficiency', 'cquiz_statistics');
    ...
    protected function col_actions($questionstat) {
        return cquiz_question_action_icons($this->cquiz, $this->cmid, $questionstat->question, $this->baseurl, $questionstat->variant);
    }
    ...
    protected function col_name($questionstat) {
        ...
        if (!is_null($questionstat->variant)) {
            ...
            $name = get_string('nameforvariant', 'cquiz_statistics', $a);
        }
        ...
        if (!is_null($questionstat->variant)) {
            if ($questionstat->subquestion) {
                ...
                $name = html_writer::link($url, $name, array('title' => get_string('detailedanalysisforvariant', 'cquiz_statistics', $questionstat->variant)));
            ...
            if ($questionstat->subquestion && !$questionstat->get_variants()) {
                ...
                $name = html_writer::link($url, $name, array('title' => get_string('detailedanalysis', 'cquiz_statistics')));
                ...
                if ($questionstat->get_variants() || $questionstat->get_sub_question_ids()) {
                    ...
                    $name = html_writer::link($url, $name, array('title' => get_string('slotstructureanalysis', 'cquiz_statistics', $number)));
                } else {
        ...
        if (!empty($questionstat->minmedianmaxnotice)) {
            $name = get_string($questionstat->minmedianmaxnotice, 'cquiz_statistics') . '<br />' . $name;
        }
     ...
     * of maximum mark for the cquiz. That is, the indended influence this question
     * on the student's overall mark.
     * @param \core_question\statistics\questions\calculated $questionstat stats for the question.
     * @return string contents of this table cell.
     */
    protected function col_intended_weight($questionstat) {
        return cquiz_report_scale_summarks_as_percentage($questionstat->maxmark, $this->cquiz);
    }
    ...
    protected function col_effective_weight($questionstat) {
        ...
        if ($questionstat->negcovar) {
            $negcovar = get_string('negcovar', 'cquiz_statistics');

            if (!$this->is_downloading()) {
                $negcovar = html_writer::tag('div', $negcovar . $OUTPUT->help_icon('negcovar', 'cquiz_statistics'), array('class' => 'negcovar'));
            }
