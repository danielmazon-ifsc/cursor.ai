...
if (empty($currentgroup)) {
    ...
    $groupstudents = get_users_by_capability($modcontext, array('mod/cquiz:reviewmyattempts', 'mod/cquiz:attempt'), '', '', '', '', $currentgroup, '', false);
}
$qubaids = cquiz_statistics_qubaids_condition($cquizid, $groupstudents, $whichattempts);
...
foreach (array_keys($fieldstoplot) as $fieldtoplot) {
    ...
    $graph->y_format[$fieldtoplot] = array(
        'colour' => cquiz_statistics_graph_get_new_colour(),
        'bar' => 'fill',
        'shadow_offset' => 1,
