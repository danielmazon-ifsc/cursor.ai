...
/**
 * Base class for the table used by a {@link cquiz_attempts_report}.
 *
 * @package   mod_cquiz
 ...
 * @author     Ricardo Drummond
 */
abstract class cquiz_attempts_report_table extends table_sql {
    ...
    /** @var object the cquiz settings for the cquiz we are reporting on. */
    protected $cquiz;
    ...
    /** @var object mod_cquiz_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    /** @var object the questions that comprise this cquiz.. */
    protected $questions;
     ...
     * @param mod_cquiz_attempts_report_options $options
     * @param array $groupstudents
     * @param array $students
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($uniqueid, $cquiz, $context, $qmsubselect, mod_cquiz_attempts_report_options $options, $groupstudents, $students, $questions, $reporturl) {
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                        new moodle_url('/mod/cquiz/review.php', array('attempt' => $attempt->attempt)), get_string('reviewattempt', 'cquiz'), array('class' => 'reviewlink'));
    }
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return cquiz_attempt::state_name($attempt->state);
        } else {
            return '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = cquiz_report_feedback_for_grade(
                cquiz_rescale_grade($attempt->sumgrades, $this->cquiz, false), $this->cquiz->id, $this->context);
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        if ($this->is_flagged($attempt->usageid, $slot)) {
            $flag = $OUTPUT->pix_icon('i/flagged', get_string('flagged', 'question'), 'moodle', array('class' => 'questionflag'));
        }
        ...
        $output = html_writer::tag('span', $feedbackimg . html_writer::tag('span', $data, array('class' => $state->get_state_class(true))) . $flag, array('class' => 'que'));
        ...
        $url = new moodle_url('/mod/cquiz/reviewquestion.php', $reviewparams);
        $output = $OUTPUT->action_link($url, $output, new popup_action('click', $url, 'reviewquestion', array('height' => 450, 'width' => 650)), array('title' => get_string('reviewresponse', 'cquiz')));
    ...
    protected function icon_for_fraction($fraction) {
        ...
        return $OUTPUT->pix_icon('i/grade_' . $feedbackclass, get_string($feedbackclass, 'question'), 'moodle', array('class' => 'icon'));
    }
    ...
    public function base_sql($reportstudents) {
        ...
        $fields = $DB->sql_concat('u.id', "'#'", 'COALESCE(cquiza.attempt, 0)') . ' AS uniqueid,';

        if ($this->qmsubselect) {
            $fields .= "\n(CASE WHEN $this->qmsubselect THEN 1 ELSE 0 END) AS gradedattempt,";
        }

        $extrafields = get_extra_user_fields_sql($this->context, 'u', '', array('id', 'idnumber', 'firstname', 'lastname', 'picture',
            'imagealt', 'institution', 'department', 'email'));
        $allnames = get_all_user_name_fields(true, 'u');
        $fields .= '
                cquiza.uniqueid AS usageid,
                cquiza.id AS attempt,
                u.id AS userid,
                u.idnumber, ' . $allnames . ',
                u.picture,
                u.imagealt,
                ...
                cquiza.timefinish,
                cquiza.timestart,
                CASE WHEN cquiza.timefinish = 0 THEN null
                     WHEN cquiza.timefinish > cquiza.timestart THEN cquiza.timefinish - cquiza.timestart
                     ELSE 0 END AS duration';
        // To explain that last bit, timefinish can be non-zero and less
        // than timestart when you have two load-balanced servers with very
        // badly synchronised clocks, and a student does a really quick attempt.
        // This part is the same for all cases. Join the users and cquiz_attempts tables.
        $from = "\n{user} u";
        $from .= "\nLEFT JOIN {cquiz_attempts} cquiza ON
                                    cquiza.userid = u.id AND cquiza.cquiz = :cquizid";
        $params = array('cquizid' => $this->cquiz->id);

        if ($this->qmsubselect && $this->options->onlygraded) {
            $from .= " AND (cquiza.state <> :finishedstate OR $this->qmsubselect)";
            $params['finishedstate'] = cquiz_attempt::FINISHED;
        }

        switch ($this->options->attempts) {
            case cquiz_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'cquiza.id IS NOT NULL AND cquiza.preview = 0';
                break;
            case cquiz_attempts_report::ENROLLED_WITH:
                // Show only students with attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND cquiza.preview = 0 AND cquiza.id IS NOT NULL";
                break;
            case cquiz_attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND cquiza.id IS NULL";
                break;
            case cquiz_attempts_report::ENROLLED_ALL:
                // Show all students with or without attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND (cquiza.preview = 0 OR cquiza.preview IS NULL)";
                break;
        }

        if ($this->options->states) {
            list($statesql, $stateparams) = $DB->get_in_or_equal($this->options->states, SQL_PARAMS_NAMED, 'state');
            $params += $stateparams;
            $where .= " AND (cquiza.state $statesql OR cquiza.state IS NULL)";
        }
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{cquiz_attempts} {$alias}cquiza", "{$alias}cquiza.uniqueid", "{$alias}cquiza.cquiz = :{$alias}cquizid", array("{$alias}cquizid" => $this->sql->params['cquizid']));
        ...
        $this->sql->from .= "\nLEFT JOIN $inlineview ON " .
                "$alias.questionusageid = cquiza.uniqueid AND $alias.slot = :{$alias}slot";
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join($this->sql->from, 'cquiza.uniqueid', $this->sql->where, $this->sql->params);
        }
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['cquiza.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    public function wrap_html_finish() {
        ...
        get_string('selectall', 'cquiz') . '</a> / ';
        echo '<a href="javascript:deselect_all_in(\'DIV\', null, \'tablecontainer\');">' .
        get_string('selectnone', 'cquiz') . '</a> ';
        echo '&nbsp;&nbsp;';
        $this->submit_buttons();
        echo '</div>';
    ...
    protected function submit_buttons() {
        global $PAGE;
        if (has_capability('mod/cquiz:deleteattempts', $this->context)) {
            echo '<input type="submit" id="deleteattemptsbutton" name="delete" value="' .
            get_string('deleteselected', 'cquiz_overview') . '"/>';
            $PAGE->requires->event_handler('#deleteattemptsbutton', 'click', 'M.util.show_confirm_dialog', array('message' => get_string('deleteattemptcheck', 'cquiz')));
        }
    }
