...
/**
 * Base class for cquiz reports that are basically a table with one row for each attempt.
 *
 * @copyright 2017 Viddia (http://viddia.com.br)
 * @author     Ricardo Drummond
 */
abstract class cquiz_attempts_report extends cquiz_default_report {
    ...
    /** @var object the cquiz context. */
    protected $context;

    /** @var mod_cquiz_attempts_report_form The settings form to use. */
    protected $form;
     ...
     * @param object $course
     */
    protected function init($mode, $formclass, $cquiz, $cm, $course) {
        $this->mode = $mode;
        ...
        $this->qmsubselect = cquiz_report_qm_filter_select($cquiz);

        $this->form = new $formclass($this->get_base_url(), array('cquiz' => $cquiz, 'currentgroup' => $currentgroup, 'context' => $this->context));
    ...
    protected function get_base_url() {
        return new moodle_url('/mod/cquiz/report.php', array('id' => $this->context->instanceid, 'mode' => $this->mode));
    }
    ...
    protected function load_relevant_students($cm, $course = null) {
        ...
        if (!$students = get_users_by_capability($this->context, array('mod/cquiz:reviewmyattempts', 'mod/cquiz:attempt'), 'u.id, 1', '', '', '', '', '', false)) {
            $students = array();
        } else {
        ...
        if (!$groupstudents = get_users_by_capability($this->context, array('mod/cquiz:reviewmyattempts', 'mod/cquiz:attempt'), 'u.id, 1', '', '', '', $currentgroup, '', false)) {
            $groupstudents = array();
        } else {
    ...
    protected function add_state_column(&$columns, &$headers) {
        $columns[] = 'state';
        $headers[] = get_string('attemptstate', 'cquiz');
    }
    ...
    protected function add_time_columns(&$columns, &$headers) {
        $columns[] = 'timestart';
        $headers[] = get_string('startedon', 'cquiz');

        $columns[] = 'timefinish';
        $headers[] = get_string('timecompleted', 'cquiz');

        $columns[] = 'duration';
        $headers[] = get_string('attemptduration', 'cquiz');
    }
     ...
     * @param bool $usercanseegrades whether the user is allowed to see grades for this cquiz.
     * @param array $columns the list of columns. Added to.
     * @param array $headers the columns headings. Added to.
     * @param bool $includefeedback whether to include the feedbacktext columns
     */
    protected function add_grade_columns($cquiz, $usercanseegrades, &$columns, &$headers, $includefeedback = true) {
        if ($usercanseegrades) {
            $columns[] = 'sumgrades';
            $headers[] = get_string('grade', 'cquiz') . '/' .
                    cquiz_format_grade($cquiz, $cquiz->grade);
        }

        if ($includefeedback && cquiz_has_feedback($cquiz)) {
            $columns[] = 'feedbacktext';
            $headers[] = get_string('feedback', 'cquiz');
        }
    }
     ...
     * @param mod_cquiz_attempts_report_options $options the display options.
     * @param bool $collapsible whether to allow columns in the report to be collapsed.
     */
    protected function set_up_table_columns($table, $columns, $headers, $reporturl, mod_cquiz_attempts_report_options $options, $collapsible) {
        ...
        if (empty($currentgroup) || $groupstudents) {
            if (optional_param('delete', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    require_capability('mod/cquiz:deleteattempts', $this->context);
                    $this->delete_selected_attempts($cquiz, $cm, $attemptids, $allowed);
    ...
    /**
     * Delete the cquiz attempts
     * @param object $cquiz the cquiz settings. Attempts that don't belong to
     * this cquiz are not deleted.
     ...
     */
    protected function delete_selected_attempts($cquiz, $cm, $attemptids, $allowed) {
        global $DB;

        foreach ($attemptids as $attemptid) {
            $attempt = $DB->get_record('cquiz_attempts', array('id' => $attemptid));
            if (!$attempt || $attempt->cquiz != $cquiz->id || $attempt->preview != 0) {
                // Ensure the attempt exists, and belongs to this cquiz. If not skip.
                continue;
            }
            ...
            // Set the course module id before calling cquiz_delete_attempt().
            $cquiz->cmid = $cm->id;
