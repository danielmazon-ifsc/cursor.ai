...
/**
 * Are there any questions in this cquiz?
 * @param int $cquizid the cquiz id.
 */
function cquiz_has_questions($cquizid) {
    global $DB;
    return $DB->record_exists('cquiz_slots', array('cquizid' => $cquizid));
}
...
function cquiz_report_get_significant_questions($cquiz) {
    ...
    $qsbyslot = $DB->get_records_sql("
            SELECT slot.slot,
              ...
              FROM {question} q
              JOIN {cquiz_slots} slot ON slot.questionid = q.id
...
/**
 * This is a wrapper for {@link cquiz_report_grade_method_sql} that takes the whole cquiz object instead of just the grading method
 * as a param. See definition for {@link cquiz_report_grade_method_sql} below.
 *
 * @param object $cquiz
 * @param string $cquizattemptsalias sql alias for 'cquiz_attempts' table
...
function cquiz_report_qm_filter_select($cquiz, $cquizattemptsalias = 'cquiza') {
    ...
    return cquiz_report_grade_method_sql($cquiz->grademethod, $cquizattemptsalias);
}

/**
 * Given a cquiz grading method return sql to test if this is an
 * attempt that will be contribute towards the grade of the user. Or return an
 * empty string if the grading method is CQUIZ_GRADEAVERAGE and thus all attempts
 * contribute to final grade.
 *
 * @param string $grademethod cquiz grading method.
 * @param string $cquizattemptsalias sql alias for 'cquiz_attempts' table
 * @return string sql to test if this is an attempt that will contribute towards the graded of the user
 */
function cquiz_report_grade_method_sql($grademethod, $cquizattemptsalias = 'cquiza') {
    switch ($grademethod) {
        case CQUIZ_GRADEHIGHEST :
            return "($cquizattemptsalias.state = 'finished' AND NOT EXISTS (
                           SELECT 1 FROM {cquiz_attempts} qa2
                            WHERE qa2.cquiz = $cquizattemptsalias.cquiz AND
                                qa2.userid = $cquizattemptsalias.userid AND
                                 qa2.state = 'finished' AND (
                COALESCE(qa2.sumgrades, 0) > COALESCE($cquizattemptsalias.sumgrades, 0) OR
               (COALESCE(qa2.sumgrades, 0) = COALESCE($cquizattemptsalias.sumgrades, 0) AND qa2.attempt < $cquizattemptsalias.attempt)
                                )))";
            ...
            return "($cquizattemptsalias.state = 'finished' AND NOT EXISTS (
                           SELECT 1 FROM {cquiz_attempts} qa2
                            WHERE qa2.cquiz = $cquizattemptsalias.cquiz AND
                                qa2.userid = $cquizattemptsalias.userid AND
                                 qa2.state = 'finished' AND
                               qa2.attempt < $cquizattemptsalias.attempt))";
...
/**
 * Get the number of students whose score was in a particular band for this cquiz.
...
function cquiz_report_highlighting_grading_method($cquiz, $qmsubselect, $qmfilter) {
    if ($cquiz->attempts == 1) {
        ...
        return '<p>' . get_string('showinggradedandungraded', 'cquiz_overview', '<span class="gradedattempt">' . cquiz_get_grading_option_name($cquiz->grademethod) .
                        '</span>') . '</p>';
    }
}
...
function cquiz_report_feedback_for_grade($grade, $cquizid, $context) {
    ...
    $feedbacktext = file_rewrite_pluginfile_urls($feedbacktext, 'pluginfile.php', $context->id, 'mod_cquiz', 'feedback', $feedbackid);
...
function cquiz_report_list($context) {
    ...
    $reportdirs = core_component::get_plugin_list('cquiz');
...
function cquiz_no_questions_message($cquiz, $cm, $context) {
    ...
    if (has_capability('mod/cquiz:manage', $context)) {
        $output .= $OUTPUT->single_button(new moodle_url('/mod/cquiz/edit.php', array('cmid' => $cm->id)), get_string('editcquiz', 'cquiz'), 'get');
    }
