...
/**
 * This file defines the cquiz manual grading report class.
 *
...
require_once($CFG->dirroot . '/mod/cquiz/report/grading/gradingsettings_form.php');

/**
 * Cquiz report to help teachers manually grade questions that need it.
 *
 ...
 * @author     Ricardo Drummond
 */
class cquiz_grading_report extends cquiz_default_report {
    ...
    public function display($cquiz, $cm, $course) {
        ...
        require_capability('mod/cquiz:grade', $this->context);
        $shownames = has_capability('cquiz/grading:viewstudentnames', $this->context);
        $showidnumbers = has_capability('cquiz/grading:viewidnumber', $this->context);
        ...
        // Get the list of questions in this cquiz.
        $this->questions = cquiz_report_get_significant_questions($cquiz);
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'cquiz_grading');
        }
        ...
        if ($this->currentgroup == self::NO_GROUPS_ALLOWED) {
            ...
            $this->users = get_users_by_capability($this->context, array('mod/cquiz:reviewmyattempts', 'mod/cquiz:attempt'), '', '', '', '', $this->currentgroup, '', false);
        }
        ...
        if ($slot && $hasquestions) {
            ...
            if (!$counts || $counts->$grade == 0) {
                redirect($this->list_questions_url(), get_string('alldoneredirecting', 'cquiz_grading'));
            }
        }

        // Start output.
        $this->print_header_and_tabs($cm, $course, $cquiz, 'grading');
    ...
    protected function get_qubaids_condition() {
        ...
        $where = "cquiza.cquiz = :mangrcquizid AND
                cquiza.preview = 0 AND
                cquiza.state = :statefinished";
        $params = array('mangrcquizid' => $this->cm->instance, 'statefinished' => cquiz_attempt::FINISHED);

        $currentgroup = groups_get_activity_group($this->cm, true);
        if ($currentgroup) {
            $users = get_users_by_capability($this->context, array('mod/cquiz:reviewmyattempts', 'mod/cquiz:attempt'), 'u.id, u.id', '', '', '', $currentgroup, '', false);
            if (empty($users)) {
                $where .= ' AND cquiza.userid = 0';
            } else {
        ...
        return new qubaid_join('{cquiz_attempts} cquiza', 'cquiza.uniqueid', $where, $params);
    }
    ...
    protected function load_attempts_by_usage_ids($qubaids) {
        ...
        $params[] = cquiz_attempt::FINISHED;
        $params[] = $this->cquiz->id;

        $fields = 'cquiza.*, u.idnumber, ';
        $fields .= get_all_user_name_fields(true, 'u');
        $attemptsbyid = $DB->get_records_sql("
                SELECT $fields
                FROM {cquiz_attempts} cquiza
                JOIN {user} u ON u.id = cquiza.userid
                WHERE cquiza.uniqueid $asql AND cquiza.state = ? AND cquiza.cquiz = ?", $params);
    ...
    protected function base_url() {
        return new moodle_url('/mod/cquiz/report.php', array('id' => $this->cm->id, 'mode' => 'grading'));
    }
    ...
    protected function format_count_for_table($counts, $type, $gradestring) {
        ...
        if ($counts->$type > 0) {
            $result .= ' ' . html_writer::link($this->grade_question_url(
                                    $counts->slot, $counts->questionid, $type), get_string($gradestring, 'cquiz_grading'), array('class' => 'gradetheselink'));
        }
        return $result;
    }
    ...
    protected function display_index($includeauto) {
        ...
        echo $OUTPUT->heading(get_string('questionsthatneedgrading', 'cquiz_grading'), 3);
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'cquiz_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'cquiz_grading');
        }
        echo html_writer::tag('p', html_writer::link($this->list_questions_url(!$includeauto), $linktext), array('class' => 'toggleincludeauto'));
        ...
        if (empty($data)) {
            echo $OUTPUT->notification(get_string('nothingfound', 'cquiz_grading'));
            return;
        }
        ...
        $table->head[] = get_string('qno', 'cquiz_grading');
        $table->head[] = get_string('questionname', 'cquiz_grading');
        $table->head[] = get_string('tograde', 'cquiz_grading');
        $table->head[] = get_string('alreadygraded', 'cquiz_grading');
        if ($includeauto) {
            $table->head[] = get_string('automaticallygraded', 'cquiz_grading');
        }
        $table->head[] = get_string('total', 'cquiz_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade, $pagesize, $page, $shownames, $showidnumbers, $order, $counts) {
        ...
        $mform = new cquiz_grading_settings_form($hidden, $counts, $shownames, $showidnumbers);
        ...
        echo $OUTPUT->heading(get_string('gradingquestionx', 'cquiz_grading', $a), 3);
        echo html_writer::tag('p', html_writer::link($this->list_questions_url(), get_string('backtothelistofquestions', 'cquiz_grading')), array('class' => 'mdl-align'));
        ...
        $a->of = $count;
        echo $OUTPUT->heading(get_string('gradingattemptsxtoyofz', 'cquiz_grading', $a), 3);
        ...
        foreach ($qubaids as $qubaid) {
            ...
            $displayoptions = cquiz_get_review_options($this->cquiz, $attempt, $this->context);
        ...
        echo html_writer::tag('div', html_writer::empty_tag('input', array(
                    'type' => 'submit', 'value' => get_string('saveandnext', 'cquiz_grading'))), array('class' => 'mdl-align')) .
    ...
    protected function get_question_heading($attempt, $shownames, $showidnumbers) {
        ...
        if ($shownames && $showidnumbers) {
            return get_string('gradingattemptwithidnumber', 'cquiz_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'cquiz_grading', $a);
    ...
    protected function process_submitted_data() {
        ...
        foreach ($qubaids as $qubaid) {
            $attempt = $attempts[$qubaid];
            $attemptobj = new cquiz_attempt($attempt, $this->cquiz, $this->cm, $this->course);
            ...
            $events[] = \mod_cquiz\event\question_manually_graded::create($params);
        }
    ...
    protected function get_usage_ids_where_question_in_state($summarystate, $slot, $questionid = null, $orderby = 'random', $page = 0, $pagesize = null) {
        ...
        if ($orderby == 'date') {
            ...
            $qubaids->from .= " JOIN {user} u ON cquiza.userid = u.id ";
