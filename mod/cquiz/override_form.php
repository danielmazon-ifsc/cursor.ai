...
require_once($CFG->dirroot . '/mod/cquiz/mod_form.php');
    ...
    protected function definition() {
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid', get_string('overridegroup', 'cquiz'), $groupchoices);
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid', get_string('overrideuser', 'cquiz'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                $users = get_users_by_capability($this->context, 'mod/cquiz:attempt', 'u.id, u.email, ' . get_all_user_name_fields(true, 'u'), $sort, '', '', '', '', false, true);
                ...
                $mform->addElement('searchableselector', 'userid', get_string('overrideuser', 'cquiz'), $userchoices);
        ...
        $mform->addElement('date_time_selector', 'timeopen', get_string('cquizopen', 'cquiz'), mod_cquiz_mod_form::$datefieldoptions);
        $mform->setDefault('timeopen', $this->cquiz->timeopen);

        $mform->addElement('date_time_selector', 'timeclose', get_string('cquizclose', 'cquiz'), mod_cquiz_mod_form::$datefieldoptions);
        ...
        $mform->addElement('select', 'attempts', get_string('attemptsallowed', 'cquiz'), $attemptoptions);
        $mform->setDefault('attempts', $this->cquiz->attempts);

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton', get_string('reverttodefaults', 'cquiz'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton', get_string('save', 'cquiz'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton', get_string('saveoverrideandstay', 'cquiz'));
