...
class mod_cquiz_mod_form extends moodleform_mod {
    ...
    /** @var array options to be used with date_time_selector fields in the cquiz. */
    ...
    /** @var int the max number of attempts allowed in any user or group override on this cquiz. */
    ...
    protected function definition() {
        ...
        // Introduction.
        $this->standard_intro_elements(get_string('introduction', 'cquiz'));

        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'timing', get_string('timing', 'cquiz'));

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen', get_string('cquizopen', 'cquiz'), self::$datefieldoptions);
        $mform->addHelpButton('timeopen', 'cquizopenclose', 'cquiz');

        $mform->addElement('date_time_selector', 'timeclose', get_string('cquizclose', 'cquiz'), self::$datefieldoptions);

        // Time limit.
        $mform->addElement('duration', 'timelimit', get_string('timelimit', 'cquiz'), array('optional' => true));
        ...
        $mform->addElement('select', 'overduehandling', get_string('overduehandling', 'cquiz'), cquiz_get_overdue_handling_options());
        ...
        // Grace period time.
        $mform->addElement('duration', 'graceperiod', get_string('graceperiod', 'cquiz'), array('optional' => true));
        ...
        if (property_exists($this->current, 'grade')) {
            ...
            $currentgrade = $cquizconfig->maximumgrade;
        }
...
//            $attemptoptions[$i] = $i;
//        }
        $mform->addElement('text', 'attempts', get_string('attemptsallowed', 'cquiz'), 'maxlength="2" size="2"');
        $mform->setType('attempts', PARAM_INT);
        $mform->addHelpButton('attempts', 'attempts', 'cquiz');
        $mform->setAdvanced('attempts', $cquizconfig->attempts_adv);
        $mform->setDefault('attempts', $cquizconfig->attempts);

        // Grading method.
        $mform->addElement('select', 'grademethod', get_string('grademethod', 'cquiz'), cquiz_get_grading_options());
        ...
        $pagegroup = array();
        $pagegroup[] = $mform->createElement('select', 'questionsperpage', get_string('newpage', 'cquiz'), cquiz_questions_per_page_options(), array('id' => 'id_questionsperpage'));
        $mform->setDefault('questionsperpage', $cquizconfig->questionsperpage);

        if (!empty($this->_cm)) {
            $pagegroup[] = $mform->createElement('checkbox', 'repaginatenow', '', get_string('repaginatenow', 'cquiz'), array('id' => 'id_repaginatenow'));
        }

        $mform->addGroup($pagegroup, 'questionsperpagegrp', get_string('newpage', 'cquiz'), null, false);
        ...
        // Navigation method.
        $mform->addElement('select', 'navmethod', get_string('navmethod', 'cquiz'), cquiz_get_navigation_options());
        ...
        $redochoices = array(0 => get_string('no'), 1 => get_string('canredoquestionsyes', 'cquiz'));
        $mform->addElement('select', 'canredoquestions', get_string('canredoquestions', 'cquiz'), $redochoices);
        $mform->addHelpButton('canredoquestions', 'canredoquestions', 'cquiz');
        $mform->setAdvanced('canredoquestions', $cquizconfig->canredoquestions_adv);
        $mform->setDefault('canredoquestions', $cquizconfig->canredoquestions);
        ...
        $mform->addElement('selectyesno', 'attemptonlast', get_string('eachattemptbuildsonthelast', 'cquiz'));
        ...
        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'reviewoptionshdr', get_string('reviewoptionsheading', 'cquiz'));
        $mform->addHelpButton('reviewoptionshdr', 'reviewoptionsheading', 'cquiz');

        // Review options.
        $this->add_review_options_group($mform, $cquizconfig, 'during', mod_cquiz_display_options::DURING, true);
        $this->add_review_options_group($mform, $cquizconfig, 'immediately', mod_cquiz_display_options::IMMEDIATELY_AFTER);
        $this->add_review_options_group($mform, $cquizconfig, 'open', mod_cquiz_display_options::LATER_WHILE_OPEN);
        $this->add_review_options_group($mform, $cquizconfig, 'closed', mod_cquiz_display_options::AFTER_CLOSE);
        ...
        foreach ($modinfo->cms as $cm) {
            if ($cm->visible && strpos($cm->modname, 'forum') !== false) {
                $forums[$cm->name] = $cm->name;
            }
        }
        $mform->addElement('select', 'helpforum', get_string('helpforum', 'cquiz'), $forums);
        $mform->addHelpButton('helpforum', 'helpforum', 'cquiz');

        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'display', get_string('appearance'));

        // Show user picture.
        $mform->addElement('select', 'showuserpicture', get_string('showuserpicture', 'cquiz'), cquiz_get_user_image_options());
        ...
        $mform->addElement('select', 'decimalpoints', get_string('decimalplaces', 'cquiz'), $options);
        ...
        $mform->addElement('select', 'questiondecimalpoints', get_string('decimalplacesquestion', 'cquiz'), $options);
        ...
        $mform->addElement('duration', 'delay1', get_string('delay1st2nd', 'cquiz'), array('optional' => true));
        ...
        $mform->addElement('duration', 'delay2', get_string('delaylater', 'cquiz'), array('optional' => true));
        ...
        $mform->addElement('duration', 'attemptsmaxdelay', get_string('attemptsmaxdelay', 'cquiz'), array('optional' => true));
        $mform->addHelpButton('attemptsmaxdelay', 'attemptsmaxdelay', 'cquiz');
        if ($this->get_max_attempts_for_any_override() < 2) {
            $mform->disabledIf('attemptsmaxdelay', 'attempts', 'eq', 1);
        }

        // Browser security choices.
        $mform->addElement('select', 'browsersecurity', get_string('browsersecurity', 'cquiz'), cquiz_access_manager::get_browser_security_choices());
        ...
        if ($needwarning) {
            $mform->addElement('static', 'nogradewarning', '', get_string('nogradewarning', 'cquiz'));
        }

        $mform->addElement('static', 'gradeboundarystatic1', get_string('gradeboundary', 'cquiz'), '100%');

        $repeatarray = array();
        $repeatedoptions = array();
        $repeatarray[] = $mform->createElement('editor', 'feedbacktext', get_string('feedback', 'cquiz'), array('rows' => 3), array('maxfiles' => EDITOR_UNLIMITED_FILES,
            'noclean' => true, 'context' => $this->context));
        $repeatarray[] = $mform->createElement('text', 'feedbackboundaries', get_string('gradeboundary', 'cquiz'), array('size' => 10));
        $repeatedoptions['feedbacktext']['type'] = PARAM_RAW;
        $repeatedoptions['feedbackboundaries']['type'] = PARAM_RAW;

        if (!empty($this->_instance)) {
            $this->_feedbacks = $DB->get_records('cquiz_feedback', array('cquizid' => $this->_instance), 'mingrade DESC');
            $numfeedbacks = count($this->_feedbacks);
        } else {
            $this->_feedbacks = array();
            $numfeedbacks = $cquizconfig->initialnumfeedbacks;
        }
        $numfeedbacks = max($numfeedbacks, 1);

        $nextel = $this->repeat_elements($repeatarray, $numfeedbacks - 1, $repeatedoptions, 'boundary_repeats', 'boundary_add_fields', 3, get_string('addmoreoverallfeedbacks', 'cquiz'), true);

        // Put some extra elements in before the button.
        $mform->insertElementBefore($mform->createElement('editor', "feedbacktext[$nextel]", get_string('feedback', 'cquiz'), array('rows' => 3), array('maxfiles' => EDITOR_UNLIMITED_FILES, 'noclean' => true,
                    'context' => $this->context)), 'boundary_add_fields');
        $mform->insertElementBefore($mform->createElement('static', 'gradeboundarystatic2', get_string('gradeboundary', 'cquiz'), '0%'), 'boundary_add_fields');
        ...
        $PAGE->requires->yui_module('moodle-mod_cquiz-modform', 'M.mod_cquiz.modform.init');
    }

    protected function add_review_options_group($mform, $cquizconfig, $whenname, $when, $withhelp = false) {
        global $OUTPUT;
        ...
        $mform->addGroup($group, $whenname . 'optionsgrp', get_string('review' . $whenname, 'cquiz'), null, false);
    ...
    public function data_preprocessing(&$toform) {
        ...
        if (count($this->_feedbacks)) {
            ...
            foreach ($this->_feedbacks as $feedback) {
                ...
                $toform['feedbacktext[' . $key . ']']['text'] = file_prepare_draft_area(
                        ...
                        null, $feedback->feedbacktext // Text.
                );
                ...
                if ($feedback->mingrade > 0) {
                    $toform['feedbackboundaries[' . $key . ']'] = round(100.0 * $feedback->mingrade / $toform['grade'], 6) . '%';
                }
                $key++;
            }
        }
        ...
        $this->preprocessing_review_settings($toform, 'during', mod_cquiz_display_options::DURING);
        $this->preprocessing_review_settings($toform, 'immediately', mod_cquiz_display_options::IMMEDIATELY_AFTER);
        $this->preprocessing_review_settings($toform, 'open', mod_cquiz_display_options::LATER_WHILE_OPEN);
        $this->preprocessing_review_settings($toform, 'closed', mod_cquiz_display_options::AFTER_CLOSE);
    ...
    public function validation($data, $files) {
        ...
        while (!empty($data['feedbackboundaries'][$i])) {
            ...
            if (strlen($boundary) > 0) {
                if ($boundary[strlen($boundary) - 1] == '%') {
                    ...
                    if (is_numeric($boundary)) {
                        ...
                        $errors["feedbackboundaries[$i]"] = get_string('feedbackerrorboundaryformat', 'cquiz', $i + 1);
                    }
            ...
            if (is_numeric($boundary) && $boundary <= 0 || $boundary >= $data['grade']) {
                $errors["feedbackboundaries[$i]"] = get_string('feedbackerrorboundaryoutofrange', 'cquiz', $i + 1);
            }
            if (is_numeric($boundary) && $i > 0 &&
                    $boundary >= $data['feedbackboundaries'][$i - 1]) {
                $errors["feedbackboundaries[$i]"] = get_string('feedbackerrororder', 'cquiz', $i + 1);
            }
        ...
        if (!empty($data['feedbackboundaries'])) {
            for ($i = $numboundaries; $i < count($data['feedbackboundaries']); $i += 1) {
                if (!empty($data['feedbackboundaries'][$i]) &&
                    ...
                    $errors["feedbackboundaries[$i]"] = get_string('feedbackerrorjunkinboundary', 'cquiz', $i + 1);
                }
            }
        }
        for ($i = $numboundaries + 1; $i < count($data['feedbacktext']); $i += 1) {
            if (!empty($data['feedbacktext'][$i]['text']) &&
                ...
                $errors["feedbacktext[$i]"] = get_string('feedbackerrorjunkinfeedback', 'cquiz', $i + 1);
            }
        }
    ...
    public function add_completion_rules() {
        ...
        $group = array();
        $group[] = $mform->createElement('advcheckbox', 'completionpass', null, get_string('completionpass', 'cquiz'), array('group' => 'cpass'));

        $group[] = $mform->createElement('advcheckbox', 'completionattemptsexhausted', null, get_string('completionattemptsexhausted', 'cquiz'), array('group' => 'cattempts'));
        $mform->disabledIf('completionattemptsexhausted', 'completionpass', 'notchecked');
        $mform->addGroup($group, 'completionpassgroup', get_string('completionpass', 'cquiz'), ' &nbsp; ', false);
        $mform->addHelpButton('completionpassgroup', 'completionpass', 'cquiz');
    ...
    public function get_max_attempts_for_any_override() {
        ...
        if (empty($this->_instance)) {
            // Cquiz not created yet, so no overrides.
            return 1;
        }
        ...
        if ($this->maxattemptsanyoverride === null) {
            $this->maxattemptsanyoverride = $DB->get_field_sql("
                    SELECT MAX(CASE WHEN attempts = 0 THEN 1000 ELSE attempts END)
                      FROM {cquiz_overrides}
                     WHERE cquiz = ?", array($this->_instance));
