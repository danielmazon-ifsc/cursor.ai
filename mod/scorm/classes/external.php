...
class mod_scorm_external extends external_api {
    ...
    public static function view_scorm($scormid) {
        ...
        $params = self::validate_parameters(self::view_scorm_parameters(), array(
                    'scormid' => $scormid
        ));
        $warnings = array();
    ...
    public static function get_scorm_attempt_count_parameters() {
        return new external_function_parameters(
            ...
            'ignoremissingcompletion' => new external_value(PARAM_BOOL, 'Ignores attempts that haven\'t reported a grade/completion', VALUE_DEFAULT, false),
                )
        );
    }
    ...
    public static function get_scorm_attempt_count($scormid, $userid, $ignoremissingcompletion = false) {
        ...
        $params = self::validate_parameters(self::get_scorm_attempt_count_parameters(), array('scormid' => $scormid, 'userid' => $userid,
    ...
    public static function get_scorm_scoes($scormid, $organization = '') {
        ...
        $params = self::validate_parameters(self::get_scorm_scoes_parameters(), array('scormid' => $scormid, 'organization' => $organization));
    ...
    public static function get_scorms_by_courses($courseids = array()) {
        ...
        if (!empty($params['courseids'])) {
            ...
            foreach ($scorms as $scorm) {
                ...
                list($module['intro'], $module['introformat']) = external_format_text($scorm->intro, $scorm->introformat, $context->id, 'mod_scorm', 'intro', null);
    ...
    public static function get_scorms_by_courses_returns() {
        ...
        return new external_single_structure(
            ...
            'scorms' => new external_multiple_structure(
                ...
                'version' => new external_value(PARAM_NOTAGS, 'SCORM version (SCORM_12, SCORM_13, SCORM_AICC)', VALUE_OPTIONAL),
                ...
                'forcenewattempt' => new external_value(PARAM_INT, 'Controls re-entry behaviour', VALUE_OPTIONAL),
                'lastattemptlock' => new external_value(PARAM_BOOL, 'Prevents to launch new attempts once finished', VALUE_OPTIONAL),
                'displayattemptstatus' => new external_value(PARAM_INT, 'How to display attempt status', VALUE_OPTIONAL),
                'displaycoursestructure' => new external_value(PARAM_BOOL, 'Display contents structure', VALUE_OPTIONAL),
                ...
                'hidetoc' => new external_value(PARAM_INT, 'How to display the SCORM structure in player', VALUE_OPTIONAL),
                ...
                'displayactivityname' => new external_value(PARAM_BOOL, 'Display the activity name above the player?', VALUE_OPTIONAL),
                ...
                'protectpackagedownloads' => new external_value(PARAM_BOOL, 'Protect package downloads?', VALUE_OPTIONAL),
                'updatefreq' => new external_value(PARAM_INT, 'Auto-update frequency for remote packages', VALUE_OPTIONAL),
                'options' => new external_value(PARAM_RAW, 'Additional options', VALUE_OPTIONAL),
                'completionstatusrequired' => new external_value(PARAM_INT, 'Status passed/completed required?', VALUE_OPTIONAL),
    ...
    public static function launch_sco($scormid, $scoid = 0) {
        ...
        $params = self::validate_parameters(self::launch_sco_parameters(), array(
                    'scormid' => $scormid,
                    'scoid' => $scoid
        ));
        $warnings = array();
