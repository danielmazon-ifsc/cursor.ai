...
function scorm_insert_track($userid, $scormid, $scoid, $attempt, $element, $value, $forcecompleted = false, $trackdata = null) {
    ...
    if ($forcecompleted) {
        ...
        if (($element == 'cmi.core.lesson_status') && ($value == 'incomplete')) {
            if ($track = $DB->get_record_select('scorm_scoes_track', 'userid=? AND scormid=? AND scoid=? AND attempt=? ' .
                    'AND element=\'cmi.core.score.raw\'', array($userid, $scormid, $scoid, $attempt))) {
                $value = 'completed';
            }
        }
        if ($element == 'cmi.core.score.raw') {
            if ($tracktest = $DB->get_record_select('scorm_scoes_track', 'userid=? AND scormid=? AND scoid=? AND attempt=? ' .
                    'AND element=\'cmi.core.lesson_status\'', array($userid, $scormid, $scoid, $attempt))) {
    ...
    if (in_array($element, array('cmi.core.score.raw', 'cmi.score.raw')) ||
            (in_array($element, array('cmi.completion_status', 'cmi.core.lesson_status', 'cmi.success_status')) && in_array($track->value, array('completed', 'passed')))) {
    ...
    if (in_array($element, array('cmi.core.score.raw', 'cmi.score.raw')) ||
            (in_array($element, array('cmi.completion_status', 'cmi.core.lesson_status', 'cmi.success_status')) && in_array($track->value, array('completed', 'failed', 'passed')))) {
        if (!$scorm) {
            $scorm = $DB->get_record('scorm', array('id' => $scormid));
        }
...
function scorm_count_launchable($scormid, $organization = '') {
    ...
    return $DB->count_records_select('scorm_scoes', "scorm = ? $sqlorganization AND " .
                    $DB->sql_isnotempty('scorm_scoes', 'launch', false, true), $params);
}
...
function scorm_print_launch($user, $scorm, $action, $cm) {
    ...
    if ($orgs = $DB->get_records_select_menu('scorm_scoes', 'scorm = ? AND ' .
            ...
            $DB->sql_isempty('scorm_scoes', 'organization', false, false), array($scorm->id), 'sortorder, id', 'id,title')) {
        if (count($orgs) > 1) {
                    ...
                    if (!empty($sco->launch)) {
                        if ($sco->scormtype == 'sco') {
                            $result->toc .= html_writer::tag('a', $sco->statusicon . '&nbsp;' .
                                            format_string($sco->title) . '&nbsp;' . $score, array('data-scoid' => $sco->id, 'title' => $sco->url));
                        } else {
...
function scorm_get_section_name($cmid, $courseid) {
    global $DB;
...
function scorm_print_scorm_details($scorm, $cm) {
    $html = '';
    $html .= '<!-- Scorm details BEGIN -->';
    $html .= html_writer::start_div('d-flex flex-column', array(
                'id' => 'scormdetails',
                'style' => 'align-items: center;'
    ));
    $html .= html_writer::div('', '', array(
                'style' => 'min-height: 1rem;'
    ));
    $html .= html_writer::start_tag('i', array(
                'class' => 'icon fa fa-bookmark-o mt-16pt',
                'style' => 'color: #288D8C !important; font-size: 3rem; display: contents;'
    ));
    $html .= html_writer::end_tag('i');
    $course = get_course($scorm->course);
    $coursename = $course->fullname;
    $scormcoursetxt = get_string('scormsourse', 'mod_scorm', $coursename);
    $scormcourseurl = new moodle_url('/course/view.php', array(
        'id' => $course->id,
    ));
    $html .= html_writer::tag('a', $scormcoursetxt, array(
                'class' => '',
                'href' => $scormcourseurl,
                'style' => ''
    ));
    $sectionname = scorm_get_section_name($cm->id, $course->id);
    if (!empty($sectionname)) {
        $sectionnametxt = get_string('scormsection', 'mod_scorm', $sectionname);
        $html .= html_writer::tag('p', $sectionnametxt, array(
                    'class' => 'lh-1 mb-0',
                    'style' => 'color: #858d94 !important; width: 80%; text-align: center;'
        ));
    }
    $html .= html_writer::start_tag('a', array(
                'class' => 'btn btn-clear btn-rounded mt-16pt',
                'href' => $scormcourseurl,
                'onclick' => 'window.history.back();'
    ));
    $html .= html_writer::start_tag('i', array(
                'class' => 'icon fa fa-chevron-left fa-fw navicon ml-2 mr-0'
    ));
    $html .= html_writer::end_tag('i');
    $html .= get_string('back');
    $html .= html_writer::end_tag('a');
    $html .= html_writer::end_div();
    $html .= '<!-- Scorm details END -->';
    return $html;
}
...
function scorm_check_launchable_sco($scorm, $scoid) {
    ...
    if ($sco = scorm_get_sco($scoid, SCO_ONLY)) {
        if ($sco->launch == '') {
            ...
            $scoes = $DB->get_records_select('scorm_scoes', 'scorm = ? AND ' . $DB->sql_isnotempty('scorm_scoes', 'launch', false, true) .
...
function scorm_get_sco_and_launch_url($scorm, $scoid, $context) {
        ...
        if ($sco = scorm_get_sco($scoid)) {
            if ($sco->launch == '') {
                ...
                if ($scoes = $DB->get_records_select(
                        'scorm_scoes', 'scorm = ? AND ' . $DB->sql_isnotempty('scorm_scoes', 'launch', false, true) . ' AND id > ?', array($scorm->id, $sco->id), 'sortorder, id')) {
                    $sco = current($scoes);
                }
            }
        }
    ...
    if (!isset($sco)) {
        $scoes = $DB->get_records_select(
                'scorm_scoes', 'scorm = ? AND ' . $DB->sql_isnotempty('scorm_scoes', 'launch', false, true), array($scorm->id), 'sortorder, id'
        );
        $sco = current($scoes);
    }
...
function scorm_update_calendar(stdClass $scorm, $cmid) {
    ...
    if ($event->id = $DB->get_field('event', 'id', array('modulename' => 'scorm', 'instance' => $scorm->id, 'eventtype' => $event->eventtype))) {
