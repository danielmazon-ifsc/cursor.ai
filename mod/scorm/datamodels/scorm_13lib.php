            ...
            if (!scorm_seq_is('suspended', $sco->id, $userid)) {
                if (!isset($sco->completionsetbycontent) || ($sco->completionsetbycontent == 0)) {
                    if (!scorm_seq_is('attemptprogressstatus', $sco->id, $userid, $seq->attempt)) {
                        $incomplete = $DB->get_field('scorm_scoes_track', 'value', array('scoid' => $sco->id,
                            'userid' => $userid,
    ...
    foreach ($conditions as $condition) {
        ...
        switch ($condition['condition']) {
            ...
            case 'attemptLimitExceeded':
                ...
                if ((!isset($r->value) && !$checknot) || (isset($r->value) && ($r->value == $checknot))) {
                    $r = $DB->get_record('scorm_scoes_track', array('scoid' => $sco->id, 'userid' => $userid,
...
function scorm_seq_flow_activity_traversal($activity, $userid, $direction, $childrenflag, $prevdirection, $seq) {
    ...
    if ($rulecheck != null) {
        ...
        if ($skip) {
            ...
            $seq = scorm_seq_flow_activity_traversal($seq->nextactivity, $userid, $direction, $childrenflag, $prevdirection, $seq);
    ...
    if (!scorm_is_leaf($activity)) {
        ...
        if ($seq->identifiedactivity == null) {
            ...
            if ($direction == 'backward' && $seq->traversaldir == 'forward') {
                $seq = scorm_seq_flow_activity_traversal($seq->identifiedactivity, $userid, 'forward', $childrenflag, 'backward', $seq);
            } else {
                $seq = scorm_seq_flow_activity_traversal($seq->identifiedactivity, $userid, $direction, $childrenflag, null, $seq);
            }
            return $seq;
        }
    }
