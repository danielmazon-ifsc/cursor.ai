...
function scorm_add_instance($scorm, $mform = null) {
    ...
    if ($record->scormtype === SCORM_TYPE_LOCAL) {
        if (!empty($scorm->packagefile)) {
            ...
            file_save_draft_area_files($scorm->packagefile, $context->id, 'mod_scorm', 'package', 0, array('subdirs' => 0, 'maxfiles' => 1));
...
function scorm_user_complete($course, $user, $mod, $scorm) {
    ...
    if ($orgs = $DB->get_records_select('scorm_scoes', 'scorm = ? AND ' .
            ...
            $DB->sql_isempty('scorm_scoes', 'organization', false, false), array($scorm->id), 'sortorder, id', 'id, identifier, title')) {
        if (count($orgs) <= 1) {
            unset($orgs);
            $orgs = array();
            $org = new stdClass();
...
function scorm_get_user_grades($scorm, $userid = 0) {
    ...
    $grades = array();
    if (empty($userid)) {
        $scousers = $DB->get_records_select('scorm_scoes_track', "scormid=? GROUP BY userid", array($scorm->id), "", "userid,null");
        if ($scousers) {
        ...
        $preattempt = $DB->get_records_select('scorm_scoes_track', "scormid=? AND userid=? GROUP BY userid", array($scorm->id, $userid), "", "userid,null");
        if (!$preattempt) {
    ...
    foreach ($scorms as $scorm) {
        ...
        if ($showattemptstatus || !empty($isopen) || !empty($scorm->timeclose)) {
            $str = html_writer::start_div('scorm overview') . html_writer::div($strscorm . ': ' .
                            html_writer::link($CFG->wwwroot . '/mod/scorm/view.php?id=' . $scorm->coursemodule, $scorm->name, array('title' => $strscorm, 'class' => $scorm->visible ? '' : 'dimmed')), 'name');
            if ($scorm->timeclose) {
...
function mod_scorm_core_calendar_provide_event_action(calendar_event $event, \core_calendar\action_factory $factory) {
    ...
    return $factory->create_instance(
                    get_string('enter', 'scorm'), new \moodle_url('/mod/scorm/view.php', array('id' => $cm->id)), 1, $actionable
    );
}
