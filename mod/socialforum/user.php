...
$url = new moodle_url('/mod/socialforum/user.php', array('id' => $userid));
if ($isspecificcourse) {
...
if ($isspecificcourse) {
    ...
    $courses = socialforum_get_courses_user_posted_in($user, $discussionsonly);
}
...
$result = SOCIALFORUM_get_posts_by_user($user, $courses, $isspecificcourse, $discussionsonly, ($page * $perpage), $perpage);
...
if (empty($result->posts)) {
    ...
    $pagetitle = get_string('noposts', 'mod_socialforum');

    // Get the page heading
    if ($isspecificcourse) {
        $pageheading = format_string($course->fullname, true, array('context' => $coursecontext));
    } else {
        $pageheading = get_string('pluginname', 'mod_socialforum');
    }
    ...
    if ($iscurrentuser) {
        ...
        // current user.
        if ($discussionsonly) {
            $notification = get_string('nodiscussionsstartedbyyou', 'mod_socialforum');
        } else {
            $notification = get_string('nopostsmadebyyou', 'mod_socialforum');
        }
        ...
        if (isset($courseid) && $courseid != SITEID) {
            ...
            if ($mode == 'posts') {
                $navbar = $PAGE->navbar->add(get_string('posts', 'mod_socialforum'), new moodle_url('/mod/socialforum/user.php', array('id' => $user->id, 'course' => $courseid)));
            } else {
                $navbar = $PAGE->navbar->add(get_string('discussions', 'mod_socialforum'), new moodle_url('/mod/socialforum/user.php', array('id' => $user->id, 'course' => $courseid, 'mode' => 'discussions')));
            }
        }
    } else if ($canviewuser) {
        ...
        if ($discussionsonly) {
            $notification = get_string('nodiscussionsstartedby', 'mod_socialforum', $fullname);
        } else {
            $notification = get_string('nopostsmadebyuser', 'mod_socialforum', $fullname);
        }
    } else {
        // Don't extend the navigation it would be giving out information that
        // the current uesr doesn't have access to.
        $notification = get_string('cannotviewusersposts', 'mod_socialforum');
        if ($isspecificcourse) {
...
$discussions = $DB->get_records_list('socialforum_discussions', 'id', array_unique($discussions));
...
foreach ($result->posts as $post) {
    if (!isset($result->socialforums[$post->socialforum]) || !isset($discussions[$post->discussion])) {
        // Something very VERY dodgy has happened if we end up here
        continue;
    }
    $socialforum = $result->socialforums[$post->socialforum];
    $cm = $socialforum->cm;
    $discussion = $discussions[$post->discussion];
    $course = $result->courses[$discussion->course];

    $socialforumurl = new moodle_url('/mod/socialforum/view.php', array('id' => $cm->id));
    $discussionurl = new moodle_url('/mod/socialforum/discuss.php', array('d' => $post->discussion));
    ...
    if ($socialforum->assessed != RATING_AGGREGATE_NONE) {
        ...
        if ($socialforum->type == 'single' or ! $post->discussion) {
            $ratingoptions->returnurl = $socialforumurl;
        } else {
    ...
    $socialforumname = format_string($socialforum->name, true, array('context' => $cm->context));
    ...
    if (!$isspecificcourse && !$hasparentaccess) {
        ...
        $fullsubjects[] = html_writer::link($socialforumurl, $socialforumname);
    } else {
        $fullsubjects[] = html_writer::tag('span', $courseshortname);
        $fullsubjects[] = html_writer::tag('span', $socialforumname);
    }
    if ($socialforum->type != 'single') {
        ...
        if ($post->parent != 0) {
            ...
            if (!$isspecificcourse && !$hasparentaccess) {
                $fullsubjects[] .= html_writer::link(new moodle_url('/mod/socialforum/discuss.php', array('d' => $post->discussion, 'parent' => $post->id)), $postname);
            } else {
    ...
    $fulllink = html_writer::link($discussionurl, get_string("postincontext", "mod_socialforum"));

    $postoutput[] = SOCIALFORUM_print_post($post, $discussion, $socialforum, $cm, $course, false, false, false, $fulllink, '', null, true, null, true);
}

$userfullname = fullname($user);

if ($discussionsonly) {
    $inpageheading = get_string('discussionsstartedby', 'mod_socialforum', $userfullname);
} else {
    $inpageheading = get_string('postsmadebyuser', 'mod_socialforum', $userfullname);
}
if ($isspecificcourse) {
    $a = new stdClass;
    ...
    if ($discussionsonly) {
        $pagetitle = get_string('discussionsstartedbyuserincourse', 'mod_socialforum', $a);
    } else {
        $pagetitle = get_string('postsmadebyuserincourse', 'mod_socialforum', $a);
    }
} else {
...
if (!empty($postoutput)) {
    ...
    echo $OUTPUT->heading(get_string('nodiscussionsstartedby', 'socialforum', $userfullname));
} else {
    echo $OUTPUT->heading(get_string('noposts', 'mod_socialforum'));
}
