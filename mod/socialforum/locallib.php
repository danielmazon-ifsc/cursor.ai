 ...
 */
class SOCIALFORUM_portfolio_caller extends portfolio_module_caller_base {
    ...
    function __construct($callbackargs) {
        ...
        if (!$this->postid && !$this->discussionid) {
            throw new portfolio_caller_exception('mustprovidediscussionorpost', 'socialforum');
        }
    }
    ...
    public function load_data() {
        ...
        if ($this->postid) {
            if (!$this->post = $DB->get_record('socialforum_posts', array('id' => $this->postid))) {
                throw new portfolio_caller_exception('invalidpostid', 'socialforum');
            }
        }
        ...
        if (!$this->discussion = $DB->get_record('socialforum_discussions', $dbparams)) {
            throw new portfolio_caller_exception('invaliddiscussionid', 'socialforum');
        }

        if (!$this->socialforum = $DB->get_record('socialforum', array('id' => $this->discussion->socialforum))) {
            throw new portfolio_caller_exception('invalidforumid', 'socialforum');
        }

        if (!$this->cm = get_coursemodule_from_instance('socialforum', $this->socialforum->id)) {
        ...
        if ($this->post) {
            if ($this->attachment) {
                ...
                $attach = $fs->get_area_files($this->modcontext->id, 'mod_socialforum', 'attachment', $this->post->id, 'timemodified', false);
                $embed = $fs->get_area_files($this->modcontext->id, 'mod_socialforum', 'post', $this->post->id, 'timemodified', false);
            ...
            $fs = get_file_storage();
            $this->posts = SOCIALFORUM_get_all_discussion_posts($this->discussion->id, 'p.created ASC');
            $this->multifiles = array();
            foreach ($this->posts as $post) {
                $attach = $fs->get_area_files($this->modcontext->id, 'mod_socialforum', 'attachment', $post->id, 'timemodified', false);
                $embed = $fs->get_area_files($this->modcontext->id, 'mod_socialforum', 'post', $post->id, 'timemodified', false);
                $files = array_merge($attach, $embed);
                if ($files) {
    ...
    function get_return_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/socialforum/discuss.php?d=' . $this->discussion->id;
    }
    ...
    function get_navigation() {
        ...
        $navlinks[] = array(
            ...
            'link' => $CFG->wwwroot . '/mod/socialforum/discuss.php?d=' . $this->discussion->id,
            'type' => 'title'
        );
    ...
    function prepare_package() {
        ...
        if ($this->attachment) { // simplest case first - single file attachment
            ...
            if ($writingleap) {
                ...
                $selection = new portfolio_format_leap2a_entry('socialforumdiscussion' . $this->discussionid, get_string('discussion', 'mod_socialforum') . ': ' . $this->discussion->name, 'selection');
    ...
    private function prepare_post_leap2a(portfolio_format_leap2a_writer $leapwriter, $post, $posthtml) {
        $entry = new portfolio_format_leap2a_entry('socialforumpost' . $post->id, $post->subject, 'resource', $posthtml);
        ...
        if (is_array($this->keyedfiles) && array_key_exists($post->id, $this->keyedfiles) && is_array($this->keyedfiles[$post->id])) {
            $leapwriter->link_files($entry, $this->keyedfiles[$post->id], 'socialforumpost' . $post->id . 'attachment');
        }
    ...
    /**
     * this is a very cut down version of what is in SOCIALFORUM_make_mail_post
     *
     * @global object
     * @param int $post
     * @return string
    ...
    private function prepare_post($post, $fileoutputextras = null) {
        ...
        $formattedtext = portfolio_rewrite_pluginfile_urls($formattedtext, $this->modcontext->id, 'mod_socialforum', 'post', $post->id, $format);
        ...
        $output .= '<div class="author">' . get_string('bynameondate', 'socialforum', $by) . '</div>';
        ...
        if (is_array($this->keyedfiles) && array_key_exists($post->id, $this->keyedfiles) && is_array($this->keyedfiles[$post->id]) && count($this->keyedfiles[$post->id]) > 0) {
            ...
            $output .= '<br /><b>' . get_string('attachments', 'mod_socialforum') . '</b>:<br /><br />';
    ...
    function check_permissions() {
        ...
        if ($this->post) {
            return (has_capability('mod/socialforum:exportpost', $context) || ($this->post->userid == $this->user->id && has_capability('mod/socialforum:exportownpost', $context)));
        }
        return has_capability('mod/socialforum:exportdiscussion', $context);
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'mod_socialforum');
    }
 ...
 * @author    Ricardo Drummond
 */
class SOCIALFORUM_file_info_container extends file_info {
    ...
    public function __construct($browser, $course, $cm, $context, $areas, $filearea) {
        ...
        $this->course = $course;
        $this->cm = $cm;
        $this->component = 'mod_socialforum';
    ...
    private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false) {
        ...
        foreach ($rs as $record) {
            if (($child = $this->browser->get_file_info($this->context, 'mod_socialforum', $this->filearea, $record->itemid)) && ($returnemptyfolders || $child->count_non_empty_children($extensions))) {
                $children[] = $child;
            }
