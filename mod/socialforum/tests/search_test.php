...
/**
 * Social Forum search unit tests.
 *
...
/**
 * Provides the unit tests for social forum search.
 *
 * @package   mod_socialforum
 * @category  test
 * @copyright 2016 Viddia (http://viddia.com.br)
 * @author    Ricardo Drummond
 */
class mod_socialforum_search_testcase extends advanced_testcase {

    /**
     * @var string Area id
     */
    protected $socialforumpostareaid = null;

    public function setUp() {
        $this->resetAfterTest(true);
        set_config('enableglobalsearch', true);

        $this->socialforumpostareaid = \core_search\manager::generate_areaid('mod_socialforum', 'post');
    ...
    public function test_search_enabled() {
        ...
        $searcharea = \core_search\manager::get_search_area($this->socialforumpostareaid);
    ...
    /**
     * Indexing mod_socialforum contents.
     *
     * @return void
     */
    public function test_posts_indexing() {
        ...
        $searcharea = \core_search\manager::get_search_area($this->socialforumpostareaid);
        $this->assertInstanceOf('\mod_socialforum\search\post', $searcharea);
    ...
    public function test_posts_document() {
        ...
        set_coursemodule_visible($socialforum2->cmid, 0);
        ...
        $post1 = $DB->get_record('socialforum_posts', array('id' => $discussion1reply1->id));
        $post1->socialforumid = $socialforum1->id;
        $post1->courseid = $socialforum1->course;

        $doc = $searcharea->get_document($post1);
        $this->assertInstanceOf('\core_search\document', $doc);
        $this->assertEquals($discussion1reply1->id, $doc->get('itemid'));
        $this->assertEquals($this->socialforumpostareaid . '-' . $discussion1reply1->id, $doc->get('id'));
    ...
    public function test_posts_access() {
        ...
        $record->userid = $user1->id;
        $record->message = 'post2';
        $discussion2reply1 = self::getDataGenerator()->get_plugin_generator('mod_socialforum')->create_post($record);
    ...
    public function test_attach_files() {
        ...
        $post = $DB->get_record('socialforum_posts', array('discussion' => $discussion1->id));
        $filerecord = array(
            'contextid' => context_module::instance($socialforum1->cmid)->id,
