...
/**
 * The social forum module mail generation tests.
 *
...
class mod_socialforum_mail_testcase extends advanced_testcase {
    ...
    /**
     * Perform message inbound setup for the mod_socialforum reply handler.
     */
    protected function helper_spoof_message_inbound_setup() {
        ...
        $record = $DB->get_record('messageinbound_handlers', array('classname' => '\mod_socialforum\message\inbound\reply_handler'));
        $record->enabled = true;
    ...
    /**
     * Create a new discussion and post within the specified social forum, as the
     * specified author.
     *
     * @param stdClass $socialforum The social forum to post in
     ...
     */
    protected function helper_post_to_socialforum($socialforum, $author, $fields = array()) {
        global $DB;
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_socialforum');

        // Create a discussion in the social forum, and then add a post to that discussion.
    ...
    protected function helper_update_post_time($post, $factor) {
        ...
        $DB->set_field('socialforum_posts', 'created', $post->created + $factor, array('id' => $post->id));
    }
    ...
    protected function helper_update_subscription_time($user, $discussion, $factor) {
        ...
        $sub = $DB->get_record('socialforum_discussion_subs', array('userid' => $user->id, 'discussion' => $discussion->id));

        // Update the subscription to have a preference in the past.
        $DB->set_field('socialforum_discussion_subs', 'preference', $sub->preference + $factor, array('id' => $sub->id));
    }
     ...
     * @param stdClass $author The author to post as
     * @return stdClass The social forum post
     */
    protected function helper_post_to_discussion($socialforum, $discussion, $author) {
        global $DB;
     ...
     *
     * @param stdClass $post The social forum post object
    ...
    /**
     * Run the social forum cron, and check that the specified posts were sent the
     * specified number of times.
     *
     * @param stdClass $post The social forum post object
     * @param integer $expected The number of times that the post should have been sent
    ...
    public function test_forced_subscription() {
        ...
        // Create a course, with a social forum.
        ...
        // Post a discussion to the social forum.
    ...
    public function test_subscription_disabled() {
        ...
        \mod_socialforum\subscriptions::subscribe_user($recipient->id, $socialforum);

        $this->assertEquals($expected, $DB->count_records('socialforum_subscriptions', array(
                    'userid' => $recipient->id,
                    'socialforum' => $socialforum->id,
        )));

        // Run cron and check that the expected number of users received the notification.
        list($discussion, $post) = $this->helper_post_to_socialforum($socialforum, $recipient);
        $messages = $this->helper_run_cron_check_count($post, $expected);

        // Unsubscribe the user again.
        \mod_socialforum\subscriptions::unsubscribe_user($recipient->id, $socialforum);
        ...
        \mod_socialforum\subscriptions::subscribe_user_to_discussion($recipient->id, $discussion);
        $this->helper_update_subscription_time($recipient, $discussion, -60);

        $reply = $this->helper_post_to_discussion($socialforum, $discussion, $author);
    ...
    public function test_automatic_with_unsubscribed_user() {
        ...
        // Create a course, with a socia lforum.
        $course = $this->getDataGenerator()->create_course();
        ...
        // Unsubscribe the 'author' user from the socia lforum.
        \mod_socialforum\subscriptions::unsubscribe_user($author->id, $socialforum);

        // Post a discussion to the socia lforum.
        list($discussion, $post) = $this->helper_post_to_socialforum($socialforum, $author);
    ...
    public function test_optional_with_subscribed_user() {
        ...
        // Subscribe the 'recipient' user from the socia lforum.
        \mod_socialforum\subscriptions::subscribe_user($recipient->id, $socialforum);
    ...
    public function test_automatic_with_unsubscribed_discussion() {
        ...
        $this->assertTrue(\mod_socialforum\subscriptions::is_subscribed($recipient->id, $socialforum, $discussion->id));
    ...
    public function test_automatic_with_subscribed_discussion_in_unsubscribed_socialforum() {
        $this->resetAfterTest(true);
        ...
        // We expect just the user subscribed to the socia lforum to receive this post at the moment as the discussion
    ...
    public function test_optional_with_unsubscribed_discussion_in_subscribed_socialforum() {
        $this->resetAfterTest(true);
        ...
        \mod_socialforum\subscriptions::unsubscribe_user_from_discussion($recipient->id, $discussion);
    ...
    /**
     * Test that a user unsubscribed from a socia lforum who has subscribed to a discussion, only receives posts made after
     * they subscribed to the discussion.
     */
    public function test_socialforum_discussion_subscription_socialforum_unsubscribed_discussion_subscribed_after_post() {
        $this->resetAfterTest(true);
    ...
    public function test_socialforum_message_inbound_multiple_posts() {
        $this->resetAfterTest(true);
        ...
        // Ensure that messageinbound is enabled and configured for the socia lforum handler.
        $this->helper_spoof_message_inbound_setup();

        $author->emailstop = '0';
        set_user_preference('message_provider_mod_socialforum_posts_loggedoff', 'email', $author);
        set_user_preference('message_provider_mod_socialforum_posts_loggedin', 'email', $author);
    ...
    public function test_long_subject() {
        ...
        // Post a discussion to the socia lforum.
        $subject = 'This is the very long social forum post subject that somebody was very kind of leaving, it is intended to check if long subject comes in mail correctly. Thank you.';
        $a = (object) array('courseshortname' => $course->shortname, 'socialforumname' => $socialforum->name, 'subject' => $subject);
        $expectedsubject = get_string('postmailsubject', 'socialforum', $a);
        list($discussion, $post) = $this->helper_post_to_socialforum($socialforum, $author, array('name' => $subject));
    ...
    public function test_subjects() {
        ...
        $reply = $this->helper_post_to_discussion($socialforum, $discussion, $commenter);
        $messages = $this->helper_run_cron_check_count($reply, 2);
        $this->assertContains($strre, $messages[0]->subject);
    }

    /**
     * dataProvider for test_socialforum_post_email_templates().
     */
    public function socialforum_post_email_templates_provider() {
        ...
        $base = array(
            ...
            'socialforums' => array(
                array(
                    'name' => 'Moodle Social Forum',
                    'socialforumposts' => array(
                        array(
                            'name' => 'Hello Moodle',
            ...
            'expectations' => array(
                array(
                    ...
                    'contents' => array(
                        ...
                        'http://www.example.com/moodle/pluginfile.php/\d*/mod_socialforum/attachment/\d*/example.txt\n',
                        'Hello Moodle', 'Moodle Social Forum', 'Welcome.*Moodle', 'Love Moodle', '1\d1'
                    ),
                ),
            ),
        );
        ...
        $newcase['socialforums'][0]['name'] = 'Moodle Social Forum\'"';
        $newcase['socialforums'][0]['socialforumposts'][0]['name'] = 'Hello Moodle\'"';
        $newcase['socialforums'][0]['socialforumposts'][0]['message'] = 'Welcome to Moodle\'"';
        $newcase['expectations'][0]['contents'] = array(
            'Attachment example.txt:', '~{\$a', '~&amp;(quot|\#039);', 'Love Moodle\'', '101\'', 'Moodle Social Forum\'"',
        ...
        $newcase['socialforums'][0]['socialforumposts'][0]['message'] = 'Welcome to Moodle>';
        $newcase['expectations'][0]['contents'] = array(
            'Attachment example.txt:', '~{\$a', '~&amp;gt;', 'Love Moodle>', '101>', 'Moodle Social Forum>',
            'Hello Moodle>', 'Welcome to Moodle>');
        ...
        $newcase = $base;
        $newcase['socialforums'][0]['socialforumposts'][0]['name'] = 'Text and image';
        $newcase['socialforums'][0]['socialforumposts'][0]['message'] = 'Welcome to Moodle, '
        ...
        $newcase['expectations'][0]['contents'] = array(
            ...
            'Attachment example.txt:\n' .
            'http://www.example.com/moodle/pluginfile.php/\d*/mod_socialforum/attachment/\d*/example.txt\n',
            'Text and image', 'Moodle Social Forum',
            'Welcome to Moodle, *\n.*'
            . 'http://www.example.com/moodle/pluginfile.php/\d+/mod_socialforum/post/\d+/'
        ...
        $newcase['expectations'][0]['contents'] = array(
            ...
            '~&amp;(amp|lt|gt|quot|\#039);',
            '<div class="attachments">( *\n *)?<a href',
            '<div class="subject">\n.*Hello Moodle\'"&gt;&amp;', '>Moodle Social Forum\'"&gt;&amp;',
            '>Welcome.*Moodle\'"&gt;&amp;', '>Love Moodle&\#039;&quot;&gt;&amp;', '>101\'"&gt;&amp');
        $htmlcases['HTML mail with quotes, gt, lt and ampersand  everywhere'] = array('data' => $newcase);

        // Text+image message i.e. @@PLUGINFILE@@ token handling.
        $newcase = $htmlbase;
        $newcase['socialforums'][0]['socialforumposts'][0]['name'] = 'HTML text and image';
        $newcase['socialforums'][0]['socialforumposts'][0]['message'] = '<p>Welcome to Moodle, '
        ...
        $newcase['expectations'][0]['contents'] = array(
            ...
            '~&(amp|lt|gt|quot|\#039);(?!course)',
            '<div class="attachments">( *\n *)?<a href',
            '<div class="subject">\n.*HTML text and image', '>Moodle Social Forum',
            '<p>Welcome to Moodle, '
            . '<img src="http://www.example.com/moodle/pluginfile.php/\d+/mod_socialforum/post/\d+/'
            . 'Screen%20Shot%202016-03-22%20at%205\.54\.36%20AM%20%281%29\.png"'
            . ' alt="" width="200" height="393" class="img-responsive" />!</p>',
            '>Love Moodle', '>1\d1');
    ...
    /**
     * Verify socia lforum emails body using templates to generate the expected results.
     *
     * @dataProvider socialforum_post_email_templates_provider
     * @param array $data provider samples.
     */
    public function test_socialforum_post_email_templates($data) {
        global $DB;
        ...
        // Create socia lforums, always force susbscribed (for easy), with the specified options.
        $posts = array();
        foreach ($data['socialforums'] as $datasocialforum) {
            $socialforumposts = isset($datasocialforum['socialforumposts']) ? $datasocialforum['socialforumposts'] : array();
            unset($datasocialforum['socialforumposts']);
            $options = array('course' => $course->id, 'forcesubscribe' => SOCIALFORUM_FORCESUBSCRIBE);
            foreach ($datasocialforum as $option => $value) {
                $options[$option] = $value;
            }
            $socialforum = $this->getDataGenerator()->create_module('socialforum', $options);

            // Create posts, always for immediate delivery (for easy), with the specified options.
            foreach ($socialforumposts as $socialforumpost) {
                $attachments = isset($socialforumpost['attachments']) ? $socialforumpost['attachments'] : array();
                unset($socialforumpost['attachments']);
                $postoptions = array('course' => $course->id, 'socialforum' => $socialforum->id, 'userid' => $user->id,
                    'mailnow' => 1, 'attachment' => !empty($attachments));
                foreach ($socialforumpost as $option => $value) {
                    $postoptions[$option] = $value;
                }
                list($discussion, $post) = $this->helper_post_to_socialforum($socialforum, $user, $postoptions);
                ...
                if ($attachments) {
                    ...
                    foreach ($attachments as $attachment) {
                        $filerecord = array(
                            'contextid' => context_module::instance($socialforum->cmid)->id,
                    ...
                    $DB->set_field('socialforum_posts', 'attachment', '1', array('id' => $post->id));
                }
            }
        }
        ...
        socialforum_cron(); // It's really annoying that we have to run cron to test this.
        // Get the mails.
