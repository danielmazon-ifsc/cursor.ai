...
class mod_socialforum_external_testcase extends externallib_advanced_testcase {
    ...
    protected function setUp() {
        ...
        require_once($CFG->dirroot . '/mod/socialforum/externallib.php');
    }
     ...
     * Test get forums
     */
    public function test_mod_socialforum_get_forums_by_courses() {
        global $USER, $CFG, $DB;
        ...
        $socialforum1 = self::getDataGenerator()->create_module('socialforum', $record);

        // Second forum.
        $record = new stdClass();
        $record->introformat = FORMAT_HTML;
        $record->course = $course2->id;
        $socialforum2 = self::getDataGenerator()->create_module('socialforum', $record);
        ...
        $discussion1 = self::getDataGenerator()->get_plugin_generator('mod_socialforum')->create_discussion($record);
        // Expect one discussion.
        $socialforum1->numdiscussions = 1;
        $socialforum1->cancreatediscussions = true;

        $record = new stdClass();
        $record->course = $course2->id;
        $record->userid = $user->id;
        $record->socialforum = $socialforum2->id;
        $discussion2 = self::getDataGenerator()->get_plugin_generator('mod_socialforum')->create_discussion($record);
        $discussion3 = self::getDataGenerator()->get_plugin_generator('mod_socialforum')->create_discussion($record);
        // Expect two discussions.
        $socialforum2->numdiscussions = 2;
        // Default limited role, no create discussion capability enabled.
        $socialforum2->cancreatediscussions = false;

        // Check the forum was correctly created.
        $this->assertEquals(2, $DB->count_records_select('socialforum', 'id = :socialforum1 OR id = :socialforum2', array('socialforum1' => $socialforum1->id, 'socialforum2' => $socialforum2->id)));
        ...
        $cm2 = get_coursemodule_from_id('socialforum', $socialforum2->cmid, 0, false, MUST_EXIST);
        $context2 = context_module::instance($cm2->id);
        $newrole = create_role('Role 2', 'role2', 'Role 2 description');
        $roleid2 = $this->assignUserCapability('mod/socialforum:viewdiscussion', $context2->id, $newrole);

        // Create what we expect to be returned when querying the two courses.
        unset($socialforum1->displaywordcount);
        unset($socialforum2->displaywordcount);

        $expectedforums = array();
        $expectedforums[$socialforum1->id] = (array) $socialforum1;
        $expectedforums[$socialforum2->id] = (array) $socialforum2;

        // Call the external function passing course ids.
        $socialforums = mod_socialforum_external::get_socialforums_by_courses(array($course1->id, $course2->id));
        $socialforums = external_api::clean_returnvalue(mod_socialforum_external::get_socialforums_by_courses_returns(), $socialforums);
        $this->assertCount(2, $socialforums);
        foreach ($socialforums as $socialforum) {
            $this->assertEquals($expectedforums[$socialforum['id']], $socialforum);
        }

        // Call the external function without passing course id.
        $socialforums = mod_socialforum_external::get_socialforums_by_courses();
        $socialforums = external_api::clean_returnvalue(mod_socialforum_external::get_socialforums_by_courses_returns(), $socialforums);
        $this->assertCount(2, $socialforums);
        foreach ($socialforums as $socialforum) {
            $this->assertEquals($expectedforums[$socialforum['id']], $socialforum);
        ...
        unset($expectedforums[$socialforum2->id]);

        // Call the external function without passing course id.
        $socialforums = mod_socialforum_external::get_socialforums_by_courses();
        $socialforums = external_api::clean_returnvalue(mod_socialforum_external::get_socialforums_by_courses_returns(), $socialforums);
        $this->assertCount(1, $socialforums);
        $this->assertEquals($expectedforums[$socialforum1->id], $socialforums[0]);
        $this->assertTrue($socialforums[0]['cancreatediscussions']);

        // Change the type of the forum, the user shouldn't be able to add discussions.
        $DB->set_field('socialforum', 'type', 'news', array('id' => $socialforum1->id));
        $socialforums = mod_socialforum_external::get_socialforums_by_courses();
        $socialforums = external_api::clean_returnvalue(mod_socialforum_external::get_socialforums_by_courses_returns(), $socialforums);
        $this->assertFalse($socialforums[0]['cancreatediscussions']);

        // Call for the second course we unenrolled the user from.
        $socialforums = mod_socialforum_external::get_socialforums_by_courses(array($course2->id));
        $socialforums = external_api::clean_returnvalue(mod_socialforum_external::get_socialforums_by_courses_returns(), $socialforums);
        $this->assertCount(0, $socialforums);
    }

    /**
     * Test get forum posts
     */
    public function test_mod_socialforum_get_socialforum_discussion_posts() {
        global $CFG, $PAGE;

        $this->resetAfterTest(true);

        // Set the CFG variable to allow track forums.
        $CFG->socialforum_trackreadposts = true;
        ...
        // Social Forum with tracking off.
        $record = new stdClass();
        $record->course = $course1->id;
        $record->trackingtype = SOCIALFORUM_TRACKING_OFF;
        $socialforum1 = self::getDataGenerator()->create_module('socialforum', $record);
        $socialforum1context = context_module::instance($socialforum1->cmid);
        ...
        $record->userid = $user2->id;
        $discussion1reply1 = self::getDataGenerator()->get_plugin_generator('mod_socialforum')->create_post($record);

        $record->parent = $discussion1reply1->id;
        $record->userid = $user3->id;
        $discussion1reply2 = self::getDataGenerator()->get_plugin_generator('mod_socialforum')->create_post($record);

        // Enrol the user in the  course.
        $enrol = enrol_get_plugin('manual');
        // Following line enrol and assign default role id to the user.
        // So the user automatically gets mod/socialforum:viewdiscussion on all forums of the course.
        ...
        $expectedposts['posts'][] = array(
            ...
            'message' => file_rewrite_pluginfile_urls($discussion1reply2->message, 'pluginfile.php', $socialforum1context->id, 'mod_socialforum', 'post', $discussion1reply2->id),
        ...
        $expectedposts['posts'][] = array(
            ...
            'message' => file_rewrite_pluginfile_urls($discussion1reply1->message, 'pluginfile.php', $socialforum1context->id, 'mod_socialforum', 'post', $discussion1reply1->id),
            'messageformat' => 1, // This value is usually changed by external_format_text() function.
        ...
        $posts = mod_socialforum_external::get_socialforum_discussion_posts($discussion1->id, 'modified', 'DESC');
        $posts = external_api::clean_returnvalue(mod_socialforum_external::get_socialforum_discussion_posts_returns(), $posts);
        ...
        $posts = mod_socialforum_external::get_socialforum_discussion_posts($discussion2->id, 'modified', 'DESC');
        $posts = external_api::clean_returnvalue(mod_socialforum_external::get_socialforum_discussion_posts_returns(), $posts);
     ...
     */
    public function test_mod_socialforum_get_socialforum_discussion_posts_qanda() {
        global $CFG, $DB;
        ...
        // We see the original post and the reply.
        $posts = mod_socialforum_external::get_socialforum_discussion_posts($discussion1->id, 'modified', 'DESC');
        $posts = external_api::clean_returnvalue(mod_socialforum_external::get_socialforum_discussion_posts_returns(), $posts);
        $this->assertEquals(2, count($posts['posts']));

        // Add a new reply, the user is going to be able to see the original post, the first reply and their new post.
        $record = new stdClass();
        $record->discussion = $discussion1->id;
        $record->parent = $discussion1->firstpost;
        $record->userid = $user1->id;
        ...
        $DB->update_record('socialforum_posts', $discussion1reply2);
     ...
     */
    public function test_mod_socialforum_get_socialforum_discussions_paginated() {
        ...
        $discussion1reply3 = self::getDataGenerator()->get_plugin_generator('mod_socialforum')->create_post($record);
        ...
        $cm = get_coursemodule_from_id('socialforum', $socialforum1->cmid, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);
        $newrole = create_role('Role 2', 'role2', 'Role 2 description');
        $this->assignUserCapability('mod/socialforum:viewdiscussion', $context->id, $newrole);

        // Create what we expect to be returned when querying the forums.

        $post1 = $DB->get_record('socialforum_posts', array('id' => $discussion1->firstpost), '*', MUST_EXIST);
        ...
        $expecteddiscussions = array(
            ...
            'numreplies' => 3,
            'numunread' => 0,
            'pinned' => SOCIALFORUM_DISCUSSION_UNPINNED
        );

        // Call the external function passing forum id.
        $discussions = mod_socialforum_external::get_socialforum_discussions_paginated($socialforum1->id);
        $discussions = external_api::clean_returnvalue(mod_socialforum_external::get_socialforum_discussions_paginated_returns(), $discussions);
        ...
        $this->unassignUserCapability('mod/socialforum:viewdiscussion', $context->id, $newrole);
        try {
            mod_socialforum_external::get_socialforum_discussions_paginated($socialforum1->id);
     ...
     */
    public function test_mod_socialforum_get_socialforum_discussions_paginated_qanda() {
        ...
        $discussionrecord->socialforum = $socialforum->id;
        $discussion = self::getDataGenerator()->get_plugin_generator('mod_socialforum')->create_discussion($discussionrecord);

        self::setAdminUser();
        $discussions = mod_socialforum_external::get_socialforum_discussions_paginated($socialforum->id);
        $discussions = external_api::clean_returnvalue(mod_socialforum_external::get_socialforum_discussions_paginated_returns(), $discussions);
    ...
    public function test_add_discussion_post() {
        ...
        $record->course = $course->id;
        $socialforum = self::getDataGenerator()->create_module('socialforum', $record);
        $cm = get_coursemodule_from_id('socialforum', $socialforum->cmid, 0, false, MUST_EXIST);
        $socialforumcontext = context_module::instance($socialforum->cmid);
        ...
        self::setUser($user);
        try {
            mod_socialforum_external::add_discussion_post($discussion->firstpost, 'some subject', 'some text here...');
            $this->fail('Exception expected due to being unenrolled from the course.');
        } catch (moodle_exception $e) {
            $this->assertEquals('requireloginerror', $e->errorcode);
        }
        ...
        $createdpost = mod_socialforum_external::add_discussion_post($discussion->firstpost, 'some subject', 'some text here...');
        $createdpost = external_api::clean_returnvalue(mod_socialforum_external::add_discussion_post_returns(), $createdpost);

        $posts = mod_socialforum_external::get_socialforum_discussion_posts($discussion->id);
        $posts = external_api::clean_returnvalue(mod_socialforum_external::get_socialforum_discussion_posts_returns(), $posts);
        ...
        $dummytext = 'Here is an inline image: <img src="' . $CFG->wwwroot
                ...
                . '" alt="inlineimage">.';
        $createdpost = mod_socialforum_external::add_discussion_post($discussion->firstpost, 'new post inline attachment', $dummytext, $options);
        $createdpost = external_api::clean_returnvalue(mod_socialforum_external::add_discussion_post_returns(), $createdpost);
        ...
        try {
            ...
            $this->assertEquals('nopostsocialforum', $e->errorcode);
        }
    }
    ...
    public function test_add_discussion() {
        ...
        try {
            mod_socialforum_external::add_discussion($socialforum->id, 'the subject', 'some text here...');
        ...
        self::setAdminUser();
        $createddiscussion = mod_socialforum_external::add_discussion($socialforum->id, 'the subject', 'some text here...');
        $createddiscussion = external_api::clean_returnvalue(mod_socialforum_external::add_discussion_returns(), $createddiscussion);
        ...
        $discussion2pinned = mod_socialforum_external::add_discussion($socialforum->id, 'the pinned subject', 'some 2 text here...', -1, array('options' => array('name' => 'discussionpinned',
                        'value' => true)));
        $discussion3 = mod_socialforum_external::add_discussion($socialforum->id, 'the non pinnedsubject', 'some 3 text here...');
        $discussions = mod_socialforum_external::get_socialforum_discussions_paginated($socialforum->id);
        $discussions = external_api::clean_returnvalue(mod_socialforum_external::get_socialforum_discussions_paginated_returns(), $discussions);
        ...
        $options = array(array('name' => 'inlineattachmentsid', 'value' => $draftidinlineattach),
            array('name' => 'attachmentsid', 'value' => $draftidattach));
        $createddiscussion = mod_socialforum_external::add_discussion($socialforum->id, 'the inline attachment subject', $dummytext, -1, $options);
        $createddiscussion = external_api::clean_returnvalue(mod_socialforum_external::add_discussion_returns(), $createddiscussion);
    ...
    public function test_add_discussion_in_course_with_groups() {
        ...
        try {
            mod_socialforum_external::add_discussion($socialforum->id, 'the subject', 'some text here...', 0);
            $this->fail('Exception expected due to invalid group permissions.');
        } catch (moodle_exception $e) {
            $this->assertEquals('cannotcreatediscussion', $e->errorcode);
        }
        ...
        try {
            mod_socialforum_external::add_discussion($socialforum->id, 'the subject', 'some text here...', $group->id);
            $this->fail('Exception expected due to invalid group permissions.');
        } catch (moodle_exception $e) {
            $this->assertEquals('cannotcreatediscussion', $e->errorcode);
        }
        ...
        // Try to post in a group the user is not enrolled.
        try {
            mod_socialforum_external::add_discussion($socialforum->id, 'the subject', 'some text here...', $group->id + 1);
        ...
        $discussion = mod_socialforum_external::add_discussion($socialforum->id, 'the subject', 'some text here...', $group->id);
        $discussion = external_api::clean_returnvalue(mod_socialforum_external::add_discussion_returns(), $discussion);
        ...
        $discussion = mod_socialforum_external::add_discussion($socialforum->id, 'the subject', 'some text here...');
        $discussion = external_api::clean_returnvalue(mod_socialforum_external::add_discussion_returns(), $discussion);
    ...
    public function test_can_add_discussion() {
        ...
        $result = mod_socialforum_external::can_add_discussion($socialforum->id);
        $result = external_api::clean_returnvalue(mod_socialforum_external::can_add_discussion_returns(), $result);
