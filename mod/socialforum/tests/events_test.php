 ...
 * @author    Ricardo Drummond
 */
class mod_socialforum_events_testcase extends advanced_testcase {
    ...
    public function test_course_searched_searchterm_validation() {
        ...
        \mod_socialforum\event\course_searched::create($params);
    }
    ...
    public function test_course_searched() {
        ...
        $this->assertInstanceOf('\mod_socialforum\event\course_searched', $event);
        $this->assertEquals($coursectx, $event->get_context());
        $expected = array($course->id, 'socialforum', 'search', "search.php?id={$course->id}&amp;search={$searchterm}", $searchterm);
    ...
    public function test_discussion_created_forumid_validation() {
        ...
        $this->setExpectedException('coding_exception', 'The \'socialforumid\' value must be set in other.');
        \mod_socialforum\event\discussion_created::create($params);
    }
    ...
    public function test_discussion_created() {
        ...
        $discussion = $this->getDataGenerator()->get_plugin_generator('mod_socialforum')->create_discussion($record);
        ...
        $this->assertInstanceOf('\mod_socialforum\event\discussion_created', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'socialforum', 'add discussion', "discuss.php?d={$discussion->id}", $discussion->id, $socialforum->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_discussion_updated_forumid_validation() {
        ...
        $this->setExpectedException('coding_exception', 'The \'socialforumid\' value must be set in other.');
        \mod_socialforum\event\discussion_updated::create($params);
    }
    ...
    public function test_discussion_updated() {
        ...
        // Check that the event contains the expected values.
        $this->assertInstanceOf('\mod_socialforum\event\discussion_updated', $event);
        $this->assertEquals($context, $event->get_context());
        $this->assertEventContextNotUsed($event);
    ...
    public function test_discussion_deleted_forumid_validation() {
        ...
        $this->setExpectedException('coding_exception', 'The \'socialforumid\' value must be set in other.');
        \mod_socialforum\event\discussion_deleted::create($params);
    }
    ...
    public function test_discussion_deleted() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_socialforum\event\discussion_deleted', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'socialforum', 'delete discussion', "view.php?id={$socialforum->cmid}", $socialforum->id, $socialforum->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_discussion_moved_fromforumid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $tosocialforum = $this->getDataGenerator()->create_module('socialforum', array('course' => $course->id));

        $context = context_module::instance($tosocialforum->cmid);

        $params = array(
            'context' => $context,
            'other' => array('tosocialforumid' => $tosocialforum->id)
        );

        $this->setExpectedException('coding_exception', 'The \'fromsocialforumid\' value must be set in other.');
        \mod_socialforum\event\discussion_moved::create($params);
    }
    ...
    public function test_discussion_moved_toforumid_validation() {
        $course = $this->getDataGenerator()->create_course();
        $fromforum = $this->getDataGenerator()->create_module('socialforum', array('course' => $course->id));
        $toforum = $this->getDataGenerator()->create_module('socialforum', array('course' => $course->id));
        $context = context_module::instance($toforum->cmid);

        $params = array(
            'context' => $context,
            'other' => array('fromsocialforumid' => $fromforum->id)
        );

        $this->setExpectedException('coding_exception', 'The \'tosocialforumid\' value must be set in other.');
        \mod_socialforum\event\discussion_moved::create($params);
    }
    ...
    public function test_discussion_moved_context_validation() {
        ...
        $record = array();
        $record['course'] = $course->id;
        $record['socialforum'] = $fromforum->id;
        $record['userid'] = $user->id;
        $discussion = $this->getDataGenerator()->get_plugin_generator('mod_socialforum')->create_discussion($record);
        ...
        $params = array(
            'context' => context_system::instance(),
            'objectid' => $discussion->id,
            'other' => array('fromsocialforumid' => $fromforum->id, 'tosocialforumid' => $toforum->id)
        );
    ...
    public function test_discussion_moved() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_socialforum\event\discussion_moved', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'socialforum', 'move discussion', "discuss.php?d={$discussion->id}",
    ...
    public function test_discussion_viewed_context_validation() {
        ...
        $this->setExpectedException('coding_exception', 'Context level must be CONTEXT_MODULE.');
        \mod_socialforum\event\discussion_viewed::create($params);
    }
    ...
    public function test_course_module_viewed_context_validation() {
        ...
        $this->setExpectedException('coding_exception', 'Context level must be CONTEXT_MODULE.');
        \mod_socialforum\event\course_module_viewed::create($params);
    }
    ...
    public function test_course_module_viewed() {
        ...
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'socialforum', 'view socialforum', "view.php?f={$socialforum->id}", $socialforum->id, $socialforum->cmid);
        $this->assertEventLegacyLogData($expected, $event);
    ...
    public function test_subscription_created_forumid_validation() {
        ...
        $this->setExpectedException('coding_exception', 'The \'socialforumid\' value must be set in other.');
        \mod_socialforum\event\subscription_created::create($params);
    }
    ...
    public function test_subscription_created() {
        ...
        $record['socialforum'] = $socialforum->id;
        $record['userid'] = $user->id;
        $subscription = $this->getDataGenerator()->get_plugin_generator('mod_socialforum')->create_subscription($record);
        ...
        $event = \mod_socialforum\event\subscription_created::create($params);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_socialforum\event\subscription_created', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'socialforum', 'subscribe', "view.php?f={$socialforum->id}", $socialforum->id, $socialforum->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/socialforum/subscribers.php', array('id' => $socialforum->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    public function test_subscription_deleted_forumid_validation() {
        ...
        $this->setExpectedException('coding_exception', 'The \'socialforumid\' value must be set in other.');
        \mod_socialforum\event\subscription_deleted::create($params);
    }
    ...
    public function test_subscription_deleted() {
        ...
        $event = \mod_socialforum\event\subscription_deleted::create($params);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_socialforum\event\subscription_deleted', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'socialforum', 'unsubscribe', "view.php?f={$socialforum->id}", $socialforum->id, $socialforum->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/socialforum/subscribers.php', array('id' => $socialforum->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    public function test_readtracking_enabled_forumid_validation() {
        ...
        $this->setExpectedException('coding_exception', 'The \'socialforumid\' value must be set in other.');
        \mod_socialforum\event\readtracking_enabled::create($params);
    }
    ...
    public function test_readtracking_enabled() {
        ...
        $event = \mod_socialforum\event\readtracking_enabled::create($params);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_socialforum\event\readtracking_enabled', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'socialforum', 'start tracking', "view.php?f={$socialforum->id}", $socialforum->id, $socialforum->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/socialforum/view.php', array('f' => $socialforum->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    public function test_readtracking_disabled_forumid_validation() {
        ...
        $this->setExpectedException('coding_exception', 'The \'socialforumid\' value must be set in other.');
        \mod_socialforum\event\readtracking_disabled::create($params);
    }
    ...
    public function test_readtracking_disabled() {
        ...
        $event = \mod_socialforum\event\readtracking_disabled::create($params);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_socialforum\event\readtracking_disabled', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'socialforum', 'stop tracking', "view.php?f={$socialforum->id}", $socialforum->id, $socialforum->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/socialforum/view.php', array('f' => $socialforum->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    public function test_subscribers_viewed_forumid_validation() {
        ...
        $this->setExpectedException('coding_exception', 'The \'socialforumid\' value must be set in other.');
        \mod_socialforum\event\subscribers_viewed::create($params);
    }
    ...
    public function test_subscribers_viewed() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_socialforum\event\subscribers_viewed', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'socialforum', 'view subscribers', "subscribers.php?id={$socialforum->id}", $socialforum->id, $socialforum->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
    ...
    public function test_user_report_viewed_reportmode_validation() {
        ...
        \mod_socialforum\event\user_report_viewed::create($params);
    }
    ...
    public function test_user_report_viewed() {
        ...
        $event = \mod_socialforum\event\user_report_viewed::create($params);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_socialforum\event\user_report_viewed', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'socialforum', 'user report',
    ...
    public function test_post_created_postid_validation() {
        ...
        $params = array(
            'context' => context_module::instance($socialforum->cmid),
            'other' => array('socialforumid' => $socialforum->id, 'socialforumtype' => $socialforum->type, 'discussionid' => $discussion->id)
        );

        \mod_socialforum\event\post_created::create($params);
    }
    ...
    public function test_post_created_discussionid_validation() {
        ...
        $params = array(
            'context' => context_module::instance($socialforum->cmid),
            'objectid' => $post->id,
            'other' => array('socialforumid' => $socialforum->id, 'socialforumtype' => $socialforum->type)
        );
    ...
    public function test_post_created_forumid_validation() {
        ...
        $params = array(
            'context' => context_module::instance($socialforum->cmid),
            'objectid' => $post->id,
            'other' => array('discussionid' => $discussion->id, 'socialforumtype' => $socialforum->type)
        );
    ...
    public function test_post_created_forumtype_validation() {
        ...
        $params = array(
            'context' => context_module::instance($socialforum->cmid),
            'objectid' => $post->id,
            'other' => array('discussionid' => $discussion->id, 'socialforumid' => $socialforum->id)
        );

        $this->setExpectedException('coding_exception', 'The \'socialforumtype\' value must be set in other.');
        \mod_socialforum\event\post_created::create($params);
    }
    ...
    public function test_post_created_context_validation() {
        ...
        $params = array(
            'context' => context_system::instance(),
            'objectid' => $post->id,
            'other' => array('discussionid' => $discussion->id, 'socialforumid' => $socialforum->id, 'socialforumtype' => $socialforum->type)
        );
    ...
    public function test_post_created() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_socialforum\event\post_created', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'socialforum', 'add post', "discuss.php?d={$discussion->id}#p{$post->id}",
            $socialforum->id, $socialforum->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/socialforum/discuss.php', array('d' => $discussion->id));
    ...
    public function test_post_created_single() {
        // Setup test data.
        $course = $this->getDataGenerator()->create_course();
        $socialforum = $this->getDataGenerator()->create_module('socialforum', array('course' => $course->id, 'type' => 'single'));
        $user = $this->getDataGenerator()->create_user();
        ...
        $this->assertInstanceOf('\mod_socialforum\event\post_created', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'socialforum', 'add post', "view.php?f={$socialforum->id}#p{$post->id}",
            $socialforum->id, $socialforum->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/socialforum/view.php', array('f' => $socialforum->id));
        $url->set_anchor('p' . $event->objectid);
    ...
    public function test_post_deleted_postid_validation() {
        ...
        \mod_socialforum\event\post_deleted::create($params);
    }
    ...
    public function test_post_deleted() {
        ...
        $discussionpost = $DB->get_records('socialforum_posts');
        ...
        $lastpost = end($posts);
        $sink = $this->redirectEvents();
        socialforum_delete_post($lastpost, true, $course, $cm, $socialforum);
        $events = $sink->get_events();
        $this->assertCount(1, $events);
        $event = reset($events);

        // Check that the events contain the expected values.
        $this->assertInstanceOf('\mod_socialforum\event\post_deleted', $event);
        $this->assertEquals(context_module::instance($socialforum->cmid), $event->get_context());
        $expected = array($course->id, 'socialforum', 'delete post', "discuss.php?d={$discussion->id}", $lastpost->id, $socialforum->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/socialforum/discuss.php', array('d' => $discussion->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
        ...
        $sink = $this->redirectEvents();
        socialforum_delete_discussion($discussion, true, $course, $cm, $socialforum);
        $events = $sink->get_events();
        ...
        foreach ($events as $event) {
            ...
            $this->assertInstanceOf('\mod_socialforum\event\post_deleted', $event);
            $this->assertEquals(context_module::instance($socialforum->cmid), $event->get_context());
            $expected = array($course->id, 'socialforum', 'delete post', "discuss.php?d={$discussion->id}", $post->id, $socialforum->cmid);
            $this->assertEventLegacyLogData($expected, $event);
            $url = new \moodle_url('/mod/socialforum/discuss.php', array('d' => $discussion->id));
            $this->assertEquals($url, $event->get_url());
            $this->assertEventContextNotUsed($event);
    ...
    public function test_post_deleted_single() {
        ...
        $this->assertInstanceOf('\mod_socialforum\event\post_deleted', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'socialforum', 'delete post', "view.php?f={$socialforum->id}", $post->id, $socialforum->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/socialforum/view.php', array('f' => $socialforum->id));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    public function test_post_updated_discussionid_validation() {
        ...
        $this->setExpectedException('coding_exception', 'The \'discussionid\' value must be set in other.');
        \mod_socialforum\event\post_updated::create($params);
    }
    ...
    public function test_post_updated() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_socialforum\event\post_updated', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'socialforum', 'update post', "discuss.php?d={$discussion->id}#p{$post->id}",
            $post->id, $socialforum->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/socialforum/discuss.php', array('d' => $discussion->id));
        $url->set_anchor('p' . $event->objectid);
        $this->assertEquals($url, $event->get_url());
    ...
    public function test_post_updated_single() {
        ...
        $this->assertInstanceOf('\mod_socialforum\event\post_updated', $event);
        $this->assertEquals($context, $event->get_context());
        $expected = array($course->id, 'socialforum', 'update post', "view.php?f={$socialforum->id}#p{$post->id}",
            $post->id, $socialforum->cmid);
        $this->assertEventLegacyLogData($expected, $event);
        $url = new \moodle_url('/mod/socialforum/view.php', array('f' => $socialforum->id));
    ...
    public function test_discussion_subscription_created() {
        ...
        \mod_socialforum\subscriptions::subscribe_user_to_discussion($user->id, $discussion);
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_socialforum\event\discussion_subscription_created', $event);


        $cm = get_coursemodule_from_instance('socialforum', $discussion->socialforum);
    ...
    public function test_discussion_subscription_created_validation() {
        ...
        $event = \mod_socialforum\event\discussion_subscription_created::create($params);
    ...
    public function test_discussion_subscription_created_validation_contextlevel() {
        ...
        $this->setExpectedException('coding_exception', 'Context level must be CONTEXT_MODULE.');
        \mod_socialforum\event\discussion_subscription_created::create($params);
    }
    ...
    public function test_discussion_subscription_deleted() {
        ...
        // Checking that the event contains the expected values.
        $this->assertInstanceOf('\mod_socialforum\event\discussion_subscription_deleted', $event);
    ...
    public function test_discussion_subscription_deleted_validation() {
        ...
        $subscription->socialforum = $socialforum->id;
        $subscription->discussion = $discussion->id;
        $subscription->preference = \mod_socialforum\subscriptions::SOCIALFORUM_DISCUSSION_UNSUBSCRIBED;
        ...
        $event = \mod_socialforum\event\discussion_subscription_deleted::create($params);
    ...
    public function test_discussion_subscription_deleted_validation_contextlevel() {
        ...
        // Without an invalid context.
        $this->setExpectedException('coding_exception', 'Context level must be CONTEXT_MODULE.');
        \mod_socialforum\event\discussion_subscription_deleted::create($params);
    }
    ...
    /**
     * Test mod_socialforum_observer methods.
     */
    public function test_observers() {
        ...
        $course = $this->getDataGenerator()->create_course();
        $trackedrecord = array('course' => $course->id, 'type' => 'general', 'forcesubscribe' => SOCIALFORUM_INITIALSUBSCRIBE);
        $untrackedrecord = array('course' => $course->id, 'type' => 'general');
        $trackedforum = $this->getDataGenerator()->create_module('socialforum', $trackedrecord);
        $untrackedforum = $this->getDataGenerator()->create_module('socialforum', $untrackedrecord);
        ...
        // The role_assign observer does it's job adding the SOCIALFORUM_subscriptions record.
        ...
        set_config('socialforum_trackingtype', 1);
        set_config('socialforum_trackreadposts', 1);

        $record = array();
        $record['course'] = $course->id;
        $record['socialforum'] = $trackedforum->id;
        $record['userid'] = $user->id;
        ...
        $record['discussion'] = $discussion->id;
        $record['userid'] = $user->id;
        $post = $socialforumgen->create_post($record);

        socialforum_tp_add_read_record($user->id, $post->id);
        socialforum_set_user_maildigest($trackedforum, 2, $user);
        socialforum_tp_stop_tracking($untrackedforum->id, $user->id);

        $this->assertEquals(1, $DB->count_records('socialforum_subscriptions'));
        $this->assertEquals(1, $DB->count_records('socialforum_digests'));
        $this->assertEquals(1, $DB->count_records('socialforum_track_prefs'));
        $this->assertEquals(1, $DB->count_records('socialforum_read'));

        // The course_module_created observer does it's job adding a subscription.
        $socialforumrecord = array('course' => $course->id, 'type' => 'general', 'forcesubscribe' => SOCIALFORUM_INITIALSUBSCRIBE);
        $extraforum = $this->getDataGenerator()->create_module('socialforum', $socialforumrecord);
        $this->assertEquals(2, $DB->count_records('socialforum_subscriptions'));

        $manplugin->unenrol_user($manualenrol, $user->id);

        $this->assertEquals(0, $DB->count_records('socialforum_digests'));
        $this->assertEquals(0, $DB->count_records('socialforum_subscriptions'));
        $this->assertEquals(0, $DB->count_records('socialforum_track_prefs'));
        $this->assertEquals(0, $DB->count_records('socialforum_read'));
    }
