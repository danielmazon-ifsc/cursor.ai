...
class mod_socialforum_external extends external_api {

    /**
     * Describes the parameters for get_socialforum.
     *
     * @return external_external_function_parameters
     * @since Moodle 2.5
     */
    public static function get_socialforums_by_courses_parameters() {
        return new external_function_parameters(
                array(
            'courseids' => new external_multiple_structure(new external_value(PARAM_INT, 'course ID', VALUE_REQUIRED, '', NULL_NOT_ALLOWED), 'Array of Course IDs', VALUE_DEFAULT, array()),
                )
        );
    }

    /**
     * Returns a list of socialforums in a provided list of courses,
     * if no list is provided all socialforums that the user can view
     * will be returned.
     *
     * @param array $courseids the course ids
     * @return array the socialforum details
     * @since Moodle 2.5
     */
    public static function get_socialforums_by_courses($courseids = array()) {
        global $CFG;

        require_once($CFG->dirroot . "/mod/socialforum/lib.php");

        $params = self::validate_parameters(self::get_socialforums_by_courses_parameters(), array('courseids' => $courseids));
        ...
        // Array to store the socialforums to return.
        $arrsocialforums = array();
        $warnings = array();
        ...
        if (!empty($params['courseids'])) {
            ...
            // Get the socialforums in this course. This function checks users visibility permissions.
            $socialforums = get_all_instances_in_courses("socialforum", $courses);
            foreach ($socialforums as $socialforum) {

                $course = $courses[$socialforum->course];
                $cm = get_coursemodule_from_instance('socialforum', $socialforum->id, $course->id);
                $context = context_module::instance($cm->id);

                // Skip socialforums we are not allowed to see discussions.
                if (!has_capability('mod/socialforum:viewdiscussion', $context)) {
                    continue;
                }

                $socialforum->name = external_format_string($socialforum->name, $context->id);
                // Format the intro before being returning using the format setting.
                list($socialforum->intro, $socialforum->introformat) = external_format_text($socialforum->intro, $socialforum->introformat, $context->id, 'mod_socialforum', 'intro', 0);
                // Discussions count. This function does static request cache.
                $socialforum->numdiscussions = socialforum_count_discussions($socialforum, $cm, $course);
                $socialforum->cmid = $socialforum->coursemodule;
                $socialforum->cancreatediscussions = socialforum_user_can_post_discussion($socialforum, null, -1, $cm, $context);

                // Add the socialforum to the array to return.
                $arrsocialforums[$socialforum->id] = $socialforum;
            }
        }

        return $arrsocialforums;
    }

    /**
     * Describes the get_socialforum return value.
     *
     * @return external_single_structure
     * @since Moodle 2.5
     */
    public static function get_socialforums_by_courses_returns() {
        return new external_multiple_structure(
                new external_single_structure(
                array(
            'id' => new external_value(PARAM_INT, 'Social Forum id'),
            'course' => new external_value(PARAM_INT, 'Course id'),
            'type' => new external_value(PARAM_TEXT, 'The socialforum type'),
            'name' => new external_value(PARAM_RAW, 'Social Forum name'),
            'intro' => new external_value(PARAM_RAW, 'The socialforum intro'),
            ...
            'numdiscussions' => new external_value(PARAM_INT, 'Number of discussions in the socialforum', VALUE_OPTIONAL),
    ...
    /**
     * Describes the parameters for get_socialforum_discussion_posts.
     *
     * @return external_external_function_parameters
     * @since Moodle 2.7
     */
    public static function get_socialforum_discussion_posts_parameters() {
        return new external_function_parameters(
                array(
    ...
    /**
     * Returns a list of socialforum posts for a discussion
     *
     * @param int $discussionid the post ids
     * @param string $sortby sort by this element (id, created or modified)
     * @param string $sortdirection sort direction: ASC or DESC
     *
     * @return array the socialforum post details
     * @since Moodle 2.7
     */
    public static function get_socialforum_discussion_posts($discussionid, $sortby = "created", $sortdirection = "DESC") {
        ...
        $params = self::validate_parameters(self::get_socialforum_discussion_posts_parameters(), array(
        ...
        $discussion = $DB->get_record('socialforum_discussions', array('id' => $discussionid), '*', MUST_EXIST);
        ...
        // This require must be here, see mod/socialforum/discuss.php.
        require_once($CFG->dirroot . "/mod/socialforum/lib.php");

        // Check they have the view socialforum capability.
        ...
        if (!$post = socialforum_get_post_full($discussion->firstpost)) {
            throw new moodle_exception('notexists', 'socialforum');
        }
        ...
        if (!socialforum_user_can_see_post($socialforum, $discussion, $post, null, $cm)) {
            throw new moodle_exception('noviewdiscussionspermission', 'socialforum');
        }
        ...
        $allposts = socialforum_get_all_discussion_posts($discussion->id, $sort, $socialforumtracked);
        ...
        foreach ($allposts as $post) {
            ...
            // Function socialforum_get_all_discussion_posts adds postread field.
            ...
            list($post->message, $post->messageformat) = external_format_text($post->message, $post->messageformat, $modcontext->id, 'mod_socialforum', 'post', $post->id);
            ...
            if (!empty($post->attachment)) {
                ...
                $fs = get_file_storage();
                if ($files = $fs->get_area_files($modcontext->id, 'mod_socialforum', 'attachment', $post->id, "filename", false)) {
                    foreach ($files as $file) {
                        $filename = $file->get_filename();
                        $fileurl = moodle_url::make_webservice_pluginfile_url(
                                        $modcontext->id, 'mod_socialforum', 'attachment', $post->id, '/', $filename);
    ...
    /**
     * Describes the get_socialforum_discussion_posts return value.
     *
     * @return external_single_structure
     * @since Moodle 2.7
     */
    public static function get_socialforum_discussion_posts_returns() {
    ...
    /**
     * Describes the parameters for get_socialforum_discussions_paginated.
     *
     * @return external_external_function_parameters
     * @since Moodle 2.8
     */
    public static function get_socialforum_discussions_paginated_parameters() {
        return new external_function_parameters(
                array(
            'socialforumid' => new external_value(PARAM_INT, 'socialforum instance id', VALUE_REQUIRED),
            'sortby' => new external_value(PARAM_ALPHA, 'sort by this element: id, timemodified, timestart or timeend', VALUE_DEFAULT, 'timemodified'),
            'sortdirection' => new external_value(PARAM_ALPHA, 'sort direction: ASC or DESC', VALUE_DEFAULT, 'DESC'),
    ...
    /**
     * Returns a list of socialforum discussions optionally sorted and paginated.
     *
     * @param int $socialforumid the socialforum instance id
     * @param string $sortby sort by this element (id, timemodified, timestart or timeend)
     * @param string $sortdirection sort direction: ASC or DESC
     * @param int $page page number
     * @param int $perpage items per page
     *
     * @return array the socialforum discussion details including warnings
     * @since Moodle 2.8
     */
    public static function get_socialforum_discussions_paginated($socialforumid, $sortby = 'timemodified', $sortdirection = 'DESC', $page = -1, $perpage = 0) {
        global $CFG, $DB, $USER, $PAGE;
        ...
        $params = self::validate_parameters(self::get_socialforum_discussions_paginated_parameters(), array(
                    'socialforumid' => $socialforumid,
                    'sortby' => $sortby,
                    'sortdirection' => $sortdirection,
                    'page' => $page,
                    'perpage' => $perpage
        ...
        // Compact/extract functions are not recommended.
        $socialforumid = $params['socialforumid'];
        $sortby = $params['sortby'];
        $sortdirection = $params['sortdirection'];
        $page = $params['page'];
        ...
        $alldiscussions = socialforum_get_discussions($cm, $sort, true, -1, -1, true, $page, $perpage, SOCIALFORUM_POSTS_ALL_USER_GROUPS);

        if ($alldiscussions) {
            $canviewfullname = has_capability('moodle/site:viewfullnames', $modcontext);

            // Get the unreads array, this takes a socialforum id and returns data for all discussions.
            $unreads = array();
            if ($cantrack = socialforum_tp_can_track_socialforums($socialforum)) {
                if ($socialforumtracked = socialforum_tp_is_tracked($socialforum)) {
                    $unreads = socialforum_get_discussions_unread($cm);
                }
            }
            // The socialforum function returns the replies for all the discussions in a given socialforum.
            $replies = socialforum_count_discussion_replies($socialforumid, $sort, -1, $page, $perpage);

            foreach ($alldiscussions as $discussion) {

                // This function checks for qanda socialforums.
                // Note that the socialforum_get_discussions returns as id the post id, not the discussion id so we need to do this.
                $discussionrec = clone $discussion;
                $discussionrec->id = $discussion->discussion;
                if (!socialforum_user_can_see_discussion($socialforum, $discussionrec, $modcontext)) {
                    $warning = array();
                    // Function socialforum_get_discussions returns socialforum_posts ids not socialforum_discussions ones.
                    $warning['item'] = 'post';
                ...
                if ($cantrack && $socialforumtracked) {
                ...
                // Rewrite embedded images URLs.
                list($discussion->message, $discussion->messageformat) = external_format_text($discussion->message, $discussion->messageformat, $modcontext->id, 'mod_socialforum', 'post', $discussion->id);
                ...
                if (!empty($discussion->attachment)) {
                    ...
                    $fs = get_file_storage();
                    if ($files = $fs->get_area_files($modcontext->id, 'mod_socialforum', 'attachment', $discussion->id, "filename", false)) {
                        foreach ($files as $file) {
                            $filename = $file->get_filename();
                            ...
                            $discussion->attachments[] = array(
                                'filename' => $filename,
                                'mimetype' => $file->get_mimetype(),
                                'fileurl' => file_encode_url($CFG->wwwroot . '/webservice/pluginfile.php', '/' . $modcontext->id . '/mod_socialforum/attachment/' . $discussion->id . '/' . $filename)
                            );
                        }
                    }
                }
    ...
    /**
     * Describes the get_socialforum_discussions_paginated return value.
     *
     * @return external_single_structure
     * @since Moodle 2.8
     */
    public static function get_socialforum_discussions_paginated_returns() {
        return new external_single_structure(
                array(
     ...
     * @since Moodle 2.9
     */
    public static function view_socialforum_parameters() {
        return new external_function_parameters(
                array(
            'socialforumid' => new external_value(PARAM_INT, 'socialforum instance id')
                )
        );
    }
     ...
     */
    public static function view_socialforum($socialforumid) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/mod/socialforum/lib.php");

        $params = self::validate_parameters(self::view_socialforum_parameters(), array(
                    'socialforumid' => $socialforumid
        ));
        $warnings = array();

        // Request and permission validation.
        $socialforum = $DB->get_record('socialforum', array('id' => $params['socialforumid']), 'id', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($socialforum, 'socialforum');

        $context = context_module::instance($cm->id);
        self::validate_context($context);

        require_capability('mod/socialforum:viewdiscussion', $context, null, true, 'noviewdiscussionspermission', 'socialforum');

        // Call the socialforum/lib API.
     ...
     * @since Moodle 2.9
     */
    public static function view_socialforum_returns() {
        return new external_single_structure(
                array(
     ...
     * @since Moodle 2.9
     */
    public static function view_socialforum_discussion_parameters() {
        return new external_function_parameters(
                array(
     ...
     * @throws moodle_exception
     */
    public static function view_socialforum_discussion($discussionid) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/mod/socialforum/lib.php");

        $params = self::validate_parameters(self::view_socialforum_discussion_parameters(), array(
                    'discussionid' => $discussionid
        ));
        $warnings = array();

        $discussion = $DB->get_record('socialforum_discussions', array('id' => $params['discussionid']), '*', MUST_EXIST);
        $socialforum = $DB->get_record('socialforum', array('id' => $discussion->socialforum), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($socialforum, 'socialforum');
     ...
     * @since Moodle 2.9
     */
    public static function view_socialforum_discussion_returns() {
        return new external_single_structure(
                array(
            'status' => new external_value(PARAM_BOOL, 'status: true if success'),
            'warnings' => new external_warnings()
    ...
    public static function add_discussion_post($postid, $subject, $message, $options = array()) {
        ...
        if (!$parent = socialforum_get_post_full($params['postid'])) {
            throw new moodle_exception('invalidparentpostid', 'socialforum');
        }

        if (!$discussion = $DB->get_record("socialforum_discussions", array("id" => $parent->discussion))) {
            throw new moodle_exception('notpartofdiscussion', 'socialforum');
        }
        ...
        if (!socialforum_user_can_post($socialforum, $discussion, $USER, $cm, $course, $context)) {
            throw new moodle_exception('nopostsocialforum', 'socialforum');
        }
        ...
        if ($postid = socialforum_add_new_post($post, $fakemform)) {
            ...
            socialforum_post_subscription($settings, $socialforum, $discussion);
        } else {
            throw new moodle_exception('couldnotadd', 'socialforum');
        }
    ...
    public static function add_discussion_parameters() {
        return new external_function_parameters(
                array(
            'socialforumid' => new external_value(PARAM_INT, 'Social Forum instance ID'),
    ...
    /**
     * Add a new discussion into an existing socialforum.
     *
     * @param int $socialforumid the socialforum instance id
     ...
     * @throws moodle_exception
     */
    public static function add_discussion($socialforumid, $subject, $message, $groupid = -1, $options = array()) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/mod/socialforum/lib.php");

        $params = self::validate_parameters(self::add_discussion_parameters(), array(
                    'socialforumid' => $socialforumid,
                    'subject' => $subject,
                    'message' => $message,
                    'groupid' => $groupid,
        ...
        // Request and permission validation.
        $socialforum = $DB->get_record('socialforum', array('id' => $params['socialforumid']), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($socialforum, 'socialforum');
        ...
        if (!groups_get_activity_groupmode($cm)) {
            ...
            if ($groupid === -1 or empty($params['groupid'])) {
                ...
                // Here we rely in the group passed, socialforum_user_can_post_discussion will validate the group.
                $groupid = $params['groupid'];
            }
        }

        if (!socialforum_user_can_post_discussion($socialforum, $groupid, -1, $cm, $context)) {
            throw new moodle_exception('cannotcreatediscussion', 'socialforum');
        }
        ...
        if (has_capability('mod/socialforum:pindiscussions', $context) && $options['discussionpinned']) {
        ...
        if ($discussionid = socialforum_add_discussion($discussion, $fakemform)) {
    ...
    /**
     * Check if the current user can add discussions in the given socialforum (and optionally for the given group).
     *
     * @param int $socialforumid the socialforum instance id
     ...
     * @throws moodle_exception
     */
    public static function can_add_discussion($socialforumid, $groupid = null) {
        global $DB, $CFG;
        require_once($CFG->dirroot . "/mod/socialforum/lib.php");

        $params = self::validate_parameters(self::can_add_discussion_parameters(), array(
                    'socialforumid' => $socialforumid,
                    'groupid' => $groupid,
        ));
        $warnings = array();
        ...
        $status = socialforum_user_can_post_discussion($socialforum, $params['groupid'], -1, $cm, $context);
