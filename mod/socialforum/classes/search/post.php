...
/**
 * Social Forum posts search area
 *
...
namespace mod_socialforum\search;
...
class post extends \core_search\base_mod {
     ...
     */
    protected $socialforumsdata = array();
    ...
    /**
     * Returns recordset containing required data for indexing socialforum posts.
     *
    ...
    public function get_recordset_by_timestamp($modifiedfrom = 0) {
        ...
        $sql = 'SELECT fp.*, f.id AS socialforumid, f.course AS courseid
                  FROM {socialforum_posts} fp
                  JOIN {socialforum_discussions} fd ON fd.id = fp.discussion
                  JOIN {socialforum} f ON f.id = fd.socialforum
    ...
    public function get_document($record, $options = array()) {
        ...
        try {
            $cm = $this->get_cm('socialforum', $record->socialforumid, $record->courseid);
    ...
    /**
     * Add the socialforum post attachments.
     *
    ...
    public function attach_files($document) {
        ...
        $cm = $this->get_cm('socialforum', $post->socialforum, $document->get('courseid'));
        $context = \context_module::instance($cm->id);

        // Get the files and attach them.
        $fs = get_file_storage();
        $files = $fs->get_area_files($context->id, 'mod_socialforum', 'attachment', $postid, "filename", false);
     ...
     * @param int $id Social Forum post id
     * @return bool
     */
    public function check_access($id) {
        ...
        try {
            ...
            $socialforum = $this->get_socialforum($post->socialforum);
            $discussion = $this->get_discussion($post->discussion);
            $cminfo = $this->get_cm('socialforum', $socialforum->id, $socialforum->course);
    ...
    /**
     * Link to the socialforum post discussion
     *
    ...
    public function get_doc_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/mod/socialforum/discuss.php', array('d' => $post->discussion));
    }

    /**
     * Link to the socialforum.
     *
     * @param \core_search\document $doc
     * @return \moodle_url
     */
    public function get_context_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/mod/socialforum/view.php', array('id' => $contextmodule->instanceid));
    }

    /**
     * Returns the specified socialforum post from its internal cache.
     *
     * @throws \dml_missing_record_exception
     * @param int $postid
     * @return stdClass
    ...
    protected function get_post($postid) {
        if (empty($this->postsdata[$postid])) {
            $this->postsdata[$postid] = socialforum_get_post_full($postid);
            if (!$this->postsdata[$postid]) {
                throw new \dml_missing_record_exception('socialforum_posts');
            }
        }
    ...
    /**
     * Returns the specified socialforum checking the internal cache.
     *
     ...
     * @return stdClass
     */
    protected function get_socialforum($socialforumid) {
        global $DB;

        if (empty($this->socialforumsdata[$socialforumid])) {
            $this->socialforumsdata[$socialforumid] = $DB->get_record('socialforum', array('id' => $socialforumid), '*', MUST_EXIST);
        }
        return $this->socialforumsdata[$socialforumid];
    }
    ...
    protected function get_discussion($discussionid) {
        ...
        if (empty($this->discussionsdata[$discussionid])) {
            $this->discussionsdata[$discussionid] = $DB->get_record('socialforum_discussions', array('id' => $discussionid), '*', MUST_EXIST);
        }
