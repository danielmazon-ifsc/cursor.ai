...
/**
 * A Handler to process replies to socialforum posts.
 *
...
namespace mod_socialforum\message\inbound;
...
class reply_handler extends \core\message\inbound\handler {
    ...
    public function get_description() {
        return get_string('reply_handler', 'mod_socialforum');
    }
    ...
    public function get_name() {
        return get_string('reply_handler_name', 'mod_socialforum');
    }
    ...
    public function process_message(\stdClass $record, \stdClass $messagedata) {
        ...
        $post = $DB->get_record('socialforum_posts', array('id' => $record->datavalue));
        if (!$post) {
        ...
        $cm = get_fast_modinfo($course->id)->instances['socialforum'][$socialforum->id];
        ...
        if (!$canpost) {
            $data = new \stdClass();
            $data->socialforum = $socialforum;
            throw new \core\message\inbound\processing_failed_exception('messageinboundnopostsocialforum', 'mod_socialforum', $data);
        }
        ...
        if (!\core_availability\info_module::is_user_visible($cm)) {
            $data = new \stdClass();
            $data->socialforum = $socialforum;
            throw new \core\message\inbound\processing_failed_exception('messageinboundsocialforumhidden', 'mod_socialforum', $data);
        }
        ...
        if (!empty($thresholdwarning) && !$thresholdwarning->canpost) {
            ...
            throw new \core\message\inbound\processing_failed_exception('messageinboundthresholdhit', 'mod_socialforum', $data);
        }

        $subject = clean_param($messagedata->envelope->subject, PARAM_TEXT);
        $restring = get_string('re', 'mod_socialforum');
        ...
        if (!empty($messagedata->attachments['attachment']) && count($messagedata->attachments['attachment'])) {
            ...
            if (empty($socialforum->maxattachments) || $socialforum->maxbytes == 1 ||
                    !has_capability('mod/socialforum:createattachment', $modcontext)) {
                // Attachments are not allowed.
                mtrace("--> User does not have permission to attach files in this socialforum. Rejecting e-mail.");

                $data = new \stdClass();
                $data->socialforum = $socialforum;
                $data->attachmentcount = $attachmentcount;
                throw new \core\message\inbound\processing_failed_exception('messageinboundattachmentdisallowed', 'mod_socialforum', $data);
            }

            if ($socialforum->maxattachments < $attachmentcount) {
                // Too many attachments.
                mtrace("--> User attached {$attachmentcount} files when only {$socialforum->maxattachments} where allowed. "
                        . " Rejecting e-mail.");

                $data = new \stdClass();
                $data->socialforum = $socialforum;
                $data->attachmentcount = $attachmentcount;
                throw new \core\message\inbound\processing_failed_exception('messageinboundfilecountexceeded', 'mod_socialforum', $data);
            }
            ...
            if ($socialforum->maxbytes < $filesize) {
                // Too many attachments.
                mtrace("--> User attached {$filesize} bytes of files when only {$socialforum->maxbytes} where allowed. "
                        . "Rejecting e-mail.");
                $data = new \stdClass();
                $data->socialforum = $socialforum;
                $data->maxbytes = display_size($socialforum->maxbytes);
                $data->filesize = display_size($filesize);
                throw new \core\message\inbound\processing_failed_exception('messageinboundfilesizeexceeded', 'mod_socialforum', $data);
            }
        }
        ...
        $addpost->id = socialforum_add_new_post($addpost, true);
        ...
        $event->add_record_snapshot('socialforum_posts', $addpost);
        $event->add_record_snapshot('socialforum_discussions', $discussion);
        $event->trigger();

        // Update completion state.
        $completion = new \completion_info($course);
        if ($completion->is_enabled($cm) && ($socialforum->completionreplies || $socialforum->completionposts)) {
            $completion->update_state($cm, COMPLETION_COMPLETE);

            mtrace("--> Updating completion status for user {$USER->id} in socialforum {$socialforum->id} for post {$addpost->id}.");
        }
    ...
    public function get_success_message(\stdClass $messagedata, $handlerresult) {
        ...
        $discussionurl = new \moodle_url('/mod/socialforum/discuss.php', array('d' => $handlerresult->discussion));
        $discussionurl->set_anchor('p' . $handlerresult->id);
        $a->discussionurl = $discussionurl->out();

        $message = new \stdClass();
        $message->plain = get_string('postbymailsuccess', 'mod_socialforum', $a);
        $message->html = get_string('postbymailsuccess_html', 'mod_socialforum', $a);
        return $message;
    }
