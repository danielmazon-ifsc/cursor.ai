...
/**
 * File containing the form definition to post in the socialforum.
 *
...
/**
 * Class to post in a socialforum.
 *
 * @package   mod_socialforum
 * @copyright 2016 Viddia (http://viddia.com.br)
 * @author    Ricardo Drummond
...
class mod_socialforum_post_form extends moodleform {
    ...
    /**
     * Returns the options array to use in filemanager for socialforum attachments
     *
    ...
    /**
     * Returns the options array to use in socialforum text editor
     *
    ...
    function definition() {
        ...
        if (empty($post->subject)) {
            $mform->addElement('text', 'subject', get_string('subject', 'mod_socialforum'), 'size="48"');
        ...
        $mform->addElement('editor', 'message', get_string('message', 'mod_socialforum'), array('rows' => '8'), self::editor_options($modcontext, (empty($post->id) ? null : $post->id)));
        ...
        if (\mod_socialforum\subscriptions::is_forcesubscribed($socialforum)) {
            $mform->addElement('checkbox', 'discussionsubscribe', get_string('discussionsubscription', 'mod_socialforum'));
            $mform->freeze('discussionsubscribe');
            $mform->setDefaults('discussionsubscribe', 0);
            $mform->addHelpButton('discussionsubscribe', 'forcesubscribed', 'mod_socialforum');
        } else if (\mod_socialforum\subscriptions::subscription_disabled($socialforum) && !$manageactivities) {
            $mform->addElement('checkbox', 'discussionsubscribe', get_string('discussionsubscription', 'mod_socialforum'));
            $mform->freeze('discussionsubscribe');
            $mform->setDefaults('discussionsubscribe', 0);
            $mform->addHelpButton('discussionsubscribe', 'disallowsubscription', 'socialforum');
        } else {
            $mform->addElement('checkbox', 'discussionsubscribe', get_string('discussionsubscription', 'mod_socialforum'));
            $mform->addHelpButton('discussionsubscribe', 'discussionsubscription', 'socialforum');
        }

        if (!empty($socialforum->maxattachments) && $socialforum->maxbytes != 1 && has_capability('mod/socialforum:createattachment', $modcontext)) {  //  1 = No attachments at all
            $mform->addElement('filemanager', 'attachments', get_string('attachment', 'mod_socialforum'), null, self::attachment_options($socialforum));
            $mform->addHelpButton('attachments', 'attachment', 'socialforum');
        }

        if (!$post->parent && has_capability('mod/socialforum:pindiscussions', $modcontext)) {
            $mform->addElement('checkbox', 'pinned', get_string('discussionpinned', 'mod_socialforum'));
            $mform->addHelpButton('pinned', 'discussionpinned', 'socialforum');
        }

        if (empty($post->id) && $manageactivities) {
            $mform->addElement('checkbox', 'mailnow', get_string('mailnow', 'mod_socialforum'));
        }

        if (!empty($CFG->socialforum_enabletimedposts) && !$post->parent && has_capability('mod/socialforum:viewhiddentimedposts', $coursecontext)) { // hack alert
            $mform->addElement('header', 'displayperiod', get_string('displayperiod', 'mod_socialforum'));

            $mform->addElement('date_time_selector', 'timestart', get_string('displaystart', 'mod_socialforum'), array('optional' => true));
            $mform->addHelpButton('timestart', 'displaystart', 'socialforum');

            $mform->addElement('date_time_selector', 'timeend', get_string('displayend', 'mod_socialforum'), array('optional' => true));
            $mform->addHelpButton('timeend', 'displayend', 'socialforum');
        } else {
        ...
        if ($groupmode = groups_get_activity_groupmode($cm, $course)) {
            ...
            foreach ($groupdata as $groupid => $group) {
                ...
                if (socialforum_user_can_post_discussion($socialforum, $groupid, null, $cm, $modcontext)) {
            ...
            $canposttoowngroups = $canposttoowngroups && has_capability('mod/socialforum:canposttomygroups', $modcontext);
            if ($canposttoowngroups) {
                ...
                // user is a member of.
                $mform->addElement('checkbox', 'posttomygroups', get_string('posttomygroups', 'mod_socialforum'));
                $mform->addHelpButton('posttomygroups', 'posttomygroups', 'socialforum');
            ...
            if (socialforum_user_can_post_discussion($socialforum, -1, null, $cm, $modcontext)) {
            ...
            // We allow this because the post may have been moved from another socialforum where groups are not available.
            // We show this even if no groups are available as groups *may* have been available but now are not.
            $canselectgroupformove = $groupcount && !empty($post->edit) && has_capability('mod/socialforum:movediscussions', $modcontext);
        ...
        if (isset($post->edit)) { // hack alert
            ...
            $submit_string = get_string('posttosocialforum', 'mod_socialforum');
        }
        ...
        $mform->addElement('hidden', 'socialforum');
    ...
    function validation($data, $files) {
        ...
        if (($data['timeend'] != 0) && ($data['timestart'] != 0) && $data['timeend'] <= $data['timestart']) {
            $errors['timeend'] = get_string('timestartenderror', 'mod_socialforum');
        }
        if (empty($data['message']['text'])) {
            $errors['message'] = get_string('erroremptymessage', 'mod_socialforum');
        }
        if (empty($data['subject'])) {
            $errors['subject'] = get_string('erroremptysubject', 'mod_socialforum');
        }
        return $errors;
    }
